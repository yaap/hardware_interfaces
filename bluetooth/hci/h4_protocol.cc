...
H4Protocol::H4Protocol(int fd, PacketReadCallback cmd_cb, PacketReadCallback acl_cb,
...
size_t H4Protocol::Send(PacketType type, const uint8_t* data, size_t length) {
    ...
    while (remaining_bytes > 0) {
        ...
        ALOGW("%s: %zu/%zu bytes written - retrying remaining %zu bytes", __func__, bytes_written,
...
size_t H4Protocol::OnPacketReady(const std::vector<uint8_t>& packet) {
    switch (hci_packet_type_) {
        ...
        case PacketType::ISO_DATA:
            ...
            break;
        default: {
            LOG_ALWAYS_FATAL("Bad packet type 0x%x", static_cast<int>(hci_packet_type_));
        }
    }
    return packet.size();
}
...
void H4Protocol::SendDataToPacketizer(uint8_t* buffer, size_t length) {
    ...
    while (buffer_offset < input_buffer.size()) {
        if (hci_packet_type_ == PacketType::UNKNOWN) {
            hci_packet_type_ = static_cast<PacketType>(input_buffer.data()[buffer_offset]);
            buffer_offset += 1;
        } else {
            bool packet_ready =
                    hci_packetizer_.OnDataReady(hci_packet_type_, input_buffer, &buffer_offset);
            if (packet_ready) {
...
void H4Protocol::OnDataReady() {
    ...
    ssize_t bytes_read = TEMP_FAILURE_RETRY(read(uart_fd_, buffer, kMaxPacketLength));
    if (bytes_read == 0) {
