...
static char iso_data[100] = "A plane angle is the inclination to one another of two lines in a ...";
...
class H4ProtocolTest : public ::testing::Test {
  protected:
    void SetUp() override {
        ...
        h4_hci_ = std::make_shared<H4Protocol>(stack_uart_fd_, cmd_cb_.AsStdFunction(),
                                               acl_cb_.AsStdFunction(), sco_cb_.AsStdFunction(),
                                               event_cb_.AsStdFunction(), iso_cb_.AsStdFunction(),
    ...
    void SendAndReadUartOutbound(PacketType type, char* data) {
        ...
        for (i = 0; i < uart_length; i++) {
            ...
            TEMP_FAILURE_RETRY(select(chip_uart_fd_ + 1, &read_fds, nullptr, nullptr, nullptr));
    ...
    void ExpectInboundAclData(char* payload, std::promise<void>* promise) {
        ...
        EXPECT_CALL(acl_cb_, Call(PacketMatches(header_ + 1, kAclHeaderSize, payload)))
    ...
    void ExpectInboundScoData(char* payload, std::promise<void>* promise) {
        ...
        EXPECT_CALL(sco_cb_, Call(PacketMatches(header_ + 1, kScoHeaderSize, payload)))
                .WillOnce(Notify(promise));
    }
    ...
    void ExpectInboundEvent(char* payload, std::promise<void>* promise) {
        ...
        EXPECT_CALL(event_cb_, Call(PacketMatches(header_ + 1, kEventHeaderSize, payload)))
                .WillOnce(Notify(promise));
    }
    ...
    void ExpectInboundIsoData(char* payload, std::promise<void>* promise) {
        ...
        EXPECT_CALL(iso_cb_, Call(PacketMatches(header_ + 1, kIsoHeaderSize, payload)))
                .WillOnce(Notify(promise));
    }
    ...
    void WriteAndExpectManyInboundAclDataPackets(char* payload) {
        ...
        // h4 type[1] + handle[2] + size[2]
        char preamble[5] = {static_cast<uint8_t>(PacketType::ACL_DATA), 19, 92, 0, 0};
        int length = strlen(payload);
        preamble[3] = length & 0xFF;
        preamble[4] = (length >> 8) & 0xFF;

        EXPECT_CALL(acl_cb_, Call(PacketMatches(preamble + 1, sizeof(preamble) - 1, payload)))
                .Times(kNumPackets);
    ...
    void WriteAndExpectManyAclDataPacketsDifferentOffsetsShort() {
        ...
        EXPECT_CALL(acl_cb_, Call(PacketMatches(preamble + 1, kAclHeaderSize, short_payload)))
                .Times(kNumPackets);
        ...
        while (written_bytes < total_bytes) {
            ...
            TEMP_FAILURE_RETRY(write(chip_uart_fd_, all_packets + written_bytes, to_write));
...
TEST_F(H4ProtocolTest, TestPartialWrites) {
    ...
    EXPECT_CALL(acl_cb_, Call(PacketMatches(header_ + 1, sizeof(header_) - 1, acl_data)))
            .Times(kNumIntervals);
    ...
    for (size_t interval = 1; interval < kNumIntervals + 1; interval++) {
        ...
        if (interval < kAclHeaderSize) {
            ...
            CallDataReady();
            TEMP_FAILURE_RETRY(
                    write(chip_uart_fd_, header_ + interval, kAclHeaderSize + 1 - interval));
            CallDataReady();
        } else {
            TEMP_FAILURE_RETRY(write(chip_uart_fd_, header_, kAclHeaderSize + 1));
            CallDataReady();
        ...
        if (extra_bytes) {
            TEMP_FAILURE_RETRY(
                    write(chip_uart_fd_, acl_data + payload_len - extra_bytes, extra_bytes));
            CallDataReady();
        }
    }
}
...
class H4ProtocolAsyncTest : public H4ProtocolTest {
  protected:
    void SetUp() override {
        ...
        fd_watcher_.WatchFdForNonBlockingReads(stack_uart_fd_,
                                               [this](int) { h4_hci_->OnDataReady(); });
    }
