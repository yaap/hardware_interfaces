...
class BluetoothSocketCallback : public BnBluetoothSocketCallback {
  public:
    BluetoothSocketCallback(
            const std::function<void(int64_t in_socketId,
    ...
    ScopedAStatus openedComplete(int64_t in_socketId,
                                 ::aidl::android::hardware::bluetooth::socket::Status in_status,
    ...
    ScopedAStatus close(int64_t /* in_socketId */, const std::string& /* in_reason */) override {
        return ::ndk::ScopedAStatus::ok();
    }

  private:
    std::function<void(int64_t in_socketId,
                       ::aidl::android::hardware::bluetooth::socket::Status in_status,
...
TEST_P(BluetoothSocketTest, registerCallback) {
    ...
    std::shared_ptr<BluetoothSocketCallback> callback =
            ndk::SharedRefBase::make<BluetoothSocketCallback>(
                    [&open_cb_promise](auto /* socket_id */, auto /* status */, auto /* reason */) {
...
TEST_P(BluetoothSocketTest, GetSocketCapabilities) {
    ...
    ScopedAStatus status = bluetooth_socket_->getSocketCapabilities(&socket_capabilities);
    ASSERT_TRUE(status.isOk());
    ASSERT_TRUE(socket_capabilities.leCocCapabilities.numberOfSupportedSockets >= 0);
    ...
    ASSERT_TRUE(socket_capabilities.rfcommCapabilities.numberOfSupportedSockets >= 0);
...
INSTANTIATE_TEST_SUITE_P(
        PerInstance, BluetoothSocketTest,
        testing::ValuesIn(android::getAidlHalInstanceNames(IBluetoothSocket::descriptor)),
