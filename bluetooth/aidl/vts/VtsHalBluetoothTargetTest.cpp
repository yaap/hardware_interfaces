...
static constexpr std::chrono::milliseconds kWaitForDegragedHwModeInitTimeout(8000);
...
class ThroughputLogger {
  public:
    explicit ThroughputLogger(std::string task)
        : total_bytes_(0), task_(std::move(task)), start_time_(std::chrono::steady_clock::now()) {}
...
class BluetoothAidlTest : public ::testing::TestWithParam<std::string> {
    std::chrono::time_point<std::chrono::system_clock> time_after_initialize_complete;
  ...
  public:
    void SetUp() override {
        ...
        bluetooth_hci_death_recipient = AIBinder_DeathRecipient_new(on_binder_death);
        ASSERT_NE(bluetooth_hci_death_recipient, nullptr);
        ASSERT_EQ(STATUS_OK, AIBinder_linkToDeath(hci->asBinder().get(),
        ...
        std::chrono::time_point<std::chrono::system_clock> timeout_after_initialize =
    ...
    void wait_for_command_complete_event(OpCode opCode, std::vector<uint8_t>& complete_event);
    ...
    void reassemble_sco_loopback_pkt(std::vector<uint8_t>& scoPackets, size_t size);
    ...
    class BluetoothHciCallbacks
      ...
      public:
        explicit BluetoothHciCallbacks(BluetoothAidlTest& parent) : parent_(parent) {};

        ~BluetoothHciCallbacks() override = default;

        ndk::ScopedAStatus initializationComplete(Status status) override {
            if (status == Status::SUCCESS) {
                parent_.time_after_initialize_complete = std::chrono::system_clock::now();
            }
...
void BluetoothAidlTest::handle_no_ops() {
    while (!event_queue.empty()) {
        ...
        auto event_view = ::bluetooth::hci::EventView::Create(
                ::bluetooth::hci::PacketView<true>(std::make_shared<std::vector<uint8_t>>(event)));
        if (!event_view.IsValid()) {
            break;
        }

        auto status_view = ::bluetooth::hci::CommandStatusView::Create(event_view);
        auto complete_view = ::bluetooth::hci::CommandCompleteView::Create(event_view);

        bool is_complete_no_op = complete_view.IsValid() &&
                                 complete_view.GetCommandOpCode() == ::bluetooth::hci::OpCode::NONE;
        bool is_status_no_op = status_view.IsValid() &&
...
void BluetoothAidlTest::discard_qca_debugging() {
    while (!acl_queue.empty()) {
        ...
        auto acl_view = ::bluetooth::hci::AclView::Create(::bluetooth::hci::PacketView<true>(
...
void BluetoothAidlTest::wait_for_command_complete_event(OpCode opCode,
...
void BluetoothAidlTest::wait_and_validate_command_complete_event(::bluetooth::hci::OpCode opCode) {
    std::vector<uint8_t> complete_event;
    ASSERT_NO_FATAL_FAILURE(wait_for_command_complete_event(opCode, complete_event));
}
...
void BluetoothAidlTest::sendAndCheckHci(int num_packets) {
    ...
    for (int n = 0; n < num_packets; n++) {
        ...
        for (size_t i = new_name_length; i < new_name_length + sizeof(number) - 1; i++) {
...
void BluetoothAidlTest::sendAndCheckAcl(int num_packets, size_t size, uint16_t handle) {
    ThroughputLogger logger{__func__};
    for (int n = 0; n < num_packets; n++) {
        ...
        ::bluetooth::hci::AclBuilder::Create(
                handle, ::bluetooth::hci::PacketBoundaryFlag::FIRST_AUTOMATICALLY_FLUSHABLE,
        ...
        ASSERT_TRUE(acl_queue.tryPopWithTimeout(acl_loopback, kWaitForAclDataTimeout));
...
void BluetoothAidlTest::enterLoopbackMode() {
    std::vector<uint8_t> cmd;
    ::bluetooth::packet::BitInserter bi{cmd};
    ::bluetooth::hci::WriteLoopbackModeBuilder::Create(bluetooth::hci::LoopbackMode::ENABLE_LOCAL)
            ->Serialize(bi);
    hci->sendHciCommand(cmd);
    ...
    while (true) {
        ...
        if (event_view.GetEventCode() == ::bluetooth::hci::EventCode::CONNECTION_COMPLETE) {
            auto complete_view = ::bluetooth::hci::ConnectionCompleteView::Create(event_view);
            ASSERT_TRUE(complete_view.IsValid());
            switch (complete_view.GetLinkType()) {
                ...
                case ::bluetooth::hci::LinkType::SCO:
                    ...
                    break;
                default:
                    ASSERT_EQ(complete_view.GetLinkType(), ::bluetooth::hci::LinkType::ACL);
            }
            connection_event_count++;
        } else {
            auto command_complete_view = ::bluetooth::hci::WriteLoopbackModeCompleteView::Create(
                    ::bluetooth::hci::CommandCompleteView::Create(event_view));
            ASSERT_TRUE(command_complete_view.IsValid());
            ASSERT_EQ(::bluetooth::hci::ErrorCode::SUCCESS, command_complete_view.GetStatus());
            command_complete_received = true;
        }
    }
}

void BluetoothAidlTest::send_and_wait_for_cmd_complete(std::unique_ptr<CommandBuilder> cmd,
                                                       std::vector<uint8_t>& cmd_complete) {
    ...
    ASSERT_NO_FATAL_FAILURE(wait_for_command_complete_event(view.GetOpCode(), cmd_complete));
}
...
TEST_P(BluetoothAidlTest, HciVersionTest) {
    ...
    std::vector<uint8_t> event;
    ASSERT_TRUE(event_queue.pop(event));
    auto complete_view = ::bluetooth::hci::ReadLocalVersionInformationCompleteView::Create(
            ::bluetooth::hci::CommandCompleteView::Create(
                    ::bluetooth::hci::EventView::Create(::bluetooth::hci::PacketView<true>(
                            std::make_shared<std::vector<uint8_t>>(event)))));
    ASSERT_TRUE(complete_view.IsValid());
...
TEST_P(BluetoothAidlTest, HciUnknownCommand) {
    std::vector<uint8_t> cmd;
    ::bluetooth::packet::BitInserter bi{cmd};
    ::bluetooth::hci::CommandBuilder::Create(static_cast<::bluetooth::hci::OpCode>(0x3cff),
    ...
    switch (event_view.GetEventCode()) {
        case ::bluetooth::hci::EventCode::COMMAND_COMPLETE: {
            auto command_complete = ::bluetooth::hci::CommandCompleteView::Create(event_view);
        ...
        case ::bluetooth::hci::EventCode::COMMAND_STATUS: {
            auto command_status = ::bluetooth::hci::CommandStatusView::Create(event_view);
            ASSERT_TRUE(command_status.IsValid());
            ASSERT_EQ(command_status.GetStatus(), ::bluetooth::hci::ErrorCode::UNKNOWN_HCI_COMMAND);
        } break;
        default:
            ADD_FAILURE();
    }
...
TEST_P(BluetoothAidlTest, LoopbackModeSingleAcl) {
    ...
    if (!acl_connection_handles.empty()) {
        ...
        sendAndCheckAcl(1, max_acl_data_packet_length - 1, acl_connection_handles[0]);
...
TEST_P(BluetoothAidlTest, SetEventMask) {
    ...
    hci->sendHciCommand(cmd);
    wait_and_validate_command_complete_event(::bluetooth::hci::OpCode::SET_EVENT_MASK);
}
...
TEST_P(BluetoothAidlTest, SetLeEventMask) {
    ...
    hci->sendHciCommand(cmd);
    wait_and_validate_command_complete_event(::bluetooth::hci::OpCode::LE_SET_EVENT_MASK);
}

// Call initialize twice, second one should fail.
TEST_P(BluetoothAidlTest, CallInitializeTwice) {
    class SecondCb : public aidl::android::hardware::bluetooth::BnBluetoothHciCallbacks {
      public:
        ndk::ScopedAStatus initializationComplete(Status status) override {
...
TEST_P(BluetoothAidlTest, Vsr_Bluetooth5Requirements) {
    ...
    send_and_wait_for_cmd_complete(ReadLocalVersionInformationBuilder::Create(), version_event);
    auto version_view = ReadLocalVersionInformationCompleteView::Create(
            CommandCompleteView::Create(EventView::Create(
                    PacketView<true>(std::make_shared<std::vector<uint8_t>>(version_event)))));
    ...
    ASSERT_GE(static_cast<int>(version.lmp_version_), static_cast<int>(version.hci_version_));
    ...
    auto le_features_view = LeReadLocalSupportedFeaturesCompleteView::Create(
            CommandCompleteView::Create(EventView::Create(
                    PacketView<true>(std::make_shared<std::vector<uint8_t>>(le_features_event)))));
    ...
    ASSERT_TRUE(le_features & static_cast<uint64_t>(LLFeaturesBits::LE_CODED_PHY));
    ASSERT_TRUE(le_features & static_cast<uint64_t>(LLFeaturesBits::LE_EXTENDED_ADVERTISING));
    ASSERT_TRUE(le_features & static_cast<uint64_t>(LLFeaturesBits::LE_PERIODIC_ADVERTISING));

    std::vector<uint8_t> num_adv_set_event;
    send_and_wait_for_cmd_complete(LeReadNumberOfSupportedAdvertisingSetsBuilder::Create(),
                                   num_adv_set_event);
    auto num_adv_set_view = LeReadNumberOfSupportedAdvertisingSetsCompleteView::Create(
            CommandCompleteView::Create(EventView::Create(
                    PacketView<true>(std::make_shared<std::vector<uint8_t>>(num_adv_set_event)))));
    ...
    ASSERT_EQ(::bluetooth::hci::ErrorCode::SUCCESS, num_resolving_list_view.GetStatus());
...
INSTANTIATE_TEST_SUITE_P(
        PerInstance, BluetoothAidlTest,
        testing::ValuesIn(android::getAidlHalInstanceNames(IBluetoothHci::descriptor)),
