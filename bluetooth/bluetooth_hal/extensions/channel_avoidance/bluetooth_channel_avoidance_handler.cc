...
HalPacket BluetoothChannelAvoidanceHandler::BuildSetChannelAvoidanceCommand(
    ...
    uint8_t cmd_length = HciConstants::kHciCommandPreambleSize + kHciChannelAvoidanceMapSize;
...
bool BluetoothChannelAvoidanceHandler::SetBluetoothChannelStatus(
    ...
    LOG(INFO) << __func__ << ": Setting Channel Map <" << BytesToHexString(channel_map) << ">.";
...
void BluetoothChannelAvoidanceHandler::OnCommandCallback(const HalPacket& event_packet) {
    ...
    LOG(success ? INFO : WARNING) << __func__ << ": Set Channel Avoidance VSE "
                                  << (success ? "succeeded" : "failed") << ". Status: 0x"
                                  << std::hex
...
void BluetoothChannelAvoidanceHandler::OnMonitorPacketCallback(
        [[maybe_unused]] MonitorMode mode, [[maybe_unused]] const HalPacket& packet) {}
