...
std::string logPowerLimit(uint8_t br_cap, uint8_t edr_cap, uint8_t ble_cap, uint8_t hr_cap) {
    return StringPrintf("<br=%d, edr=%d, ble=%d, hr=%d>", br_cap, edr_cap, ble_cap, hr_cap);
}

uint8_t scopeCap(uint8_t cap, bool high_resolution_cap) {
    if (cap > 80) {
        LOG(WARNING) << __func__ << ": cap " << +cap << " is greater than 80, set to 80";
        cap = 80;
    } else if (cap < 0) {
        LOG(WARNING) << __func__ << ": cap " << +cap << " is smaller than 0, set to 0";
        cap = 0;
    }
...
HalPacket BluetoothSarHandler::BuildCommandHRMode(const std::array<uint8_t, 4>& chain_0_cap,
    ...
    command[4] = is_ble_non_connection_enabled ? kHciVscSetPowerCapSubOpCodeLENonConnectionMode
...
HalPacket BluetoothSarHandler::BuildCommand(const std::array<uint8_t, 3>& chain_0_cap,
...
HalPacket BluetoothSarHandler::BuildCommand(uint8_t br_cap, uint8_t edr_cap, uint8_t ble_cap,
                                            bool high_resolution_cap) {
    const std::array<uint8_t, 3> chain_cap({br_cap, edr_cap, ble_cap});
    const std::array<uint8_t, 6> beamforming_cap(
            {br_cap, edr_cap, ble_cap, br_cap, edr_cap, ble_cap});
    return BuildCommand(chain_cap, chain_cap, beamforming_cap, high_resolution_cap);
}
...
bool BluetoothSarHandler::SetBluetoothTechBasedTxPowerCap(int8_t br_cap, int8_t edr_cap,
                                                          int8_t ble_cap) {
    if (!IsBluetoothEnabled()) {
    ...
    CLIENT_LOG(INFO) << __func__ << ": " << logPowerLimit(br_cap, edr_cap, ble_cap);
    HalPacket set_power_cap = BuildCommand(br_cap, edr_cap, ble_cap, high_resolution_cap_);
    return SendCommand(set_power_cap);
}
...
bool BluetoothSarHandler::SetBluetoothModeBasedTxPowerCap(
    ...
    tx_power_cap_ss << "Chain 0 Power Cap:"
                    ...
                    << logPowerLimit(beamforming_cap[0], beamforming_cap[1], beamforming_cap[2])
                    << ", Chain 1:"
                    << logPowerLimit(beamforming_cap[3], beamforming_cap[4], beamforming_cap[5]);

    if (!IsBluetoothEnabled()) {
        CLIENT_LOG(WARNING) << __func__ << ": Unable to set power cap - " << tx_power_cap_ss.str();
        return false;
    }
    CLIENT_LOG(INFO) << __func__ << ": " << tx_power_cap_ss.str();

    HalPacket set_power_cap =
            BuildCommand(chain_0_cap, chain_1_cap, beamforming_cap, high_resolution_cap_);
    return SendCommand(set_power_cap);
}
bool BluetoothSarHandler::SetBluetoothModeBasedTxPowerCapPlusHR(
    ...
    std::stringstream tx_power_cap_ss;
    tx_power_cap_ss << "Chain 0 Power Cap:"
                    << logPowerLimit(chain_0_cap[0], chain_0_cap[1], chain_0_cap[2], chain_0_cap[3])
                    << ", Chain 1 Power Cap:"
                    << logPowerLimit(chain_1_cap[0], chain_1_cap[1], chain_1_cap[2], chain_1_cap[3])
                    << ", Beamforming Power Cap Chain 0: "
                    << logPowerLimit(beamforming_cap[0], beamforming_cap[1], beamforming_cap[2],
                                     beamforming_cap[3])
                    << ", Chain 1:"
                    << logPowerLimit(beamforming_cap[4], beamforming_cap[5], beamforming_cap[6],
                                     beamforming_cap[7]);
    ...
    HalPacket set_power_cap =
            BuildCommandHRMode(chain_0_cap, chain_1_cap, beamforming_cap, high_resolution_cap_,
...
void BluetoothSarHandler::OnMonitorPacketCallback([[maybe_unused]] MonitorMode mode,
...
void BluetoothSarHandler::OnBluetoothEnabled() {
    LOG(DEBUG) << __func__;
    high_resolution_cap_ = HalConfigLoader::GetLoader().IsSarBackoffHighResolutionEnabled();
    is_ble_non_connection_enabled_ = HalConfigLoader::GetLoader().IsBleNonConnectionSarEnabled();
}

void BluetoothSarHandler::OnBluetoothDisabled() {
    LOG(DEBUG) << __func__;
}
