...
class TestSarHandler : public BluetoothSarHandler {
  public:
    HalPacket BuildCommandHRModeWrapper(const std::array<uint8_t, 4>& chain_0_cap,
        ...
        return BuildCommandHRMode(chain_0_cap, chain_1_cap, beamforming_cap, high_resolution_cap,
    ...
    HalPacket BuildCommandWrapper(const std::array<uint8_t, 3>& chain_0_cap,
                                  ...
                                  bool high_resolution_cap) {
        return BuildCommand(chain_0_cap, chain_1_cap, beamforming_cap, high_resolution_cap);
    }

    HalPacket BuildCommandWrapper(uint8_t br_cap, uint8_t edr_cap, uint8_t ble_cap,
                                  bool high_resolution_cap) {
...
class BluetoothSarTest : public Test {
  protected:
    ...
    void TearDown() override {
        EXPECT_CALL(mock_hci_router_client_agent_, UnregisterClient(bluetooth_sar_handler_.get()))
...
class BleNonConnectionModeTest : public BluetoothSarTest,
                                 public WithParamInterface<std::tuple<bool, uint8_t>> {};

TEST_P(BleNonConnectionModeTest, HandleModeWhenBuildCommandHRModeWrapper) {
    const auto& [is_ble_non_connection_enabled, expected_sub_op_code] = GetParam();
...
INSTANTIATE_TEST_SUITE_P(BleNonConnectionMode, BleNonConnectionModeTest,
...
TEST_F(BluetoothSarTest, HandleBuildCommandHRModeWrapper) {
    ...
    for (size_t i = 0; i < kHciVscSetPowerCapChain1PowerLimitSizePlusHR; i++) {
        EXPECT_EQ(command_hr[6 + kHciVscSetPowerCapChain0PowerLimitSizePlusHR + i], chain1_cap[i]);
    }

    // Verify beamforming cap values (no scaling expected in high resolution
    // mode).
    for (size_t i = 0; i < kHciVscSetPowerCapBeamformingPowerLimitSizePlusHR; i++) {
...
TEST_F(BluetoothSarTest, HandleBuildCommandWithArray) {
    ...
    for (size_t i = 0; i < kHciVscSetPowerCapChain1PowerLimitSize; i++) {
        EXPECT_EQ(command_hr[5 + kHciVscSetPowerCapChain0PowerLimitSize + i], chain1_cap[i]);
    }
...
TEST_F(BluetoothSarTest, HandleBuildCommandWithArrayBoundary) {
    ...
    std::array<uint8_t, 3> chain1_cap = {50, 60, 70};
    std::array<uint8_t, 6> beamforming_cap = {10, 20, 30, 40, 50, 60};
    HalPacket command = bluetooth_sar_handler_->BuildCommandWrapper(chain0_cap, chain1_cap,
                                                                    beamforming_cap, true);
...
TEST_F(BluetoothSarTest, HandleBuildCommand) {
    ...
    // Test with high resolution cap.
    bool high_resolution_cap = true;
    HalPacket command_hr = bluetooth_sar_handler_->BuildCommandWrapper(br_cap, edr_cap, ble_cap,
                                                                       high_resolution_cap);
    ...
    // Test with low resolution cap.
    high_resolution_cap = false;
    HalPacket command_lr = bluetooth_sar_handler_->BuildCommandWrapper(br_cap, edr_cap, ble_cap,
                                                                       high_resolution_cap);
...
TEST_F(BluetoothSarTest, HandleSetBluetoothTxPowerCap) {
    uint8_t cap = 10;
    HalPacket command = bluetooth_sar_handler_->BuildCommandWrapper(cap, cap, cap, false);
...
TEST_F(BluetoothSarTest, HandleSetBluetoothTechBasedTxPowerCap) {
    ...
    uint8_t ble_cap = 30;
    HalPacket command =
            bluetooth_sar_handler_->BuildCommandWrapper(br_cap, edr_cap, ble_cap, false);
    EXPECT_CALL(mock_hci_router_, SendCommand(command, _)).Times(1);

    EXPECT_FALSE(bluetooth_sar_handler_->SetBluetoothTechBasedTxPowerCap(br_cap, edr_cap, ble_cap));

    EnableBluetooth();
    EXPECT_TRUE(bluetooth_sar_handler_->SetBluetoothTechBasedTxPowerCap(br_cap, edr_cap, ble_cap));
}
...
TEST_F(BluetoothSarTest, SetBluetoothModeBasedTxPowerCap) {
    ...
    HalPacket command = bluetooth_sar_handler_->BuildCommandWrapper(chain0_cap, chain1_cap,
                                                                    beamforming_cap, false);
    EXPECT_CALL(mock_hci_router_, SendCommand(command, _)).Times(1);

    EXPECT_FALSE(bluetooth_sar_handler_->SetBluetoothModeBasedTxPowerCap(chain0_cap, chain1_cap,
                                                                         beamforming_cap));

    EnableBluetooth();
    EXPECT_TRUE(bluetooth_sar_handler_->SetBluetoothModeBasedTxPowerCap(chain0_cap, chain1_cap,
                                                                        beamforming_cap));
}
