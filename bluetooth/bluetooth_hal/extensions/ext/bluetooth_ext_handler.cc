...
bool BluetoothExtHandler::SetBluetoothCmdPacket(char16_t opcode, const std::vector<uint8_t>& params,
                                                bool* ret) {
    ...
    if (params_len > 0) {
        ...
        memcpy(hci_cmd.data() + 1 + HciConstants::kHciCommandPreambleSize, params.data(),
               params_len);
    }
    ...
    if (future_status == std::future_status::timeout) {
        LOG(WARNING) << __func__ << ": Wait for VSE 0x" << std::hex << static_cast<uint16_t>(opcode)
                     << " timed out after " << kMaxCommandWaitTimeMs << "ms";
        *ret = false;
...
void BluetoothExtHandler::OnCommandCallback(const ::bluetooth_hal::hci::HalPacket& event) {
    bool success = (event.GetCommandCompleteEventResult() ==
                    static_cast<uint8_t>(EventResultCode::kSuccess));

    LOG(success ? INFO : WARNING) << __func__ << ": Recv VSE "
