...
class BluetoothExtHandlerTest : public Test {
  protected:
    ...
    HalPacket BuildExpectedCommand(uint16_t opcode, const std::vector<uint8_t>& params) {
        ...
        if (params_len > 0) {
            memcpy(expected_cmd.data() + 1 + HciConstants::kHciCommandPreambleSize, params.data(),
                   params_len);
        }
        return expected_cmd;
    }
...
TEST_F(BluetoothExtHandlerTest, SetBluetoothCmdPacketReturnSuccess) {
    ...
    EXPECT_CALL(mock_hci_router_, SendCommand(_, _))
            .WillOnce(DoAll(SaveHalPacketArg(&sent_cmd), SignalPromise(&cmd_sent_promise),
                            Return(true)));
    ...
    ASSERT_EQ(cmd_sent_future.wait_for(std::chrono::seconds(1)), std::future_status::ready)
    ...
    HalPacket success_event = CreateCommandCompleteEvent(test_opcode, EventResultCode::kSuccess);
    router_callback_->OnCommandCallback(success_event);

    // Wait for SetBluetoothCmdPacket to finish and check results.
    EXPECT_TRUE(future_result.get());  // Check the return value of SetBluetoothCmdPacket.
    EXPECT_TRUE(result);               // Check the output parameter 'ret'.
}

TEST_F(BluetoothExtHandlerTest, SetBluetoothCmdPacketReturnSuccessWithNoParams) {
    ASSERT_NE(router_callback_, nullptr);
...
TEST_F(BluetoothExtHandlerTest, SetBluetoothCmdPacketReturnCallbackFailure) {
    ...
    HalPacket failure_event = CreateCommandCompleteEvent(test_opcode, EventResultCode::kFailure);
...
TEST_F(BluetoothExtHandlerTest, SetBluetoothCmdPacketReturnCommandTimeout) {
    ...
    std::future_status status =
            future_result.wait_for(std::chrono::milliseconds(1500));  // Wait > 1000ms.
