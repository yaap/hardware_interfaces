...
constexpr std::chrono::seconds kPendingCallbackTimeout = std::chrono::seconds(3);

}  // namespace

BluetoothCccHandler::BluetoothCccHandler() {
    // Register monitor for CCC LMP events.
    HciEventMonitor ccc_lmp_event_monitor(static_cast<uint8_t>(EventCode::kVendorSpecific),
...
bool BluetoothCccHandler::RegisterForLmpEvents(
    ...
    if (lmp_event_size == 0 || lmp_event_size > static_cast<size_t>(CccLmpEventId::kMax)) {
        LOG(WARNING) << __func__ << ": Incorrect size LMP events:" << lmp_event_size;
        return false;
    }
    ...
    auto command = BluetoothCccTimesyncCommand::CreateAddCommand(address, address_type,
                                                                 CccDirection::kTx, lmp_event_ids);
...
bool BluetoothCccHandler::UnregisterLmpEventsWithType(const BluetoothAddress& address,
                                                      const AddressType address_type) {
    std::unique_lock<std::mutex> lock(mutex_);
    LOG(INFO) << __func__ << ": address: " << address.ToString();

    if (!IsBluetoothEnabled()) {
        LOG(WARNING) << __func__ << ": Bluetooth is OFF, unable to unregister for LMP events";
        return false;
    }
    ...
    auto it = std::remove_if(monitor_callbacks_.begin(), monitor_callbacks_.end(),
    ...
    if (removed_addresses.empty()) {
        LOG(WARNING) << __func__ << ": was not registered for address: " << address.ToString();
        return false;
    }
    ...
    for (const auto& removed_address : removed_addresses) {
        auto command =
                BluetoothCccTimesyncCommand::CreateRemoveCommand(removed_address, address_type);
        if (!SendCommand(command)) {
            LOG(WARNING) << __func__ << ": Failed to send REMOVE command for address: "
...
void BluetoothCccHandler::OnMonitorPacketCallback([[maybe_unused]] MonitorMode mode,
    ...
    for (const auto& callback : monitor_callbacks_) {
        if (callback->IsAddressEqual(address, static_cast<AddressType>(address_type)) &&
            callback->ContainsEventId(lmp_event_id)) {
            callback->OnEventGenerated(timestamp, address, direction, lmp_event_id, event_counter);
        }
    }
}
...
uint64_t BluetoothCccHandler::GetSystemTime(uint8_t current_toggle_count, uint16_t offset) {
    int fd = 0;
    if ((fd = SystemCallWrapper::GetWrapper().Open(kTimesyncProcNode.data(), O_RDONLY)) < 0) {
    ...
    for (uint8_t i = 0; i < toggle_count; i++) {
        ...
        SystemCallWrapper::GetWrapper().Read(fd, buff, TimesyncConstants::kUint64MaxDigitInDec);
    }
