...
BluetoothCccTimesyncEvent::BluetoothCccTimesyncEvent(const HalPacket& packet) : HalPacket(packet) {
    ...
    is_valid_ = (size() == TimesyncConstants::kEventLength && GetType() == HciPacketType::kEvent &&
                 IsVendorEvent() &&
                 At(TimesyncEventOffset::kSubEventCode) == TimesyncConstants::kSubEventCode);
    ...
    std::reverse_copy(
            ...
            begin() + static_cast<uint8_t>(TimesyncEventOffset::kAddress) + address_.size(),
            address_.begin());
...
bool BluetoothCccTimesyncEvent::IsValid() {
    return is_valid_;
}
...
CccDirection BluetoothCccTimesyncEvent::GetDirection() const {
    ...
    return (direction <= CccDirection::kMax) ? direction : CccDirection::kUndefined;
}

uint64_t BluetoothCccTimesyncEvent::GetTimestamp() const {
    return timestamp_;
}
...
CccLmpEventId BluetoothCccTimesyncEvent::GetEventId() const {
    ...
    } else if (event_id_ == static_cast<uint8_t>(CccLmpEventIdByte::kLlPhyUpdateInd)) {
