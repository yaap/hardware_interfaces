...
void CccInitializer() {
    RegisterHalService(SharedRefBase::make<BluetoothCcc>());
}
...
class BluetoothCccDeathRecipient {
  public:
    ...
    void ServiceDied() {
        ...
        is_dead_ = true;
        BluetoothCccHandler::GetHandler().UnregisterLmpEvents(BluetoothAddress(address_));
    }
...
class BluetoothCccHandlerCallbackImpl : public BluetoothCccHandlerCallback {
  public:
    explicit BluetoothCccHandlerCallbackImpl(
            ...
            const BluetoothAddress& address, const std::vector<CccLmpEventId>& lmp_event_ids,
    ...
    void OnEventGenerated(const CccTimestamp& timestamp, const BluetoothAddress& address,
        ...
        bluetooth_ccc_callback_->onEventGenerated(
                ...
                static_cast<Direction>(direction), static_cast<LmpEventId>(lmp_event_id),
                event_counter);
    }
...
ScopedAStatus BluetoothCcc::registerForLmpEvents(
    ...
    const auto death_recipient = std::make_shared<BluetoothCccDeathRecipient>(bluetooth_address);
    bool status = BluetoothCccHandler::GetHandler().RegisterForLmpEvents(
            std::make_unique<BluetoothCccHandlerCallbackImpl>(callback, bluetooth_address,
                                                              lmp_event_ids, death_recipient));
    return status ? ScopedAStatus::ok() : ScopedAStatus::fromExceptionCode(EX_NULL_POINTER);
}

ScopedAStatus BluetoothCcc::unregisterLmpEvents(const std::array<uint8_t, 6>& address) {
    bool status = BluetoothCccHandler::GetHandler().UnregisterLmpEvents(BluetoothAddress(address));
    return status ? ScopedAStatus::ok() : ScopedAStatus::fromExceptionCode(EX_NULL_POINTER);
}
