...
class MockCallbackVerifier : public BluetoothCccHandlerCallback {
  public:
    MockCallbackVerifier()
        : BluetoothCccHandlerCallback(BluetoothAddress({}), std::vector<CccLmpEventId>()) {}
    MOCK_METHOD(void, OnEventGenerated,
                (const CccTimestamp& timestamp, const BluetoothAddress& address,
                 CccDirection direction, CccLmpEventId lmp_event_id, uint16_t event_counter),
                (override));
...
class MockBluetoothCccHandlerCallback : public BluetoothCccHandlerCallback {
  public:
    MockBluetoothCccHandlerCallback(BluetoothAddress& address, std::vector<CccLmpEventId> lmp_ids,
                                    std::shared_ptr<MockCallbackVerifier> verifier,
                                    AddressType address_type = AddressType::kRandom)
        : BluetoothCccHandlerCallback(address, address_type, lmp_ids), verifier_(verifier) {};
    ...
    void OnEventGenerated(const CccTimestamp& timestamp, const BluetoothAddress& address,
        ...
        verifier_->OnEventGenerated(timestamp, address, direction, lmp_event_id, event_counter);
    }
  ...
  protected:
    ...
    void EnableBluetooth() {
        ...
        ON_CALL(mock_system_call_wrapper_, Read(_, _, _))
                .WillByDefault(Invoke([&]([[maybe_unused]] int fd, void* buf, size_t count) {
...
TEST_F(BluetoothCccHandlerTest, HandleRegisterForLmpEventsSuccess) {
    ...
    EXPECT_FALSE(ccc_handler_->RegisterForLmpEvents(
            std::make_unique<MockBluetoothCccHandlerCallback>(address_, lmp_ids, mock_callback)));
    EnableBluetooth();
...
TEST_F(BluetoothCccHandlerTest, HandleRegisterForLmpEventsTooManyLmpEvent) {
    std::vector<CccLmpEventId> lmp_ids = {CccLmpEventId::kConnectInd,
                                          CccLmpEventId::kLlPhyUpdateInd, CccLmpEventId::kMax};
    auto mock_callback = std::make_shared<MockCallbackVerifier>();
    EnableBluetooth();
    EXPECT_FALSE(ccc_handler_->RegisterForLmpEvents(
            std::make_unique<MockBluetoothCccHandlerCallback>(address_, lmp_ids, mock_callback)));
...
TEST_F(BluetoothCccHandlerTest, HandleUnregisterLmpEventsBeforeCommandComplete) {
    std::vector<CccLmpEventId> lmp_ids = {CccLmpEventId::kConnectInd,
                                          CccLmpEventId::kLlPhyUpdateInd};
    auto mock_callback = std::make_shared<MockCallbackVerifier>();
...
TEST_F(BluetoothCccHandlerTest, HandleMonitoringTimeSyncEventWithDifferentAddress) {
    std::vector<CccLmpEventId> lmp_ids = {CccLmpEventId::kConnectInd,
                                          CccLmpEventId::kLlPhyUpdateInd};
    auto mock_callback = std::make_shared<MockCallbackVerifier>();
...
TEST_F(BluetoothCccHandlerTest, HandleMonitoringTimeSyncEventWithDifferentAddressType) {
    std::vector<CccLmpEventId> lmp_ids = {CccLmpEventId::kConnectInd,
                                          CccLmpEventId::kLlPhyUpdateInd};
    auto mock_callback = std::make_shared<MockCallbackVerifier>();
    auto timesync_event = CreateTimesyncEvent(address_, 1, 0x1);
    auto address_type_public = CreateTimesyncEvent(address_, 2, 0x2, AddressType::kPublic);
    ...
    EXPECT_CALL(mock_hci_router_, SendCommand(_, _)).Times(1);
    EXPECT_CALL(*mock_callback, OnRegistered(true)).Times(1);
    EXPECT_CALL(*mock_callback, OnEventGenerated(_, address_, _, _, 0x2)).Times(1);

    EnableBluetooth();
    // Register LMP event for address_ kPublic
    EXPECT_TRUE(
            ccc_handler_->RegisterForLmpEvents(std::make_unique<MockBluetoothCccHandlerCallback>(
...
TEST_F(BluetoothCccHandlerTest, HandleMonitoringMultipleTimeSyncEvent) {
    ...
    EXPECT_CALL(*mock_callback2, OnEventGenerated(_, different_address_, _, _, _)).Times(1);

    EnableBluetooth();
    // Register LMP event for address_
    EXPECT_TRUE(ccc_handler_->RegisterForLmpEvents(
            std::make_unique<MockBluetoothCccHandlerCallback>(address_, lmp_ids, mock_callback1)));
    ccc_handler_->OnCommandCallback(GenerateTimesyncCommandCompleteEvent(true));
