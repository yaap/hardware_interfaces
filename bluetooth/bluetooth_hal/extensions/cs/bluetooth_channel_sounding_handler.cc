...
std::shared_ptr<BluetoothChannelSoundingSessionInterface> CreateSession(
        [[maybe_unused]] std::shared_ptr<IBluetoothChannelSoundingSessionCallback> callback,
        [[maybe_unused]] Reason reason) {
#ifdef USE_RANGING_V1
    return SharedRefBase::make<BluetoothChannelSoundingSessionV1>(callback, reason);
#elif USE_RANGING_V2
    return SharedRefBase::make<BluetoothChannelSoundingSessionV2>(callback, reason);
#else
    return nullptr;
#endif
}

void SendFakeRasNotification(const BluetoothChannelSoundingParameters& parameters,
...
RangingSettingCommand HandleOldData(const std::vector<uint8_t>& raw_data) {
    ...
    data.mode_0_channel_map = raw_data[kOldIdxRangingSettingCommandMode0ChannelMap];
...
void BluetoothChannelSoundingHandler::SetCsVendorSpecificDataMask(uint32_t mask) {
    cs_vendor_specific_data_mask_ = mask;
}

bool BluetoothChannelSoundingHandler::GetVendorSpecificData(
        std::optional<std::vector<std::optional<VendorSpecificData>>>* return_value) {
    ...
    const std::vector<HalPacket>& calibration_commands = cs_loader.GetCsCalibrationCommands();
    ...
    if (AndroidBaseWrapper::GetWrapper().GetBoolProperty(kIsCalibrationEnabled.data(), true)) {
        for (const auto& command : calibration_commands) {
            SendCommand(command);
        }
    }

    *return_value = std::make_optional<std::vector<std::optional<VendorSpecificData>>>();

    if (local_capabilities_.size() < kCommandCompleteReadLocalCapabilityValueLength) {
        LOG(INFO) << __func__ << ": Didn't get the current value. Return default value.";
        return true;
    }
...
bool BluetoothChannelSoundingHandler::GetSupportedCsSecurityLevels(
    ...
    std::vector<CsSecurityLevel> supported_security_levels = {CsSecurityLevel::ONE};
...
bool BluetoothChannelSoundingHandler::OpenSession(
        ...
        [[maybe_unused]] std::shared_ptr<IBluetoothChannelSoundingSession>* return_value) {
    ...
    if (IsUuidMatched(in_params.vendorSpecificData) &&
        in_params.vendorSpecificData.value()[0].value().opaqueValue[0] == kDataTypeReply) {
        HandleVendorSpecificReply(in_params.aclHandle, in_params.vendorSpecificData, in_callback);
        return true;
    }
    ...
    for (auto& data : vendor_specific_data.value()) {
        ...
        if (command_data.event_mask != kCommandValueIgnore) {
            ...
            if (is_old_format) {
                switch (command_data.event_mask) {
                    case kCommandValueEnable:
                        LOG(INFO) << __func__ << ": Send EnableEventMaskForConnectionCommand.";
                        command = BuildSetEventMaskForConnectionCommand(
                                connection_handle,
                                static_cast<uint32_t>(0b0111));  // Enable all 3 events.
                        break;
                    case kCommandValueDisable:
                        LOG(INFO) << __func__ << ": Send DisableEventMaskForConnectionCommand.";
                        command = BuildSetEventMaskForConnectionCommand(
                                connection_handle,
                    ...
                    default:
                        LOG(ERROR) << __func__
                                   << ": Invalid command value: " << command_data.event_mask;
                        callback->onOpenFailed(Reason::LOCAL_STACK_REQUEST);
                        return;
                }
            } else {
                command = BuildSetEventMaskForConnectionCommand(connection_handle,
                                                                command_data.event_mask);
            }

            if (is_inline_pct_enabled) {
                bool enable_subevent_result_event = IsBitSet(command_data.event_mask, 0);
                bool enable_subevent_result_continue_event = IsBitSet(command_data.event_mask, 1);
                bool enable_procedure_enable_complete_event = IsBitSet(command_data.event_mask, 2);
                LOG(INFO) << __func__
                          << ": Send SetEventMaskForConnectionCommand: Subevent Result event("
                          << enable_subevent_result_event << "), Subevent Result Continue event("
        ...
        if (command_data.mode_0_channel_map != kCommandValueIgnore) {
            ...
            switch (command_data.mode_0_channel_map) {
                ...
                default:
                    LOG(ERROR) << __func__
                               << ": Invalid command value: " << command_data.mode_0_channel_map;
                    callback->onOpenFailed(Reason::LOCAL_STACK_REQUEST);
                    return;
            }
            SendCommand(command);
...
void BluetoothChannelSoundingHandler::OnCommandCallback(const HalPacket& packet) {
    ...
    LOG(status ? INFO : WARNING) << __func__ << ": Recv VSE <" << packet.ToString() << "> "
                                 << (status ? "[Success]" : "[Failed]");

    if (!status ||
        packet.GetCommandOpcodeFromGeneratedEvent() != kHciVscSpecialRangingSettingOpcode ||
        HciConstants::kHciCommandCompleteResultOffset + 1 >= packet.size()) {
        return;
    }

    uint8_t sub_opcode = packet[HciConstants::kHciCommandCompleteResultOffset + 1];
    ...
    if (sub_opcode == kHciVscReadLocalCapabilitySubOpCode) {
        ...
        for (int i = 0; i < kCommandCompleteReadLocalCapabilityValueLength; i++) {
            local_capabilities_.push_back(packet[kCommandCompleteReadLocalCapabilityOffset + i]);
        }
    }
};

void BluetoothChannelSoundingHandler::HandleCsSubevent(const HalPacket& packet) {
    ...
    uint16_t connection_handle = packet[offset] + ((packet[offset + 1] << 8u) & 0xff00);
    ...
    offset += 5;
    uint16_t procedure_counter = packet[offset] + ((packet[offset + 1] << 8u) & 0xff00);

    if (tracker->get().cur_procedure_counter == procedure_counter) {
        LOG(DEBUG) << __func__ << ": Skip duplicate fake notification, procedure_counter: "
                   << procedure_counter;
        return;
    }

    LOG(DEBUG) << __func__ << ": Send fake notification, connection_handle:" << connection_handle
               << ", procedure_counter:" << procedure_counter;

    tracker->get().cur_procedure_counter = procedure_counter;
    SendFakeRasNotification(tracker->get().parameters, tracker->get().cur_procedure_counter);
}
...
void BluetoothChannelSoundingHandler::OnMonitorPacketCallback([[maybe_unused]] MonitorMode mode,
                                                              const HalPacket& packet) {
