...
bool IsUuidMatched(
    ...
    if (!uuid0.has_value() || uuid0->characteristicUuid != kUuidSpecialRangingSettingCapability) {
        LOG(WARNING) << __func__ << ": uuid0 doesn't match kUuidSpecialRangingSettingCapability.";
        return false;
    }

    if (uuid0->opaqueValue.size() < 5) {
        LOG(WARNING) << __func__ << ": Invalid data for kUuidSpecialRangingSettingCapability.";
        return false;
    }

    const auto uuid1 = data[1];
    if (!uuid1.has_value() || uuid1->characteristicUuid != kUuidSpecialRangingSettingCommand) {
        LOG(WARNING) << __func__ << ": uuid0 doesn't match kUuidSpecialRangingSettingCommand.";
        return false;
    }
...
HalPacket BuildEnableMode0ChannelMapCommand(uint16_t connection_handle, uint8_t enable) {
    HalPacket command;
...
HalPacket BuildSetEventMaskForConnectionCommand(uint16_t connection_handle, uint32_t event_mask) {
    /* Bit masks format:
...
HalPacket BuildRasNotification(const BluetoothChannelSoundingParameters& parameters,
    ...
    packet[14] =
            ((procedure_counter >> 8u) & 0x0f) + 0x10;  // ranging_counter and configuration_id.
