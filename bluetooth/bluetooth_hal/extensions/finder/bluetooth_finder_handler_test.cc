...
class TestBluetoothFinderHandler : public BluetoothFinderHandler {
  public:
    ...
    HalPacket BuildFinderResetCommandWrapper() { return BuildFinderResetCommand(); }

    HalPacket BuildPrecomputedKeyCommandWrapper(const std::vector<Eid>& keys, uint_t cur_key_idx) {
...
class BluetoothFinderHandlerTest : public Test {
  protected:
    ...
    HalPacket CreateCommandCompleteEvent(uint16_t opcode, EventResultCode status) {
...
TEST_F(StartFinderProcessTest, StartPofButNotShuttingDown) {
    SimulateBluetoothChipReady();
    EXPECT_CALL(mock_android_base_wrapper_, GetProperty(_, _)).Times(1).WillOnce(Return(""));
    EXPECT_TRUE(handler_->SendEids(test_keys_));
    EXPECT_FALSE(handler_->StartPoweredOffFinderMode());
    EXPECT_EQ(handler_->GetState(), BluetoothFinderHandler::State::kIdle);
}
...
TEST_F(StartFinderProcessTest, StartPofWithSingleKeyBatchReturnSuccess) {
    ...
    HalPacket expected_set_keys_cmd = handler_->BuildPrecomputedKeyCommandWrapper(test_keys_, 0);
    ...
    ASSERT_EQ(pof_sent_future.wait_for(std::chrono::seconds(1)), std::future_status::ready)
...
TEST_F(StartFinderProcessTest, StartPofWithMultipleKeyBatchesReturnSuccess) {
    ...
    HalPacket expected_set_keys1_cmd = handler_->BuildPrecomputedKeyCommandWrapper(test_keys_, 0);
    ...
    handler_->SetCurrentKeyIndex(num_keys_first_batch);  // Set index for expectation
    HalPacket expected_set_keys2_cmd =
            handler_->BuildPrecomputedKeyCommandWrapper(test_keys_, num_keys_first_batch);
    handler_->SetCurrentKeyIndex(0);  // Reset index before starting the actual flow
    ...
    ASSERT_EQ(keys1_sent_future.wait_for(std::chrono::seconds(1)), std::future_status::ready);
    ...
    ASSERT_EQ(keys2_sent_future.wait_for(std::chrono::seconds(1)), std::future_status::ready);
    router_callback_->OnCommandCallback(
            CreateCommandCompleteEvent(kHciVscPofOpcode, EventResultCode::kSuccess));
...
TEST_F(StartFinderProcessTest, StartPofFailOnReset) {
    ...
    HalPacket reset_failure_event = CreateCommandCompleteEvent(
            static_cast<uint16_t>(CommandOpCode::kHciReset), EventResultCode::kFailure);
