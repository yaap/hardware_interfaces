...
bool BluetoothFinderHandler::IsEnabled() {
...
bool BluetoothFinderHandler::SendEids(const std::vector<Eid>& keys) {
    ...
    if (state_ != State::kIdle) {
        LOG(WARNING) << __func__ << ": Could not send keys while entering powered off finder mode.";
        return false;
    }
...
bool BluetoothFinderHandler::SetPoweredOffFinderMode(bool enable) {
    ...
    if (state_ != State::kIdle) {
        LOG(WARNING) << __func__ << ": Could not set mode while entering powered off finder mode.";
        return false;
    }
...
bool BluetoothFinderHandler::StartPoweredOffFinderMode() {
    ...
    std::string shutdown_action =
            AndroidBaseWrapper::GetWrapper().GetProperty(Property::kShutDownAction, "");
    ...
    if (!IsBluetoothChipReady()) {
        LOG(WARNING) << __func__ << ": Unable to start powered off mode: bluetooth is off.";
        return false;
    } else if (!keys_.size()) {
...
void BluetoothFinderHandler::OnMonitorPacketCallback([[maybe_unused]] MonitorMode mode,
...
void BluetoothFinderHandler::OnCommandCallback(const HalPacket& event) {
    ...
    LOG(command_success_ ? INFO : WARNING)
            << __func__ << ": Recv VSE " << (command_success_ ? "[Success]" : "[Failed]");

    // Handle state transition based on the current state and command result.
    if (!command_success_) {
        LOG(ERROR) << __func__ << ": Command failed in state " << static_cast<int>(state_.load());
...
void BluetoothFinderHandler::HandleNextStep(State next_state) {
    ...
    switch (state_) {
        ...
        case State::kSendingKeys: {
            ...
            LOG(INFO) << __func__ << ": Sending keys starting from index " << current_key_index_
                      << ".";
...
HalPacket BluetoothFinderHandler::BuildPrecomputedKeyCommand(const std::vector<Eid>& keys,
                                                             uint_t cur_key_idx) {
    HalPacket command;
    uint8_t num_remaining_keys = keys.size() - cur_key_idx;
    uint8_t num_keys_to_send =
            (num_remaining_keys <= kMaxKeyNumPerVsc ? num_remaining_keys : kMaxKeyNumPerVsc);
    ...
    for (size_t i = 0; i < num_keys_to_send; ++i) {
        ...
        std::copy_n(key.begin(), kBytesPerKey, command.begin() + 7 + i * kBytesPerKey);
    }
...
HalPacket BluetoothFinderHandler::BuildStartPoweredOffFinderModeCommand(int32_t cur_key_idx) {
    HalPacket command;
...
bool BluetoothFinderHandler::SendCommandAndWait(const HalPacket& packet) {
    ...
    std::future_status status =
            command_promise_.get_future().wait_for(std::chrono::milliseconds(kMaxCommandWaitTime));
