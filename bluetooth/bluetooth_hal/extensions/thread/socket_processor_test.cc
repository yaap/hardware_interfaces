...
constexpr uint8_t kPayloadSizeBuffer[2] = {static_cast<uint8_t>(kDataSize & 0xFF),
...
class SocketProcessorTestBase : public Test {
  protected:
    ...
    void TestInit() {
        ...
        ASSERT_EQ(SocketProcessor::GetProcessor()->GetServerSocket(), kInvalidFileDescriptor);
        ASSERT_EQ(SocketProcessor::GetProcessor()->GetClientSocket(), kInvalidFileDescriptor);
    }
...
class SocketProcessorTestSeqPacket : public SocketProcessorTestBase {
  protected:
    void SetUp() override {
        ...
        SocketProcessor::GetProcessor()->SetSocketMode(SocketMode::kSockModeSeqPacket);
    }
};
...
TEST_F(SocketProcessorTestBase, OpenServerReturnSuccess) {
    ...
    ASSERT_NE(SocketProcessor::GetProcessor()->GetServerSocket(), kInvalidFileDescriptor);
}
...
TEST_F(SocketProcessorTestBase, AcceptClientReturnSuccess) {
    ...
    SocketProcessor::GetProcessor()->SetServerSocket(1);
    ASSERT_NE(SocketProcessor::GetProcessor()->AcceptClient(), kInvalidFileDescriptor);
}
...
TEST_F(SocketProcessorTestBase, AcceptClientWithInvalidServerSocketReturnFail) {
    ...
    ASSERT_EQ(SocketProcessor::GetProcessor()->AcceptClient(), kInvalidFileDescriptor);
}
...
TEST_F(SocketProcessorTestBase, OpenSocketFileMonitorReturnNotifyFail) {
    EXPECT_CALL(mock_system_call_wrapper_, InotifyInit()).Times(1).WillOnce(Return(-1));
    EXPECT_CALL(mock_system_call_wrapper_, InotifyAddWatch(_, _, _)).Times(0);

    ASSERT_EQ(SocketProcessor::GetProcessor()->OpenSocketFileMonitor(), kInvalidFileDescriptor);
    ASSERT_EQ(SocketProcessor::GetProcessor()->GetSocketFileMonitor(), kInvalidFileDescriptor);
}
...
TEST_F(SocketProcessorTestBase, OpenSocketFileMonitorReturnAddWatchFail) {
    ...
    EXPECT_CALL(
            ...
            InotifyAddWatch(_, MatcherFactory::CreateStringMatcher(kThreadDispatcherFolderPath), _))
            .Times(1)
            .WillOnce(Return(-1));
...
TEST_F(SocketProcessorTestBase, OpenSocketFileMonitorReturnSuccess) {
    ...
    ASSERT_NE(SocketProcessor::GetProcessor()->OpenSocketFileMonitor(), kInvalidFileDescriptor);
    ASSERT_NE(SocketProcessor::GetProcessor()->GetSocketFileMonitor(), kInvalidFileDescriptor);
}
...
TEST_F(SocketProcessorTestBase, CheckIfSocketFileExistedReturnFileExisted) {
    EXPECT_CALL(mock_system_call_wrapper_,
            ...
            .Times(1)
            .WillOnce(Return(0));
    EXPECT_CALL(mock_system_call_wrapper_, IsSocketFile(_)).Times(1).WillOnce(Return(true));
...
TEST_F(SocketProcessorTestBase, CheckIfSocketFileExistedReturnFileExistedButNotSocket) {
    EXPECT_CALL(mock_system_call_wrapper_,
                Stat(MatcherFactory::CreateStringMatcher(test_socket_path_), _))
            .Times(1)
            .WillOnce(Return(0));
    EXPECT_CALL(mock_system_call_wrapper_, IsSocketFile(_)).Times(1).WillOnce(Return(false));
...
TEST_F(SocketProcessorTestStream, SendPayloadAtOneTimeSuccess) {
    ...
    ON_CALL(mock_system_call_wrapper_,
            Send(_, MatcherFactory::CreateByteContentMatcher(kPayloadSizeBuffer, 2), _, _))
            .WillByDefault(Return(2));
    ON_CALL(mock_system_call_wrapper_,
            Send(_, MatcherFactory::CreateByteContentMatcher(send_data.data(), 5), _, _))
...
TEST_F(SocketProcessorTestStream, SendPayloadInMultipleTimesSuccess) {
    ...
    ON_CALL(mock_system_call_wrapper_,
            ...
            .WillByDefault(Return(2));
    ON_CALL(mock_system_call_wrapper_,
            Send(_, MatcherFactory::CreateByteContentMatcher(send_data.data() + 2, 3), _, _))
            .WillByDefault(Return(2));
    ON_CALL(mock_system_call_wrapper_,
            Send(_, MatcherFactory::CreateByteContentMatcher(send_data.data() + 4, 1), _, _))
            .WillByDefault(Return(1));
...
TEST_F(SocketProcessorTestStream, RecvPayloadAtOneTimeSuccess) {
    ...
    EXPECT_CALL(mock_system_call_wrapper_, Recv(_, _, 1, _))
            .WillOnce(DoAll(Invoke([]([[maybe_unused]] int fd, void* buffer,
                                      [[maybe_unused]] size_t length, [[maybe_unused]] int flags) {
