  ...
  protected:
    void SetUp() override {
        ...
        ON_CALL(mock_system_call_wrapper_, Select(_, _, _, _, _))
                .WillByDefault(Invoke([]([[maybe_unused]] int nfds,
                    ...
                    std::this_thread::sleep_for(std::chrono::milliseconds(kSelectMockSleepTimeMs));
                    return 1;
                }));

        ON_CALL(mock_socket_processor_, GetServerSocket()).WillByDefault(Return(kServerFd));

        ON_CALL(mock_socket_processor_, OpenSocketFileMonitor())
                .WillByDefault(Return(kSocketMonitorFd));

        ON_CALL(mock_socket_processor_, GetClientSocket()).WillByDefault(Return(kClientFd));
    ...
    void SetUpDaemonStartExpectations() {
        EXPECT_CALL(mock_system_call_wrapper_, CreatePipe(_, _))
                .Times(AtLeast(1))
                .WillRepeatedly(DoAll(Invoke([](int pipefd[2], [[maybe_unused]] int flags) -> void {
    ...
    void SetUpDaemonStopExpectations() {
        if (thread_daemon_->IsDaemonRunning()) {
            ...
            EXPECT_CALL(mock_system_call_wrapper_, Write(_, _, 1)).Times(1).WillOnce(Return(1));
    ...
    void SetUpClientConnectExpectations() {
        ...
        EXPECT_CALL(mock_system_call_wrapper_, FdIsSet(kSocketMonitorFd, _))
                .WillRepeatedly(Return(0));
        EXPECT_CALL(mock_system_call_wrapper_, FdIsSet(kClientFd, _)).WillRepeatedly(Return(0));
    }
    ...
    void SetupClientSignalReceptionExpectations() {
        ...
        EXPECT_CALL(mock_system_call_wrapper_, FdIsSet(kNotificationFd, _))
                .WillRepeatedly(Return(0));
        EXPECT_CALL(mock_system_call_wrapper_, FdIsSet(kServerFd, _)).WillRepeatedly(Return(0));
        EXPECT_CALL(mock_system_call_wrapper_, FdIsSet(kSocketMonitorFd, _))
                .WillRepeatedly(Return(0));
...
TEST_F(ThreadDaemonTest, StartDaemonReturnSuccess) {
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(kDaemonThreadInitWaitTimeMs));
}

TEST_F(ThreadDaemonTest, StartDaemonReturnFail) {
    EXPECT_CALL(mock_system_call_wrapper_, CreatePipe(_, _)).Times(1).WillOnce(Return(1));
...
TEST_F(ThreadDaemonTest, AcceptClientReturnSuccess) {
    ...
    EXPECT_CALL(mock_socket_processor_, AcceptClient()).Times(1).WillOnce(Return(kClientFd));
...
TEST_F(ThreadDaemonTest, AcceptClientTwiceReturnFail) {
    ...
    EXPECT_CALL(mock_system_call_wrapper_, FdIsSet(kNotificationFd, _)).WillRepeatedly(Return(0));
    EXPECT_CALL(mock_system_call_wrapper_, FdIsSet(kServerFd, _))
            .WillOnce(Return(1))
            .WillOnce(Return(1))
            .WillRepeatedly(Return(0));
    EXPECT_CALL(mock_system_call_wrapper_, FdIsSet(kSocketMonitorFd, _)).WillRepeatedly(Return(0));
    EXPECT_CALL(mock_system_call_wrapper_, FdIsSet(kClientFd, _)).WillRepeatedly(Return(0));
...
TEST_F(ThreadDaemonTest, SendUplinkWithInvalidHeaderSizeReturnFail) {
    ...
    EXPECT_CALL(mock_socket_processor_, Send(MatcherFactory::CreateVectorMatcher(empty_packet)))
            .Times(1);
...
TEST_F(ThreadDaemonTest, SendUplinkWithInvalidPacketSizeReturnFail) {
    ...
    std::vector<uint8_t> invalid_packet_size_hal_packet = {0x70, 0x00, 0x00, 0x05, 0x00,
                                                           0x00, 0x01, 0x02, 0x03};
...
TEST_F(ThreadDaemonTest, SendDownlinkReturnSuccess) {
    ...
    HalPacket valid_hal_packet({0x70, 0x00, 0x00, 0x05, 0x00, 0x00, 0x01, 0x02, 0x03, 0x04});
    EXPECT_CALL(mock_packet_handler_,
                HalPacketCallback(MatcherFactory::CreateHalPacketMatcher(valid_hal_packet)))
            .Times(1);
...
TEST_F(ThreadDaemonTest, ReceiveDeleteFileSignalButNotSocketFile) {
    ...
    EXPECT_CALL(mock_socket_processor_, IsSocketFileExisted()).Times(1).WillOnce(Return(true));
    EXPECT_CALL(mock_socket_processor_, CloseSocketFileMonitor()).Times(0);
    EXPECT_CALL(mock_socket_processor_, CloseClient()).Times(0);
    EXPECT_CALL(mock_socket_processor_, CloseServer()).Times(0);
...
TEST_F(ThreadDaemonTest, ReceiveDeleteSocketFileSignalReturnDaemonRestart) {
    ...
    EXPECT_CALL(mock_system_call_wrapper_, Read(kSocketMonitorFd, _, _))
            .WillOnce(DoAll(Invoke([]([[maybe_unused]] int fd, void* buffer,
                            ...
                            Return(sizeof(struct inotify_event))));
    EXPECT_CALL(mock_socket_processor_, IsSocketFileExisted()).Times(1).WillOnce(Return(false));
