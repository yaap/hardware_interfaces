...
bool ThreadDaemon::IsDaemonRunning() const {
...
bool ThreadDaemon::Stop() {
    ...
    if (!std::atomic_exchange(&is_daemon_running_, false)) {
        LOG(WARNING) << __func__ << ": Daemon is already stopped. No need to close.";
        return false;
    }
...
void ThreadDaemon::ConfigureSocketProcessor() {
    SocketProcessor::Initialize(kThreadDispatcherSocketPath,
    ...
    auto socket_mode = static_cast<SocketMode>(::android::base::GetIntProperty(
            Property::kThreadDispatcherSocketMode, static_cast<int>(SocketMode::kSockModeSeqPacket),
...
void ThreadDaemon::MonitorSocket() {
    LOG(DEBUG) << __func__ << ": Server socket: " << socket_processor_->GetServerSocket();
    ...
    while (is_daemon_running_) {
        ...
        int ret_val = SystemCallWrapper::GetWrapper().Select(FD_SETSIZE, &monitor_fds, nullptr,
                                                             nullptr, nullptr);
        if (ret_val <= 0) {
            continue;  // No activity or an error occurred, continue monitoring.
        }

        if (SystemCallWrapper::GetWrapper().FdIsSet(notification_listen_fd_, &monitor_fds)) {
        ...
        if (SystemCallWrapper::GetWrapper().FdIsSet(socket_processor_->GetSocketFileMonitor(),
                                                    &monitor_fds)) {
            ...
            if (bytes_read > 0) {
                ...
                if (event->mask & IN_DELETE && !socket_processor_->IsSocketFileExisted()) {
        ...
        if (is_client_connected_ && SystemCallWrapper::GetWrapper().FdIsSet(
        ...
        if (SystemCallWrapper::GetWrapper().FdIsSet(socket_processor_->GetServerSocket(),
                                                    &monitor_fds)) {
...
void ThreadDaemon::DaemonRoutine() {
    while (require_starting_) {
        ...
        if (socket_processor_->OpenServer()) {
            if (socket_processor_->OpenSocketFileMonitor() == kInvalidFileDescriptor) {
...
void ThreadDaemon::CleanUpServer() {
    socket_processor_->CloseServer();
}
...
bool ThreadDaemon::CheckIfHardwareReset(const std::vector<uint8_t>& packet) {
    return packet.size() == kHardwareResetCommandSize && packet[0] == kSpinelHeader &&
           packet[1] == kThreadCommandReset && packet[2] == kThreadCommandResetHardware;
}
...
HalPacket ThreadDaemon::ConstructToHalPacket(const std::vector<uint8_t>& packet) {
...
std::vector<uint8_t> ThreadDaemon::ExtractFromHalPacket(const HalPacket& packet) {
    ...
    uint16_t packet_size =
            static_cast<uint16_t>(packet[3]) | (static_cast<uint16_t>(packet[4]) << 8);

    if (packet.size() != 1 + HciConstants::kHciThreadPreambleSize + packet_size) {
        LOG(WARNING) << __func__ << ": Data size does not match with the actual data.";
        return raw_packet;
    }

    // Extract the raw packet data.
    raw_packet.insert(raw_packet.end(), packet.begin() + 1 + HciConstants::kHciThreadPreambleSize,
                      packet.end());
