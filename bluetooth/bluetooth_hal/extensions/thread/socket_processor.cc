...
int SocketProcessorImpl::GetServerSocket() const {
    return server_socket_;
}

int SocketProcessorImpl::GetClientSocket() const {
    return client_socket_;
}

bool SocketProcessorImpl::IsSocketFileExisted() const {
    struct stat st;
    return (SystemCallWrapper::GetWrapper().Stat(socket_path_.c_str(), &st) == 0) &&
...
bool SocketProcessorImpl::SendPacket(const std::vector<uint8_t>& data) {
    ssize_t bytes_sent = TEMP_FAILURE_RETRY(
            SystemCallWrapper::GetWrapper().Send(client_socket_, data.data(), data.size(), 0));
    if (bytes_sent <= 0) {
...
bool SocketProcessorImpl::SendStream(const std::vector<uint8_t>& data) {
    ...
    bytes_sent = TEMP_FAILURE_RETRY(SystemCallWrapper::GetWrapper().Send(client_socket_, len_buffer,
                                                                         sizeof(len_buffer), 0));
    if (bytes_sent <= 0) {
        PrintSocketErr(bytes_sent, SocketDirection::kSend);
        return false;
    ...
    while (total_bytes_sent < data.size()) {
        ssize_t bytes_sent = TEMP_FAILURE_RETRY(SystemCallWrapper::GetWrapper().Send(
                client_socket_, data.data() + total_bytes_sent, data.size() - total_bytes_sent, 0));
        if (bytes_sent <= 0) {
            PrintSocketErr(bytes_sent, SocketDirection::kSend);
            return false;
        }
    ...
    switch (read_state_) {
        ...
        case ReadState::kDataFlag: {
            uint16_t payload_size =
                    static_cast<uint16_t>(data_[0]) | (static_cast<uint16_t>(data_[1]) << 8);
            if (!payload_size) {
...
bool SocketProcessorImpl::RecvStreamWithFixLength(uint16_t length) {
    ...
    while (total_bytes_should_read > 0) {
        ssize_t bytes_read = TEMP_FAILURE_RETRY(SystemCallWrapper::GetWrapper().Recv(
                client_socket_, data_.data() + cur_idx, total_bytes_should_read, 0));
        if (bytes_read <= 0) {
            PrintSocketErr(bytes_read, SocketDirection::kRecv);
            return false;
        }
...
bool SocketProcessorImpl::CreateSocket() {
    int new_server_socket =
            SystemCallWrapper::GetWrapper().Socket(AF_UNIX, static_cast<int>(socket_mode_), 0);
...
bool SocketProcessorImpl::BindSocket() {
    ...
    if (SystemCallWrapper::GetWrapper().Bind(server_socket_,
...
bool SocketProcessorImpl::ListenForClients() {
    if (SystemCallWrapper::GetWrapper().Listen(server_socket_, kMaxWaitingConnectReq) == -1) {
...
void SocketProcessorImpl::PrintSocketErr(int ret_val, SocketDirection dir) {
    if (ret_val == -1) {
        LOG(WARNING) << __func__ << ": Unable to receive Thread frames from the Thread HAL, "
                     << "client socket: " << client_socket_
                     << ", direction: " << static_cast<int>(dir);
    } else if (ret_val == 0) {
        LOG(WARNING) << __func__ << ": Client connection is closed or send buffer is full, "
                     << "direction: " << static_cast<int>(dir);
    }
}
...
void SocketProcessor::Initialize(const std::string& socket_path,
    ...
    CHECK(hal_packet_cb != std::nullopt) << __func__ << ": hal_packet_cb == nullptr";
