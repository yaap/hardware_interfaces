...
class MockHciRouterClient : public HciRouterClientCallback {
  public:
    ...
    MOCK_METHOD(void, OnHalStateChanged, (HalState new_state, HalState old_state), (override));
...
class HciRouterClientAgentTest : public Test {
  protected:
    ...
    void ShutdownBluetooth() { agent_->NotifyHalStateChange(HalState::kInit, HalState::kShutdown); }

    void PowerOnBluetooth() {
        agent_->NotifyHalStateChange(HalState::kBtChipReady, HalState::kFirmwareReady);
    }
    ...
    void EnableBluetooth() {
        ...
        ASSERT_EQ(agent_->DispatchPacketToClients(reset_packet), MonitorMode::kNone);
    }
...
TEST_F(HciRouterClientAgentTest, HandleDispatchPacketToClients) {
    ...
    ON_CALL(mock_router_client, OnPacketCallback(packet)).WillByDefault(Return(expected_mode));
...
TEST_F(HciRouterClientAgentTest, HandleDispatchPacketToClientsWithMultipleClients) {
    HalPacket packet({0x01, 0x02, 0x03, 0x04});
...
TEST_F(HciRouterClientAgentTest, HandleNotifyHalStateChangeShutdownToInit) {
    ...
    EXPECT_CALL(mock_router_client, OnHalStateChanged(HalState::kInit, HalState::kShutdown))
            .Times(1);
...
TEST_F(HciRouterClientAgentTest, HandleNotifyHalStateChangeInitToFirmwareDownloading) {
    MockHciRouterClient mock_router_client;
    EXPECT_CALL(mock_router_client, OnBluetoothChipReady()).Times(0);
    EXPECT_CALL(mock_router_client, OnBluetoothChipClosed()).Times(0);
    EXPECT_CALL(mock_router_client, OnBluetoothEnabled()).Times(0);
...
TEST_F(HciRouterClientAgentTest,
    ...
    EXPECT_CALL(mock_router_client, OnBluetoothEnabled()).Times(0);
    EXPECT_CALL(mock_router_client, OnBluetoothDisabled()).Times(0);
    EXPECT_CALL(mock_router_client, OnHalStateChanged(HalState::kFirmwareDownloadCompleted,
...
TEST_F(HciRouterClientAgentTest,
    ...
    EXPECT_CALL(mock_router_client, OnBluetoothDisabled()).Times(0);
    EXPECT_CALL(mock_router_client,
                OnHalStateChanged(HalState::kFirmwareReady, HalState::kFirmwareDownloadCompleted))
            .Times(1);
    ...
    ASSERT_FALSE(agent_->IsBluetoothChipReady());
    ASSERT_FALSE(agent_->IsBluetoothEnabled());
    agent_->NotifyHalStateChange(HalState::kFirmwareReady, HalState::kFirmwareDownloadCompleted);
    ASSERT_FALSE(agent_->IsBluetoothChipReady());
    ASSERT_FALSE(agent_->IsBluetoothEnabled());
...
TEST_F(HciRouterClientAgentTest, HandleNotifyHalStateChangeFirmwareReadyToBtChipReady) {
    MockHciRouterClient mock_router_client;
...
TEST_F(HciRouterClientAgentTest, HandleNotifyHalStateChangeBtChipReadyToRunning) {
    MockHciRouterClient mock_router_client;
    EXPECT_CALL(mock_router_client, OnBluetoothChipReady()).Times(1);
    EXPECT_CALL(mock_router_client, OnBluetoothChipClosed()).Times(0);
    EXPECT_CALL(mock_router_client, OnBluetoothEnabled()).Times(0);
    EXPECT_CALL(mock_router_client, OnBluetoothDisabled()).Times(0);
    EXPECT_CALL(mock_router_client, OnHalStateChanged(HalState::kRunning, HalState::kBtChipReady))
            .Times(1);
...
TEST_F(HciRouterClientAgentTest, HandleNotifyHalStateChangeBtChipReadyToRunningWithReset) {
    MockHciRouterClient mock_router_client;
    EXPECT_CALL(mock_router_client, OnBluetoothChipReady()).Times(1);
    EXPECT_CALL(mock_router_client, OnBluetoothChipClosed()).Times(0);
...
TEST_F(HciRouterClientAgentTest, HandleNotifyHalStateChangeWithMultipleClients) {
    MockHciRouterClient mock_router_client1;
