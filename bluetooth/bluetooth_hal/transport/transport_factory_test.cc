...
TEST(TransportFactoryTest, UnsubscribingOneOfMultipleStillNotifiesOthers) {
    ...
    EXPECT_CALL(subscriber1, NotifyHalStateChange(HalState::kFirmwareReady)).Times(1);
    EXPECT_CALL(subscriber2, NotifyHalStateChange(_)).Times(0);
    EXPECT_CALL(subscriber3, NotifyHalStateChange(HalState::kFirmwareReady)).Times(1);
...
TEST(TransportFactoryTest, SubscribingSameSubscriberMultipleTimesNotifiesOnce) {
    ...
    EXPECT_CALL(subscriber, NotifyHalStateChange(HalState::kBtChipReady)).Times(1);
...
class MockVendorTransport : public TransportInstance {
  public:
    ...
    bool Send(const ::bluetooth_hal::hci::HalPacket& packet) override { return MockedSend(packet); }

    TransportType GetInstanceTransportType() const override { return instance_type_; }
...
TEST_F(VendorTransportTest, RegisterNullVendorTransportReturnsFalse) {
    EXPECT_FALSE(TransportFactory::RegisterVendorTransport(kVendorType1, nullptr));
}

TEST_F(VendorTransportTest, RegisterVendorTransportWithInvalidTypeTooLowReturnsFalse) {
    auto type = static_cast<TransportType>(99);  // Below kVendorStart.
    auto factory = [type]() { return std::make_unique<MockVendorTransport>(type); };
    EXPECT_FALSE(TransportFactory::RegisterVendorTransport(type, factory));
}

TEST_F(VendorTransportTest, RegisterVendorTransportWithInvalidTypeTooHighReturnsFalse) {
...
TEST_F(VendorTransportTest, RegisterVendorTransportSuccessfully) {
    auto factory = []() {
        auto transport = std::make_unique<StrictMock<MockVendorTransport>>(kVendorType1);
        EXPECT_CALL(*transport, MockedCleanup()).Times(1);
        return transport;
    };
    EXPECT_TRUE(TransportFactory::RegisterVendorTransport(kVendorType1, factory));

    std::vector<TransportType> priorities = {kVendorType1, TransportType::kUartH4};
    EXPECT_CALL(mock_hal_config_loader_, GetTransportTypePriority())
            .WillRepeatedly(ReturnRef(priorities));
    EXPECT_EQ(TransportFactory::GetTransport().GetInstanceTransportType(), kVendorType1);
}

TEST_F(VendorTransportTest, RegisterDuplicateVendorTransportTypeOverwritesBeforeInit) {
    auto factory1 = []() {
        auto transport = std::make_unique<StrictMock<MockVendorTransport>>(kVendorType1);
        EXPECT_CALL(*transport, MockedCleanup()).Times(0);
        return transport;
    };
    EXPECT_TRUE(TransportFactory::RegisterVendorTransport(kVendorType1, factory1));
    ...
    EXPECT_TRUE(TransportFactory::RegisterVendorTransport(kVendorType1, factory2));
...
TEST_F(VendorTransportTest, RegisterDuplicateVendorTransportTypeCannotOverwritesAfterInit) {
    auto factory1 = [this]() {
        auto transport = std::make_unique<StrictMock<MockVendorTransport>>(kVendorType1);
        EXPECT_CALL(*transport, MockedInitialize(&mock_callback_)).WillOnce(Return(true));
        EXPECT_CALL(*transport, MockedCleanup()).Times(2);
        return transport;
    };
    EXPECT_TRUE(TransportFactory::RegisterVendorTransport(kVendorType1, factory1));
    ...
    EXPECT_FALSE(TransportFactory::RegisterVendorTransport(kVendorType1, factory2));
...
TEST_F(VendorTransportTest, GetTransportSelectsHighestPriorityVendor) {
    ...
    auto factory2 = []() {
        auto transport = std::make_unique<StrictMock<MockVendorTransport>>(kVendorType2);
        EXPECT_CALL(*transport, MockedCleanup()).Times(2);
        return transport;
    };

    EXPECT_TRUE(TransportFactory::RegisterVendorTransport(kVendorType1, factory1));
    EXPECT_TRUE(TransportFactory::RegisterVendorTransport(kVendorType2, factory2));

    std::vector<TransportType> priorities = {kVendorType2, kVendorType1, TransportType::kUartH4};
    EXPECT_CALL(mock_hal_config_loader_, GetTransportTypePriority())
            .WillRepeatedly(ReturnRef(priorities));
...
TEST_F(VendorTransportTest, UnregisterNonExistentVendorTransportReturnsFalse) {
    EXPECT_FALSE(TransportFactory::UnregisterVendorTransport(static_cast<TransportType>(150)));
}

TEST_F(VendorTransportTest, UnregisterInvalidVendorTransportTypeReturnsFalse) {
    EXPECT_FALSE(TransportFactory::UnregisterVendorTransport(TransportType::kUartH4));
...
TEST_F(VendorTransportTest, UnregisterInactiveVendorTransportSuccessfully) {
    ...
    std::vector<TransportType> priorities_2_then_default = {kVendorType2, TransportType::kUartH4};
    EXPECT_CALL(mock_hal_config_loader_, GetTransportTypePriority())
    ...
    std::vector<TransportType> priorities_1_then_default = {kVendorType1, TransportType::kUartH4};
    EXPECT_CALL(mock_hal_config_loader_, GetTransportTypePriority())
    ...
    EXPECT_EQ(TransportFactory::GetTransport().GetInstanceTransportType(), TransportType::kUartH4);
    ...
    EXPECT_EQ(TransportFactory::GetTransport().GetInstanceTransportType(), kVendorType2);

    Mock::VerifyAndClearExpectations(&mock_hal_config_loader_);
}

TEST_F(VendorTransportTest, UnregisterAndThenTryToUseReturnsFallback) {
    auto factory = []() { return std::make_unique<StrictMock<MockVendorTransport>>(kVendorType1); };
    EXPECT_TRUE(TransportFactory::RegisterVendorTransport(kVendorType1, factory));
...
TEST_F(VendorTransportTest, SwitchToNonExistentVendorFailsAndPreservesCurrent) {
    ...
    auto factory2 = []() {
        auto transport = std::make_unique<StrictMock<MockVendorTransport>>(kVendorType2);
        EXPECT_CALL(*transport, MockedCleanup()).Times(1);  // Will be cleaned up at the end
        return transport;
    };
    EXPECT_TRUE(TransportFactory::RegisterVendorTransport(kVendorType2, factory2));
...
TEST_F(VendorTransportTest, RegisterTransportAfterInitSuccessfully) {
    ...
    std::vector<TransportType> priorities = {kVendorType1, kVendorType2, TransportType::kUartH4};
    EXPECT_CALL(mock_hal_config_loader_, GetTransportTypePriority())
            .WillRepeatedly(ReturnRef(priorities));
...
TEST_F(VendorTransportTest, GetVendorTransportReturnSameInstance) {
    ...
    auto factory = []() {
        auto transprot = std::make_unique<StrictMock<MockVendorTransport>>(kVendorType1);
