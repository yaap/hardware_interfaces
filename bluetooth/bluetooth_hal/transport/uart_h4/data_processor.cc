...
size_t DataProcessor::Send(std::span<const uint8_t> packet) {
    ...
    while (remaining_bytes > 0) {
        ssize_t ret = TEMP_FAILURE_RETRY(SystemCallWrapper::GetWrapper().Writev(fd_, &iov, 1));
        if (ret == -1) {
            if (errno == EAGAIN) {
                continue;
            }
            HAL_LOG(ERROR) << __func__ << ": Error writing to UART (" << strerror(errno) << ").";
            break;
        } else if (ret == 0) {
...
void DataProcessor::Recv(int fd) {
    ...
    const ssize_t bytes_read =
            TEMP_FAILURE_RETRY(SystemCallWrapper::GetWrapper().Read(fd, buffer, max_len));
    if (bytes_read == 0) {
