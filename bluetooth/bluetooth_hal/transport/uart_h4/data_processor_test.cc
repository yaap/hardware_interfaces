...
TEST_F(DataProcessorTest, SendPacketAtOnceReturnSuccess) {
    ...
    EXPECT_CALL(mock_system_call_wrapper_, Writev(test_fd_, _, _)).WillOnce(Return(packet.size()));
...
TEST_F(DataProcessorTest, SendPacketReturnFailWithSystemFailure) {
    const std::vector<uint8_t> packet = {0x00, 0x01, 0x02, 0x03};
    EXPECT_CALL(mock_system_call_wrapper_, Writev(test_fd_, _, _)).WillOnce(Return(-1));
...
TEST_F(DataProcessorTest, SendPacketReturnFailWithEmptyWritten) {
    const std::vector<uint8_t> packet = {0x00, 0x01, 0x02, 0x03};
    EXPECT_CALL(mock_system_call_wrapper_, Writev(test_fd_, _, _)).WillOnce(Return(0));
...
TEST_F(DataProcessorTest, ReadDataWithSystemFailureNoCallbackInvoked) {
    ON_CALL(mock_system_call_wrapper_, Read(test_fd_, _, _)).WillByDefault(Return(-1));
    EXPECT_CALL(mock_packet_handler_, HalPacketCallback(_)).Times(0);

    EXPECT_DEATH(data_processor_->Recv(test_fd_), "");
}

TEST_F(DataProcessorTest, ReadDataReturnFailWithConnectionClosedNoCallbackInvoked) {
...
INSTANTIATE_TEST_SUITE_P(
        ...
        Values(HciPacketTestParam{.type = HciPacketType::kAclData,
               ...
               HciPacketTestParam{.type = HciPacketType::kEvent,
                                  ...
                                  .payload = std::vector<uint8_t>{0x01, 0x02, 0x03}},
               HciPacketTestParam{.type = HciPacketType::kIsoData,
                                  .preamble = std::vector<uint8_t>{0x00, 0x00, 0x03, 0x00},
                                  .payload = std::vector<uint8_t>{0x01, 0x02, 0x03}},
               HciPacketTestParam{.type = HciPacketType::kThreadData,
                                  .preamble = std::vector<uint8_t>{0x00, 0x00, 0x03, 0x00},
                                  .payload = std::vector<uint8_t>{0x01, 0x02, 0x03}}));
