...
bool TransportUartH4::Initialize(TransportInstanceCallback* transport_instance_callback) {
    ...
    data_processor_ = std::make_unique<DataProcessor>(uart_fd_.get(), [&](const HalPacket& packet) {
        LOG(VERBOSE) << __func__ << ": Packet ready from data processor, notifying callback.";
...
bool TransportUartH4::Send(const HalPacket& packet) {
    ...
    if (!ResumeFromLowPowerMode()) {
        LOG(WARNING) << __func__ << ": Failed to resume from low power mode after sending packet.";
    }

    bool sent_successfully = data_processor_->Send(std::span(packet)) == packet.size();
...
void TransportUartH4::RefreshLpmTimer() {
    ...
    if (is_lpm_resumed_) {
        low_power_timer_.Schedule(std::bind_front(&TransportUartH4::SuspendToLowPowerMode, this),
...
bool TransportUartH4::ResumeFromLowPowerMode() {
    ...
    if (!HalConfigLoader::GetLoader().IsLowPowerModeSupported() ||
        ...
        LOG(VERBOSE) << __func__ << ": LPM not supported ("
                     ...
                     << "), or already resumed (" << is_lpm_resumed_ << "). Skipping resume.";
        return true;
    }
    ...
    if (!PowerManager::ResumeFromLowPowerMode()) {
        LOG(ERROR) << __func__ << ": PowerManager failed to resume from low power mode.";
        return false;
    }
...
bool TransportUartH4::SuspendToLowPowerMode() {
    ...
    if (!PowerManager::SuspendToLowPowerMode()) {
        LOG(ERROR) << __func__ << ": PowerManager failed to suspend to low power mode.";
        return false;
    }
...
bool TransportUartH4::InitializeDataPath() {
    ...
    LOG(INFO) << __func__ << ": UART open: " << (success ? "successfully." : "failed.");
    return success;
};
...
bool TransportUartH4::SetupLowPowerMode() {
    if (!HalConfigLoader::GetLoader().IsLowPowerModeSupported()) {
        LOG(INFO) << __func__ << ": Low power mode not supported by config. Skipping setup.";
        return true;
    }

    bool success = PowerManager::SetupLowPowerMode();
    LOG(INFO) << __func__ << ": Low power mode setup " << (success ? "succeeded" : "failed") << ".";
...
void TransportUartH4::TeardownLowPowerMode() {
    ...
    if (!HalConfigLoader::GetLoader().IsLowPowerModeSupported()) {
        LOG(INFO) << __func__ << ": Low power mode not supported by config. Skipping teardown.";
        return;
    }
...
void TransportUartH4::NotifyHalStateChange(HalState hal_state) {
    LOG(INFO) << __func__ << ": HAL state changed to " << HalStateToString(hal_state) << " ("
    ...
    switch (hal_state) {
        ...
        case HalState::kFirmwareDownloadCompleted: {
            ...
            LOG(DEBUG) << __func__ << ": Updating UART baud rate to " << static_cast<int>(baud_rate)
                       << " for state " << static_cast<int>(hal_state);
        ...
        default:
            LOG(DEBUG) << __func__ << ": No action for HAL state " << static_cast<int>(hal_state);
            break;
    }
}

void TransportUartH4::EnableTransportWakelock(bool enable) {
    LOG(INFO) << __func__ << ": Transport wakelock " << (enable ? "enabled" : "disabled") << ".";
...
void TransportUartH4::RegisterVendorPacketValidator(
    ...
    VendorPacketValidatorInterface::RegisterVendorPacketValidator(std::move(factory));
}
