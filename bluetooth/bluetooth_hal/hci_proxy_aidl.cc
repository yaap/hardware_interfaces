  ...
  public:
    ...
    void UnlinkToDeath(const std::shared_ptr<IBluetoothHciCallbacks>& cb) {
        ...
        binder_status_t unlink_to_death_return_status = AIBinder_unlinkToDeath(
                bluetooth_hci_callback_->asBinder().get(), client_death_recipient_, this);
    ...
    void ServiceDied() {
        if (bluetooth_hci_callback_ != nullptr &&
            ...
            LOG(ERROR) << "BluetoothHalDeathRecipient::serviceDied - Bluetooth service died";
            has_died_ = true;
        } else {
...
HciProxyAidl::HciProxyAidl() : death_recipient_(std::make_shared<BluetoothHalDeathRecipient>()) {
...
ScopedAStatus HciProxyAidl::initialize(const std::shared_ptr<IBluetoothHciCallbacks>& cb) {
    bool status = BluetoothHci::GetHci().Initialize(std::make_shared<HciProxyCallback>(cb));
    ...
    unlink_cb_ = [cb](std::shared_ptr<BluetoothHalDeathRecipient>& death_recipient) {
...
ScopedAStatus HciProxyAidl::sendHciCommand(const std::vector<uint8_t>& command) {
