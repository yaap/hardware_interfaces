...
class HalConfigLoaderImpl : public HalConfigLoader {
  public:
    ...
    const std::vector<std::string>& GetHwStagesWithoutLppControlBtPowerPin() const override;
  ...
  private:
    ...
    int vendor_transport_crash_interval_sec_{cfg_consts::kDefaultVendorTransportCrashIntervalSec};
    std::string uart_device_port_{cfg_consts::kDefaultBtUartDevicePort};
    std::vector<TransportType> transport_priority_list_{cfg_consts::kDefaultBtTransportType};
    ...
    std::string lpm_wakelock_ctrl_proc_node_{cfg_consts::kLpmWakelockCtrlProcNode};
...
int HalConfigLoaderImpl::GetBtRegOnDelayMs() const {
    return reg_on_delay_ms_;
}
...
const std::vector<TransportType>& HalConfigLoaderImpl::GetTransportTypePriority() const {
    if (IsTranportFallbackEnabled() && transport_fallback_type_ != TransportType::kUnknown) {
        static const std::vector<TransportType> fallback_types{transport_fallback_type_};
...
const std::vector<std::string>& HalConfigLoaderImpl::GetHwStagesWithoutLppControlBtPowerPin()
        const {
    return hw_stages_without_lpp_control_bt_power_pin_;
}

const std::vector<std::string>& HalConfigLoaderImpl::GetUnsupportedHwStages() const {
...
bool HalConfigLoaderImpl::IsTranportFallbackEnabled() const {
    return AndroidBaseWrapper::GetWrapper().GetBoolProperty(Property::kTransportFallbackEnabled,
                                                            false);
}

BaudRate HalConfigLoaderImpl::GetUartBaudRate([[maybe_unused]] TransportType type) const {
...
bool HalConfigLoaderImpl::IsBtSnoopLogFullModeOn() const {
    ...
    const std::string bt_snoop_log_mode =
            AndroidBaseWrapper::GetWrapper().GetProperty(Property::kBtSnoopLogMode, "disabled");
...
void HalConfigLoaderImpl::UpdateBqrEventMask(const std::string& mask) {
    const std::string current_bqr_event_mask =
            AndroidBaseWrapper::GetWrapper().GetProperty(Property::kBqrEventMask, "false");
...
void HalConfigLoaderImpl::UpdateTransportFallbackType(int type) {
    ...
    transport_fallback_type_ =
            (fallback_type >= TransportType::kUartH4 && fallback_type < TransportType::kUnknown)
                    ? fallback_type
                    : TransportType::kUnknown;
}

void HalConfigLoaderImpl::UpdateLdacQualityMode(const std::string& mode) {
    LOG(INFO) << __func__ << ": Set " << Property::kLdacDefaultQualityMode << ": " << mode << ".";
    AndroidBaseWrapper::GetWrapper().SetProperty(Property::kLdacDefaultQualityMode, mode);
}
...
bool HalConfigLoaderImpl::LoadConfigFromString(std::string_view content) {
    ...
    if (config.has_sar_backoff_high_resolution_enabled()) {
        is_sar_backoff_high_resolution_enabled_ = config.sar_backoff_high_resolution_enabled();
    }
    ...
    if (config.has_vendor_transport_crash_interval_sec()) {
        vendor_transport_crash_interval_sec_ = config.vendor_transport_crash_interval_sec();
    }
    ...
    if (config.has_energy_controller_logging_enabled()) {
        is_energy_controller_logging_enabled_ = config.energy_controller_logging_enabled();
    }
    ...
    if (config.has_ble_non_connection_sar_enabled()) {
        is_ble_non_connection_sar_enabled_ = config.ble_non_connection_sar_enabled();
    }
    ...
    if (config.has_enhanced_packet_validation_supported()) {
        is_enhanced_packet_validation_supported_ = config.enhanced_packet_validation_supported();
    }
...
std::string HalConfigLoaderImpl::DumpConfigToString() const {
    ...
    ss << "IsSarBackoffHighResolutionEnabled: " << IsSarBackoffHighResolutionEnabled() << "\n";
    ss << "GetBtRegOnDelayMs: " << GetBtRegOnDelayMs() << "\n";
    ss << "GetBtUartDevicePort: \"" << GetBtUartDevicePort() << "\"\n";
    ss << "GetTransportTypePriority (Effective): " << VectorToString(GetTransportTypePriority())
       << "\n";
    ...
    ss << "GetVendorTransportCrashIntervalSec: " << GetVendorTransportCrashIntervalSec() << "\n";
    ss << "IsHpUartSkipSuspendSupported: " << IsHpUartSkipSuspendSupported() << "\n";
    ss << "IsEnergyControllerLoggingSupported: " << IsEnergyControllerLoggingSupported() << "\n";
    ...
    ss << "GetKernelRxWakelockTimeMilliseconds: " << GetKernelRxWakelockTimeMilliseconds() << "\n";
    ...
    ss << "IsEnhancedPacketValidationSupported: \"" << IsEnhancedPacketValidationSupported()
       << "\"\n";
