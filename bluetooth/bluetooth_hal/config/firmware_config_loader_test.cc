...
TEST(FirmwareConfigLoaderTest, IsSameSingleton) {
    ...
    EXPECT_EQ(&FirmwareConfigLoader::GetLoader(), &FirmwareConfigLoader::GetLoader());
}
...
TEST_F(FirmwareConfigLoaderTestBase, GetNextFirmwareDataOnInit) {
    EXPECT_FALSE(FirmwareConfigLoader::GetLoader().GetNextFirmwareData().has_value());
}
...
TEST_F(FirmwareConfigLoaderTestBase, GetFirmwareFileCountAfterLoadingConfig) {
    ...
    EXPECT_TRUE(
            FirmwareConfigLoader::GetLoader().LoadConfigFromString(kMultiTransportValidContent));
...
class LoadConfigSetupCommandTypeTest : public FirmwareConfigLoaderTestBase,
...
INSTANTIATE_TEST_SUITE_P(
        ...
        Values(SetupCommandValueTestParam{.command_type = SetupCommandType::kReset,
                                          .expected_command = {}},
               SetupCommandValueTestParam{.command_type = SetupCommandType::kReadChipId,
                                          .expected_command = {}},
               SetupCommandValueTestParam{.command_type = SetupCommandType::kUpdateChipBaudRate,
                                          .expected_command = {}},
               SetupCommandValueTestParam{.command_type = SetupCommandType::kSetFastDownload,
                                          .expected_command = {}},
               SetupCommandValueTestParam{.command_type = SetupCommandType::kDownloadMinidrv,
                                          .expected_command = {}},
               SetupCommandValueTestParam{.command_type = SetupCommandType::kReadFwVersion,
                                          .expected_command = {}},
               SetupCommandValueTestParam{.command_type = SetupCommandType::kSetupLowPowerMode,
                                          .expected_command = {}},
               SetupCommandValueTestParam{.command_type = SetupCommandType::kWriteBdAddress,
                                          .expected_command = {}}));

class LoadConfigSetupCommandTypeTestVendor : public FirmwareConfigLoaderTestBase,
                                             public WithParamInterface<SetupCommandValueTestParam> {
  protected:
    void SetUp() override {
        FirmwareConfigLoaderTestBase::SetUp();
...
INSTANTIATE_TEST_SUITE_P(TestVendorSetupCommandTypeConfig, LoadConfigSetupCommandTypeTestVendor,
                         Values(SetupCommandValueTestParam{.command_type = SetupCommandType::kReset,
                                                           .expected_command = {}},
                                SetupCommandValueTestParam{
                                        .command_type = SetupCommandType::kUpdateChipBaudRate,
                                        .expected_command = {0x01, 0x18, 0xfc, 0x06, 0x00, 0x00,
                                                             0x00, 0x09, 0x3d, 0x00}}));
...
TEST_F(FirmwareConfigLoaderTestBase, DumpConfigToString) {
    ...
    EXPECT_NE(dump.find("Active Configuration for Transport Type: 1"), std::string::npos);
    EXPECT_NE(dump.find("Data Reading Method: (Default) COMMAND_BASED"),  // Check default.
              std::string::npos);
    ...
    EXPECT_NE(dump.find("Active Configuration for Transport Type: 100"), std::string::npos);
}

TEST_F(FirmwareConfigLoaderTestBase, LoadMultiTransportConfigAndSelect) {
    std::vector<TransportType> priority_list = {
            TransportType::kUartH4, static_cast<TransportType>(TransportType::kVendorStart)};
    EXPECT_CALL(mock_hal_config_loader_, GetTransportTypePriority())
            .WillRepeatedly(ReturnRef(priority_list));
    ...
    auto reset_cmd =
            FirmwareConfigLoader::GetLoader().GetSetupCommandPacket(SetupCommandType::kReset);
    EXPECT_TRUE(reset_cmd.has_value());
    EXPECT_EQ(reset_cmd.value().get().GetPayload(), std::vector<uint8_t>({0x01, 0x03, 0x0c, 0x00}));
    ...
    EXPECT_EQ(baud_rate_cmd.value().get().GetPayload(),
              std::vector<uint8_t>({0x01, 0x18, 0xfc, 0x06, 0x00, 0x00, 0x00, 0x09, 0x3d, 0x00}));
...
TEST_F(FirmwareConfigLoaderTestBase, LoadConfigWithEmptyFirmwareConfigs) {
    constexpr std::string_view kEmptyFirmwareConfigs = R"({"firmware_configs": []})";
    std::vector<TransportType> priority_list = {TransportType::kUartH4};
    EXPECT_CALL(mock_hal_config_loader_, GetTransportTypePriority())
            .WillRepeatedly(ReturnRef(priority_list));

    EXPECT_TRUE(FirmwareConfigLoader::GetLoader().LoadConfigFromString(kEmptyFirmwareConfigs));

    EXPECT_EQ(FirmwareConfigLoader::GetLoader().GetLoadMiniDrvDelayMs(),
              cfg_consts::kDefaultLoadMiniDrvDelayMs);
    EXPECT_EQ(FirmwareConfigLoader::GetLoader().GetLaunchRamDelayMs(),
              cfg_consts::kDefaultLaunchRamDelayMs);
    auto cmd = FirmwareConfigLoader::GetLoader().GetSetupCommandPacket(SetupCommandType::kReset);
    EXPECT_FALSE(cmd.has_value());
}

TEST_F(FirmwareConfigLoaderTestBase, ResetFirmwareDataLoadingStateWithActiveConfig) {
    std::vector<TransportType> priority_list = {TransportType::kUartH4};
    EXPECT_CALL(mock_hal_config_loader_, GetTransportTypePriority())
            .WillRepeatedly(ReturnRef(priority_list));
    EXPECT_TRUE(
    ...
    EXPECT_TRUE(FirmwareConfigLoader::GetLoader().ResetFirmwareDataLoadingState());
}

TEST_F(FirmwareConfigLoaderTestBase, ResetFirmwareDataLoadingStateNoActiveConfig) {
    constexpr std::string_view kEmptyFirmwareConfigs = R"({"firmware_configs": []})";
    std::vector<TransportType> priority_list = {};
    EXPECT_CALL(mock_hal_config_loader_, GetTransportTypePriority())
            .WillRepeatedly(ReturnRef(priority_list));
    ...
    EXPECT_FALSE(FirmwareConfigLoader::GetLoader().ResetFirmwareDataLoadingState());
}
...
TEST(SetupCommandPacketTest, HandleDifferentSetupCommandPacketTypes) {
    ...
    for (int i = 0; i <= static_cast<int>(SetupCommandType::kWriteBdAddress); ++i) {
...
class FirmwareDataPacketByPacketTest : public FirmwareConfigLoaderTestBase {
  protected:
    void SetUp() override {
        ...
        EXPECT_CALL(mock_hal_config_loader_, GetTransportTypePriority())
                .WillRepeatedly(ReturnRef(priority_list));
        EXPECT_TRUE(FirmwareConfigLoader::GetLoader().LoadConfigFromString(kConfigPacketByPacket));
    }
...
TEST_F(FirmwareDataPacketByPacketTest, GetNextFirmwareDataPacketByPacketReturnReadHeaderFailError) {
    EXPECT_CALL(mock_system_call_wrapper_,
                Open(MatcherFactory::CreateStringMatcher("/test/fw/test_fw_packet.bin"), _))
            .WillOnce(Return(kFile1Fd));

    ASSERT_TRUE(FirmwareConfigLoader::GetLoader().ResetFirmwareDataLoadingState());
...
TEST_F(FirmwareDataPacketByPacketTest,
    ...
    EXPECT_CALL(mock_system_call_wrapper_, Read(kFile1Fd, _, 3))
            .WillOnce(DoAll(Invoke([&](int, void* buf, size_t) {
                            ...
                            Return(3)));
    EXPECT_CALL(mock_system_call_wrapper_, Read(kFile1Fd, _, 2)).WillOnce(Return(-1));  // Error.
    auto data_packet = FirmwareConfigLoader::GetLoader().GetNextFirmwareData();
    EXPECT_FALSE(data_packet.has_value());
}

TEST_F(FirmwareDataPacketByPacketTest, GetNextFirmwareDataPacketByPacketSuccess) {
    EXPECT_CALL(mock_system_call_wrapper_,
                Open(MatcherFactory::CreateStringMatcher("/test/fw/test_fw_packet.bin"), _))
            .WillOnce(Return(kFile1Fd));
    ...
    EXPECT_EQ(data_packet1->GetPayload(), HalPacket({0x01, 0x01, 0xFC, 0x02, 0xAA, 0xBB}));
    ...
    EXPECT_EQ(data_packet2->GetPayload(), HalPacket({0x01, 0x4E, 0xFC, 0x01, 0xCC}));
...
TEST_F(MultiFilePacketByPacketTest, ReadsDataFromMultipleFilesSuccessfully) {
    // File 1.
    EXPECT_CALL(mock_system_call_wrapper_,
                Open(MatcherFactory::CreateStringMatcher("/test/fw_multi/file1_packet.bin"), _))
            .WillOnce(Return(kFile1Fd));
    ...
    // File 2.
    EXPECT_CALL(mock_system_call_wrapper_,
                Open(MatcherFactory::CreateStringMatcher("/test/fw_multi/file2_packet.bin"), _))
    ...
    EXPECT_EQ(data_packet3->GetPayload(), HalPacket({0x01, 0x01, 0xFC, 0x02, 0xDD, 0xEE}));
    ...
    EXPECT_EQ(data_packet4->GetPayload(), HalPacket({0x01, 0x4E, 0xFC, 0x01, 0xFF}));
...
class FirmwareAccumulatedBufferTest : public FirmwareConfigLoaderTestBase {
  protected:
    void SetUp() override {
        ...
        EXPECT_CALL(mock_hal_config_loader_, GetTransportTypePriority())
                .WillRepeatedly(ReturnRef(priority_list));
        EXPECT_TRUE(FirmwareConfigLoader::GetLoader().LoadConfigFromString(kConfigAccumulated));
    }
...
TEST_F(FirmwareAccumulatedBufferTest,
    ...
    EXPECT_CALL(mock_system_call_wrapper_,
                Open(MatcherFactory::CreateStringMatcher("/test/fw/test_fw_accum.bin"), _))
            .WillOnce(Return(kFile1Fd));

    ASSERT_TRUE(FirmwareConfigLoader::GetLoader().ResetFirmwareDataLoadingState());

    EXPECT_CALL(mock_system_call_wrapper_, Read(kFile1Fd, _, 3)).WillOnce(Return(-1));
...
TEST_F(FirmwareAccumulatedBufferTest,
    ...
    EXPECT_CALL(mock_system_call_wrapper_, Read(kFile1Fd, _, 3))
            .WillOnce(DoAll(Invoke([&](int, void* buf, size_t) {
                            ...
                            Return(3)));
    EXPECT_CALL(mock_system_call_wrapper_, Read(kFile1Fd, _, 5)).WillOnce(Return(-1));
    auto data_packet = FirmwareConfigLoader::GetLoader().GetNextFirmwareData();
    EXPECT_FALSE(data_packet.has_value());
}
...
TEST_F(FirmwareAccumulatedBufferTest, GetNextFirmwareDataAccumulatedBuffer) {
    ...
    EXPECT_CALL(mock_system_call_wrapper_, Read(kFile2Fd, _, 3))
            .WillOnce(DoAll(
                    Invoke([&](int, void* buf, size_t) { memcpy(buf, fw_data_p1_hdr.data(), 3); }),
                    Return(3)))
            .WillOnce(DoAll(
                    Invoke([&](int, void* buf, size_t) { memcpy(buf, fw_data_p2_hdr.data(), 3); }),
                    Return(3)))
            .WillOnce(DoAll(
                    Invoke([&](int, void* buf, size_t) { memcpy(buf, fw_data_p3_hdr.data(), 3); }),
                    Return(3)));

    EXPECT_CALL(mock_system_call_wrapper_, Read(kFile2Fd, _, 2))
            .WillOnce(DoAll(
                    Invoke([&](int, void* buf, size_t) { memcpy(buf, fw_data_p1_pld.data(), 2); }),
                    Return(2)));
    EXPECT_CALL(mock_system_call_wrapper_, Read(kFile2Fd, _, 1))
            .WillOnce(DoAll(
                    Invoke([&](int, void* buf, size_t) { memcpy(buf, fw_data_p2_pld.data(), 1); }),
                    Return(1)))
            .WillOnce(DoAll(
                    Invoke([&](int, void* buf, size_t) { memcpy(buf, fw_data_p3_pld.data(), 1); }),
                    Return(1)));
    ...
    ASSERT_TRUE(data_packet2.has_value());
    EXPECT_EQ(data_packet2->GetDataType(), DataType::kDataEnd);
    EXPECT_EQ(data_packet2->GetPayload(), HalPacket({0x01, 0x4E, 0xFC, 0x01, 0xEE}));

    EXPECT_FALSE(FirmwareConfigLoader::GetLoader().GetNextFirmwareData().has_value());
}

TEST_F(FirmwareAccumulatedBufferTest, GetNextFirmwareDataAccumulatedBufferExceedsInternalBuffer) {
    EXPECT_CALL(mock_system_call_wrapper_,
                Open(MatcherFactory::CreateStringMatcher("/test/fw/test_fw_accum.bin"), _))
...

    {
        ...
        for (int i = 0; i < 126; ++i) {
            EXPECT_CALL(mock_system_call_wrapper_, Read(kFile3Fd, _, 3))
                    .WillOnce(DoAll(
                            Invoke([&](int, void* buf, size_t) { memcpy(buf, p1_hdr.data(), 3); }),
                            Return(3)));
        ...
        EXPECT_CALL(mock_system_call_wrapper_, Read(kFile3Fd, _, 3))
                .WillOnce(DoAll(
                        Invoke([&](int, void* buf, size_t) { memcpy(buf, p2_hdr.data(), 3); }),
                        Return(3)));
        ...
        EXPECT_CALL(mock_system_call_wrapper_, Read(kFile3Fd, _, 3))
                .WillOnce(DoAll(
                        Invoke([&](int, void* buf, size_t) { memcpy(buf, p3_hdr.data(), 3); }),
                        Return(3)));
    ...
    EXPECT_CALL(mock_system_call_wrapper_, Read(kFile3Fd, _, 3))
            .WillOnce(DoAll(Invoke([&](int, void* buf, size_t) { memcpy(buf, p4_hdr.data(), 3); }),
                            Return(3)));
...
TEST_F(MultiFileAccumulatedTest, ReadsDataFromMultipleFilesSuccessfully) {
    ...
    EXPECT_CALL(
            ...
            Open(MatcherFactory::CreateStringMatcher("/test/fw_multi_accum/file1_accum.bin"), _))
            .WillOnce(Return(kFile1Fd));
    ...
    EXPECT_CALL(
            mock_system_call_wrapper_,
            Open(MatcherFactory::CreateStringMatcher("/test/fw_multi_accum/file2_accum.bin"), _))
            .WillOnce(Return(kFile2Fd));
    ...
    EXPECT_EQ(data_packet3->GetPayload(), HalPacket({0x01, 0x4E, 0xFC, 0x01, 0xFF}));

    // No more data.
    EXPECT_FALSE(FirmwareConfigLoader::GetLoader().GetNextFirmwareData().has_value());
}

class FirmwareDataPacketByPacketFixedSizeTest : public FirmwareConfigLoaderTestBase {
  protected:
    void SetUp() override {
        FirmwareConfigLoaderTestBase::SetUp();
        std::vector<TransportType> priority_list = {TransportType::kUartH4};
...
TEST_F(FirmwareDataPacketByPacketFixedSizeTest, ReadSingleFileSuccessfully) {
    EXPECT_CALL(mock_system_call_wrapper_,
                Open(MatcherFactory::CreateStringMatcher("/test/fw/test_fw_packet_fixed.bin"), _))
            .WillOnce(Return(kFile1Fd));
    ASSERT_TRUE(FirmwareConfigLoader::GetLoader().ResetFirmwareDataLoadingState());
...
TEST_F(FirmwareAccumulatedFixedSizeTest, AccumulateMultipleChunks) {
    EXPECT_CALL(mock_system_call_wrapper_,
                Open(MatcherFactory::CreateStringMatcher("/test/fw/test_fw_accum_fixed.bin"), _))
            .WillOnce(Return(kFile1Fd));
    ASSERT_TRUE(FirmwareConfigLoader::GetLoader().ResetFirmwareDataLoadingState());
    ...
    EXPECT_CALL(mock_system_call_wrapper_, Read(kFile1Fd, _, kChunkSize))
            .WillOnce(
                    DoAll(Invoke([&](int, void* buf, size_t c) { memcpy(buf, chunk1.data(), c); }),
                          Return(kChunkSize)))
            .WillOnce(
                    DoAll(Invoke([&](int, void* buf, size_t c) { memcpy(buf, chunk2.data(), c); }),
                          Return(kChunkSize)))
    ...
    expected_payload1.insert(expected_payload1.end(), chunk1.begin(), chunk1.end());
    expected_payload1.insert(expected_payload1.end(), chunk2.begin(), chunk2.end());
    ...
    expected_payload2.insert(expected_payload2.end(), chunk3.begin(), chunk3.end());
...
TEST_F(FirmwareAccumulatedFixedSizeTest, ExceedsInternalBuffer) {
    ...
    const size_t num_full_chunks_to_fill_buffer = kInternalBufferSize / kChunkSize;  // 1024 chunks.
    ...
    HalPacket expected_first_batch;
    {
        ...
        for (size_t i = 0; i < num_full_chunks_to_fill_buffer; ++i) {
            EXPECT_CALL(mock_system_call_wrapper_, Read(kFile1Fd, _, kChunkSize))
                    .WillOnce(DoAll(
                            Invoke([&](int, void* buf, size_t c) { memcpy(buf, chunk.data(), c); }),
                            Return(kChunkSize)));
            expected_first_batch.insert(expected_first_batch.end(), chunk.begin(), chunk.end());
        }
    ...
    expected_third_batch.insert(expected_third_batch.end(), last_chunk.begin(), last_chunk.end());
    EXPECT_EQ(data_packet3->GetPayload(), expected_third_batch);

    EXPECT_FALSE(FirmwareConfigLoader::GetLoader().GetNextFirmwareData().has_value());
}

class FirmwareAccumulatedCustomBufferSizeTest : public FirmwareConfigLoaderTestBase {
  protected:
    void SetUp() override {
        FirmwareConfigLoaderTestBase::SetUp();
        std::vector<TransportType> priority_list = {TransportType::kUartH4};
...
TEST_F(FirmwareAccumulatedCustomBufferSizeTest, RespectsCustomBufferSizeWithMultipleCycles) {
    EXPECT_CALL(mock_system_call_wrapper_,
                Open(MatcherFactory::CreateStringMatcher("/test/fw/test_fw_accum_custom.bin"), _))
            .WillOnce(Return(kFile1Fd));
    ASSERT_TRUE(FirmwareConfigLoader::GetLoader().ResetFirmwareDataLoadingState());
    ...
    auto mock_read_cmd = [&](uint16_t opcode, uint8_t len) {
        ...
        EXPECT_CALL(mock_system_call_wrapper_, Read(kFile1Fd, _, static_cast<size_t>(len)))
                .WillOnce(DoAll(
                        Invoke([=](int, void* buf, size_t) { memset(buf, kTestFillByte, len); }),
                        Return(len)));
    };

    {
        ...
        // Launch RAM command
        mock_read_cmd(cfg_consts::kDefaultHciVscLaunchRamOpcode, kLaunchRamVscPayloadLen);
        EXPECT_CALL(mock_system_call_wrapper_, Close(kFile1Fd)).Times(1);
    }
...
class FirmwareDataCommandBasedCustomOpcodeTest : public FirmwareConfigLoaderTestBase {
  protected:
    void SetUp() override {
        FirmwareConfigLoaderTestBase::SetUp();
        std::vector<TransportType> priority_list = {TransportType::kUartH4};
...
TEST_F(FirmwareDataCommandBasedCustomOpcodeTest, UsesCustomOpcode) {
    EXPECT_CALL(mock_system_call_wrapper_,
                Open(MatcherFactory::CreateStringMatcher("/test/fw/test_fw_cmd_custom_op.bin"), _))
            .WillOnce(Return(kFile1Fd));
    ASSERT_TRUE(FirmwareConfigLoader::GetLoader().ResetFirmwareDataLoadingState());
    ...
    std::vector<uint8_t> packet2_header = {
            ...
            static_cast<uint8_t>((kCustomLaunchRamOpcode >> 8) & 0xFF), 0x01};  // Custom Launch RAM
    std::vector<uint8_t> packet2_payload = {0xCC};

    EXPECT_CALL(mock_system_call_wrapper_, Read(kFile1Fd, _, 3))
            .WillOnce(DoAll(
                    Invoke([&](int, void* buf, size_t) { memcpy(buf, packet1_header.data(), 3); }),
                    Return(3)));
    EXPECT_CALL(mock_system_call_wrapper_, Read(kFile1Fd, _, 2))
            .WillOnce(DoAll(
                    Invoke([&](int, void* buf, size_t) { memcpy(buf, packet1_payload.data(), 2); }),
                    Return(2)));
    ...
    EXPECT_CALL(mock_system_call_wrapper_, Read(kFile1Fd, _, 3))
            .WillOnce(DoAll(
                    Invoke([&](int, void* buf, size_t) { memcpy(buf, packet2_header.data(), 3); }),
                    Return(3)));
    EXPECT_CALL(mock_system_call_wrapper_, Read(kFile1Fd, _, 1))
            .WillOnce(DoAll(
                    Invoke([&](int, void* buf, size_t) { memcpy(buf, packet2_payload.data(), 1); }),
                    Return(1)));
    EXPECT_CALL(mock_system_call_wrapper_, Close(kFile1Fd)).Times(1);
...
class FirmwareDataFixedSizeDefaultChunkTest : public FirmwareConfigLoaderTestBase {
  protected:
    void SetUp() override {
        FirmwareConfigLoaderTestBase::SetUp();
        std::vector<TransportType> priority_list = {TransportType::kUartH4};
...
TEST_F(FirmwareDataFixedSizeDefaultChunkTest, UsesDefaultChunkSize) {
    EXPECT_CALL(
            mock_system_call_wrapper_,
            Open(MatcherFactory::CreateStringMatcher("/test/fw/test_fw_fixed_default_chunk.bin"),
                 _))
            .WillOnce(Return(kFile1Fd));
    ASSERT_TRUE(FirmwareConfigLoader::GetLoader().ResetFirmwareDataLoadingState());
    ...
    EXPECT_CALL(mock_system_call_wrapper_, Read(kFile1Fd, _, cfg_consts::kDefaultFixedChunkSize))
            .WillOnce(DoAll(
                    Invoke([&](int, void* buf, size_t count) { memcpy(buf, chunk.data(), count); }),
