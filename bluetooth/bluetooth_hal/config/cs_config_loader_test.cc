...
class CsConfigLoaderTest : public Test {
  protected:
    void SetUp() override { MockAndroidBaseWrapper::SetMockWrapper(&mock_android_base_wrapper_); }
...
TEST_F(CsConfigLoaderTest, ParseValidContentAndGetCsCalibrationCommands) {
    ...
    const std::vector<HalPacket>& commands = CsConfigLoader::GetLoader().GetCsCalibrationCommands();

    EXPECT_EQ(commands.size(), 2);

    EXPECT_EQ(commands[0], std::vector<uint8_t>({0x01, 0x34, 0x12, 0x08, 0x01, 0x02, 0x03, 0x04,
                                                 0x05, 0x06, 0x07, 0x08}));

    EXPECT_EQ(commands[1],
              std::vector<uint8_t>({0x01, 0x34, 0x12, 0x05, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E}));
}
...
TEST_F(CsConfigLoaderTest, LoadConfigFromFile_ValidFile) {
    ...
    ASSERT_TRUE(android::base::WriteStringToFd(std::string(kValidContent), temp_file.fd));
...
TEST_F(CsConfigLoaderTest, LoadConfigFromFile_NonExistentFile) {
    ...
    EXPECT_FALSE(CsConfigLoader::GetLoader().LoadConfigFromFile("/a/b/c/d/non_existent_file.json"));
...
TEST_F(CsConfigLoaderTest, LoadConfigFromFile_InvalidContent_KeepsOldConfig) {
    ...
    EXPECT_TRUE(CsConfigLoader::GetLoader().LoadConfigFromString(kValidContent));
    const auto& original_commands = CsConfigLoader::GetLoader().GetCsCalibrationCommands();
    ...
    const auto& commands_after_failed_load = CsConfigLoader::GetLoader().GetCsCalibrationCommands();
...
TEST_F(CsConfigLoaderTest, GetCsCalibrationCommands_ReloadsFromProperty) {
    ...
    TemporaryFile temp_file;
    ASSERT_TRUE(temp_file.fd != -1);
    ASSERT_TRUE(android::base::WriteStringToFd(std::string(kAnotherValidContent), temp_file.fd));
    EXPECT_CALL(mock_android_base_wrapper_, GetProperty(Property::kCsConfigFile, ""))
    ...
    EXPECT_EQ(reloaded_commands[0], std::vector<uint8_t>({0x02, 0xD2, 0x04, 0x02, 0x63, 0x64}));
}

TEST_F(CsConfigLoaderTest, GetCsCalibrationCommands_ReloadFails_KeepsOriginalConfig) {
    // 1. Load an initial configuration.
    EXPECT_TRUE(CsConfigLoader::GetLoader().LoadConfigFromString(kValidContent));
    const auto& original_commands = CsConfigLoader::GetLoader().GetCsCalibrationCommands();
