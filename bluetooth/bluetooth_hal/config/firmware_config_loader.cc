...
class FirmwareConfigLoaderImpl : public FirmwareConfigLoader {
  public:
    ...
    std::optional<std::reference_wrapper<const SetupCommandPacket>> GetSetupCommandPacket(
            SetupCommandType command_type) const override;
  ...
  private:
    void LoadSetupCommandsFromConfig(
            ...
            std::unordered_map<SetupCommandType, std::unique_ptr<SetupCommandPacket>>& target_map);
    ...
    std::unordered_map<TransportType, FirmwareConfigForTransport> transport_specific_configs_;
    std::optional<std::reference_wrapper<const FirmwareConfigForTransport>> active_config_;
    ...
    size_t accumulated_buffer_max_size_{cfg_consts::kDefaultAccumulatedBufferMaxSize};
};
...
bool FirmwareConfigLoaderImpl::OpenNextFirmwareFile() {
    ...
    if (static_cast<size_t>(current_firmware_file_index_) >= current_firmware_filenames_.size()) {
    ...
    LOG(INFO) << __func__ << ": Attempting to open firmware file: " << firmware_path;
    firmware_file_fd_ = SystemCallWrapper::GetWrapper().Open(firmware_path.c_str(), O_RDONLY);
    if (firmware_file_fd_ < 0) {
        LOG(ERROR) << __func__ << ": Cannot open firmware file: " << firmware_path
                   << ". Error: " << strerror(errno);
        return false;
    }
    LOG(INFO) << __func__ << ": Successfully opened firmware file: " << firmware_path;
    return true;
}
...
std::optional<DataPacket> FirmwareConfigLoaderImpl::GetNextFirmwareData() {
    ...
    if (!active_config_ || !active_config_->get().has_firmware_data_loading_type()) {
        LOG(WARNING) << __func__ << ": Data loading type not set, defaulting to PACKET_BY_PACKET.";
        return GetNextFirmwareDataByPacket();
    }

    DataLoadingType data_loading_type =
            static_cast<DataLoadingType>(active_config_->get().firmware_data_loading_type());
...
FirmwareConfigLoaderImpl::GetSetupCommandPacket(SetupCommandType command_type) const {
    if (!active_config_) {
        LOG(ERROR) << __func__ << ": No active firmware configuration selected.";
        return std::nullopt;
    }
...
int FirmwareConfigLoaderImpl::GetLoadMiniDrvDelayMs() const {
    ...
    return config.has_load_mini_drv_delay_ms() ? config.load_mini_drv_delay_ms()
...
int FirmwareConfigLoaderImpl::GetLaunchRamDelayMs() const {
    ...
    const auto& config = active_config_->get();
    return config.has_launch_ram_delay_ms() ? config.launch_ram_delay_ms()
...
std::string FirmwareConfigLoaderImpl::DumpConfigToString() const {
    ...
    ss << "Loaded Transport Specific Configurations: " << transport_specific_configs_.size()
       << "\n";
    ...
    for (const auto& [transport_type, config] : transport_specific_configs_) {
        ...
        ss << "    Accumulated Buffer Max Size: " << config.accumulated_buffer_max_size()
           << " bytes\n";
    ...
    if (active_config_) {
        ss << "Active Configuration for Transport Type: " << active_config_->get().transport_type()
           << "\n";
    } else {
...
bool FirmwareConfigLoaderImpl::LoadConfigFromString(std::string_view content) {
    ...
    if (!status.ok()) {
        LOG(ERROR) << __func__ << ": Failed to parse json file, error: " << status.message();
        return false;
    }
    ...
    const TransportType active_transport_type = TransportFactory::GetTransportType();
    if (active_transport_type != TransportType::kUnknown) {
        ...
        for (auto type : transport_type_priorities) {
            if (transport_specific_configs_.find(type) != transport_specific_configs_.end()) {
...
bool FirmwareConfigLoaderImpl::SelectFirmwareConfiguration(TransportType transport_type) {
    auto it = transport_specific_configs_.find(transport_type);
    if (it == transport_specific_configs_.end()) {
        LOG(ERROR) << __func__ << ": No firmware configuration found for transport type "
    ...
    LOG(INFO) << __func__ << ": Selected firmware configuration for transport type "
              << static_cast<int>(transport_type);
    ...
    accumulated_buffer_max_size_ = config.has_accumulated_buffer_max_size()
    ...
    switch (config.data_reading_method_case()) {
        case FirmwareConfigForTransport::kCommandBasedReading:
            ...
            launch_ram_opcode_ = config.command_based_reading().has_launch_ram_opcode()
        ...
        case FirmwareConfigForTransport::kFixedSizeReading:
            ...
            LOG(INFO) << __func__ << ": Data reading method set to FIXED_SIZE, Chunk Size: "
...
void FirmwareConfigLoaderImpl::LoadSetupCommandsFromConfig(
    ...
    auto add = [&](SetupCommandType type, const RepeatedField<uint32_t>& data) {
        target_map.emplace(type, std::make_unique<SetupCommandPacket>(type, to_vector(data)));
    };
    ...
    if (commands.hci_update_chip_baud_rate_size()) {
        add(SetupCommandType::kUpdateChipBaudRate, commands.hci_update_chip_baud_rate());
    }
    ...
    if (commands.hci_setup_low_power_mode_size()) {
        add(SetupCommandType::kSetupLowPowerMode, commands.hci_setup_low_power_mode());
    }
...
std::optional<DataPacket> FirmwareConfigLoaderImpl::GetNextPacketByCommand() {
    while (true) {
        ...
        ssize_t payload_bytes_read = TEMP_FAILURE_RETRY(SystemCallWrapper::GetWrapper().Read(
                firmware_file_fd_, packet_payload.data() + 1 + header.size(), payload_size));
...
std::optional<DataPacket> FirmwareConfigLoaderImpl::GetNextPacketByFixedSize() {
    ...
    bool is_end_of_current_file = (bytes_read < static_cast<ssize_t>(fixed_chunk_size_));
...
std::optional<DataPacket> FirmwareConfigLoaderImpl::GetNextSinglePacket() {
    return data_reading_method_ == DataReadingMethod::kCommandBased ? GetNextPacketByCommand()
                                                                    : GetNextPacketByFixedSize();
}

std::optional<DataPacket> FirmwareConfigLoaderImpl::GetNextFirmwareDataByPacket() {
    return GetNextSinglePacket();
}

std::optional<DataPacket> FirmwareConfigLoaderImpl::GetNextFirmwareDataByAccumulation() {
    ...
    if (previous_packet_.has_value()) {
        ...
        accumulated_buffer.insert(accumulated_buffer.end(), previous_packet_->GetPayload().begin(),
    ...
    while (accumulated_buffer.size() <= accumulated_buffer_max_size_) {
        ...
        if ((next_packet->GetDataType() == DataType::kDataEnd && !accumulated_buffer.empty()) ||
        ...
        if (next_packet->GetDataType() == DataType::kDataEnd) {
            return DataPacket(DataType::kDataEnd, std::move(next_packet->GetPayload()));
        }

        accumulated_buffer.insert(accumulated_buffer.end(), next_packet->GetPayload().begin(),
