...
void BluetoothHci::HandleServiceDied() {
    ...
    if (DebugCentral::Get().IsCoredumpGenerated()) {
        LOG(ERROR) << __func__ << ": Restart Bluetooth HAL after coredump is generated";
        kill(getpid(), SIGKILL);
    }
    Close();
}
...
bool BluetoothHci::SendHciCommand(const HalPacket& packet) {
    SCOPED_ANCHOR(AnchorType::kSendHciCommand,
                  (std::stringstream()
                   << __func__ << ": 0x" << std::hex << std::setw(4) << std::setfill('0')
                   << packet.GetCommandOpcode() << " - " << std::dec << packet.size() << " bytes")
                          .str());
...
bool BluetoothHci::SendAclData(const HalPacket& packet) {
    SCOPED_ANCHOR(AnchorType::kSendAclData,
                  (std::stringstream() << __func__ << ": " << packet.size() << " bytes").str());
    SendDataToController(packet);
    return true;
}

bool BluetoothHci::SendScoData(const HalPacket& packet) {
    SCOPED_ANCHOR(AnchorType::kSendScoData,
                  (std::stringstream() << __func__ << ": " << packet.size() << " bytes").str());
    SendDataToController(packet);
    return true;
}
...
bool BluetoothHci::SendIsoData(const HalPacket& packet) {
    SCOPED_ANCHOR(AnchorType::kSendIsoData,
                  (std::stringstream() << __func__ << ": " << packet.size() << " bytes").str());
    SendDataToController(packet);
    return true;
}
...
void BluetoothHci::DispatchPacketToStack(const HalPacket& packet) {
    std::lock_guard<std::mutex> lock(callback_mutex_);
    if (bluetooth_hci_callback_ == nullptr) {
        LOG(ERROR) << "bluetooth_hci_callback is null! packet=" << packet.ToString();
        return;
    }
    HciPacketType type = packet.GetType();
    switch (type) {
        case HciPacketType::kEvent: {
            SCOPED_ANCHOR(AnchorType::kCallbackHciEvent,
        ...
        case HciPacketType::kAclData: {
            SCOPED_ANCHOR(AnchorType::kCallbackAclData,
        ...
        case HciPacketType::kScoData: {
            SCOPED_ANCHOR(AnchorType::kCallbackScoData,
        ...
        case HciPacketType::kIsoData: {
            SCOPED_ANCHOR(AnchorType::kCallbackIsoData,
...
void BluetoothHci::HandleHalStateChanged(HalState new_state, [[maybe_unused]] HalState old_state) {
    std::lock_guard<std::mutex> lock(callback_mutex_);
    if (is_initializing_ && bluetooth_hci_callback_ != nullptr) {
        switch (new_state) {
            case HalState::kRunning:
                ...
                is_initializing_ = false;
                bluetooth_hci_callback_->InitializationComplete(BluetoothHciStatus::kSuccess);
                break;
