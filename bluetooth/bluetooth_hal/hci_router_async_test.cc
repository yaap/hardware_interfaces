...
class MockTransportInstanceCallback : public TransportInstanceCallback {
  public:
    MOCK_METHOD(void, OnTransportPacketReady, (const HalPacket& packet), (override));
...
class HciRouterAsyncTest : public Test {
  protected:
    void SetUp() override {
        ...
        mock_transport_instance_callback_ = std::make_shared<MockTransportInstanceCallback>();
        ...
        router_->Initialize(mock_hci_router_callback_, mock_transport_instance_callback_.get());
    }
    ...
    std::shared_ptr<MockTransportInstanceCallback> mock_transport_instance_callback_;
...
TEST_F(HciRouterAsyncTest, DoInRouterThread) {
    ...
    bool result = router_->DoInRouterThread([&promise]() { promise.set_value(); });
    EXPECT_TRUE(result);
...
TEST_F(HciRouterAsyncTest, SynchronousDoInRouterThread) {
    ...
    bool result = router_->SynchronousDoInRouterThread([&]() { task_executed = true; });
    EXPECT_TRUE(result);
...
TEST_F(HciRouterAsyncTest, SendCommandTwiceWithoutEvent) {
    ...
    router_->SendCommand(cmd_reset, std::make_shared<HalPacketCallback>([](const HalPacket&) {}));
    router_->SendCommand(cmd_set_host_le_support,
...
TEST_F(HciRouterAsyncTest, SendHciCommandInCallback) {
    ...
    HalPacket event;
    auto callback = std::make_shared<HalPacketCallback>([&](const HalPacket& packet) {
        event = packet;
        router_->SendCommand(cmd_set_host_le_support,
                             std::make_shared<HalPacketCallback>([](const HalPacket&) {}));
    });
...
TEST_F(HciRouterAsyncTest, SendHciCommandInCallbackAfterAnotherSendCommand) {
    ...
    HalPacket event;
    auto callback_reset = std::make_shared<HalPacketCallback>([&](const HalPacket& packet) {
        event = packet;
        router_->SendCommand(cmd_set_host_le_support,
                             std::make_shared<HalPacketCallback>([](const HalPacket&) {}));
    });
...
TEST_F(HciRouterAsyncTest, SendDebugInfoCommandAfterSendCommand) {
    ...
    router_->SendCommand(cmd_reset, std::make_shared<HalPacketCallback>([](const HalPacket&) {}));
    router_->SendCommand(cmd_debug_info,
                         std::make_shared<HalPacketCallback>([](const HalPacket&) {}));
}
...
TEST_F(HciRouterAsyncTest, UpdateHalState) {
    ...
    EXPECT_CALL(mock_hci_router_client_agent_,
            ...
            .Times(1);
    EXPECT_CALL(mock_transport_factory_, NotifyHalStateChange(HalState::kPreFirmwareDownload))
            .Times(1);
...
TEST_F(HciRouterAsyncTest, ReplaceHciEventByClient) {
    ...
    EXPECT_CALL(*mock_hci_router_callback_, OnPacketCallback(event_after_intercept)).Times(1);
...
TEST_F(HciRouterAsyncTest, ComplexInterceptScenarioB) {
    ...
    EXPECT_CALL(mock_hci_router_client_agent_, DispatchPacketToClients(command_A))
            .WillOnce(Invoke([&](const HalPacket&) {
                ...
                auto callback =
                        std::make_shared<HalPacketCallback>([&](const HalPacket& captured_packet) {
