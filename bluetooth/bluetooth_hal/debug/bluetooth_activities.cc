...
constexpr std::string_view kBluetoothActivitiesDebuggingTitle = "Bluetooth Activities";
...
class BluetoothActivitiesImpl : public BluetoothActivities,
  ...
  public:
    ...
    void HandleDisconnectCompleteEvent(const ::bluetooth_hal::hci::HalPacket& event);
  ...
  private:
    ...
    void OnDeviceConnected(const BluetoothAddress& bd_address, uint16_t connection_handle);
...
BluetoothActivitiesImpl::BluetoothActivitiesImpl()
    : ble_connection_complete_event_monitor_(HciBleMetaEventMonitor(
      ...
      ble_enhanced_connection_complete_v1_event_monitor_(HciBleMetaEventMonitor(
              static_cast<uint8_t>(BleMetaEventSubCode::kEnhancedConnectionCompleteV1))),
      ble_enhanced_connection_complete_v2_event_monitor_(HciBleMetaEventMonitor(
              static_cast<uint8_t>(BleMetaEventSubCode::kEnhancedConnectionCompleteV2))),
      connection_complete_event_monitor_(
              HciEventMonitor(static_cast<uint8_t>(EventCode::kConnectionComplete))),
      disconnection_complete_event_monitor_(
              HciEventMonitor(static_cast<uint8_t>(EventCode::kDisconnectionComplete))) {
...
void BluetoothActivities::Start() {
    BluetoothActivities::Get();
}
...
bool BluetoothActivitiesImpl::IsConnected(uint16_t connection_handle) const {
    return connected_device_address_.find(connection_handle) != connected_device_address_.end();
}
...
void BluetoothActivitiesImpl::OnMonitorPacketCallback([[maybe_unused]] MonitorMode mode,
...
void BluetoothActivitiesImpl::UnregisterConnectionCountChangedCallback(uint32_t id) {
    if (!hal_flags::bt_activities_subscription()) {
        return;
    }
...
void BluetoothActivitiesImpl::HandleBleMetaEvent(const HalPacket& event) {
    ...
    ConnectionActivity activity{
            .connection_handle = event.AtUint16LittleEndian(kBleConnectionHandleOffset),
    ...
    if (event_status == static_cast<uint8_t>(EventResultCode::kSuccess)) {
        ...
        CLIENT_LOG(INFO) << __func__ << ": " << activity.event << ", connection handle: "
                         << ToHexString(activity.connection_handle, kUint16HexStringDigit)
                         << ", BD address: " << activity.bd_address.ToString() << ".";
    }
}

void BluetoothActivitiesImpl::HandleConnectCompleteEvent(const HalPacket& event) {
    ...
    ConnectionActivity activity{
            ...
            .event = "Connect Complete " + ToHexString(event.GetEventCode(), kUint8HexStringDigit),
            .status = std::string(GetResultString(event_status)),
            .timestamp = Logger::GetLogFormatTimestamp(),
    };
    UpdateConnectionHistory(activity);
...
void BluetoothActivitiesImpl::HandleDisconnectCompleteEvent(const HalPacket& event) {
    uint8_t event_status = event.At(kDisconnectionEventStatusOffset);
    uint16_t connection_handle = event.AtUint16LittleEndian(kDisconnectionHandleOffset);
    ConnectionActivity activity{
...
void BluetoothActivitiesImpl::OnDeviceConnected(const BluetoothAddress& bd_address,
                                                uint16_t connection_handle) {
...
void BluetoothActivitiesImpl::UpdateConnectionHistory(const ConnectionActivity& device) {
...
std::vector<Coredump> BluetoothActivitiesImpl::Dump() {
    ...
    bluetooth_activities_coredumps.emplace_back(kBluetoothActivitiesDebuggingTitle.data(),
                                                connection_history_dump_, CoredumpPosition::kEnd);
