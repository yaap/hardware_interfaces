...
DebugClient::DebugClient() {
    DebugCentral::Get().RegisterDebugClient(this);
}

DebugClient::~DebugClient() {
    DebugCentral::Get().UnregisterDebugClient(this);
}

void DebugClient::OnGenerateCoredump([[maybe_unused]] CoredumpErrorCode error_code,
...
DebugClient::ClientLogStream DebugClient::ClientLog(LogSeverity severity, const char* tag) {
    if (log_tag_.empty()) {
        SetClientLogTag(tag);
    }
...
void DebugClient::SetClientLogTag(const std::string& tag) {
    log_tag_ = tag;
}

DebugClient::ClientLogStream::ClientLogStream(std::deque<std::string>& log_buffer,
