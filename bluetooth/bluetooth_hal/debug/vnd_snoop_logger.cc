...
static_assert(kIsLittleEndian || (kIsBigEndian && kIsLittleEndian != kIsBigEndian),
...
constexpr uint32_t kBtSnoopDatalinkType =
        kIsLittleEndian ? 0xea030000 : 0x03ea;  // Datalink Type code for HCI UART (H4) is 1002.
...
constexpr std::string_view kBtLogPathPrefix = "/data/vendor/bluetooth/btsnoop_hci_vnd";
...
std::string GetBtSnoopLogMode() {
    return android::base::GetProperty(Property::kBtSnoopLogMode, kBtLogModeDisabled);
}

bool IsBtVndSnoopLogEnabled() {
    return android::base::GetBoolProperty(Property::kBtVendorSnoopEnabledProperty, false);
}
...
std::string GetBtVndSnoopLogMode() {
    ...
    if (bt_snoop_log_mode == kBtLogModeDisabled) {
        return HalConfigLoader::GetLoader().IsUserDebugOrEngBuild() ? kBtLogModeFiltered
                                                                    : kBtLogModeDisabled;
    }
...
size_t GetMaxPacketsPerFile() {
    const size_t max_packets_per_file = android::base::GetUintProperty<uint64_t>(
            Property::kBtSnoopMaxPacketsPerFileProperty, kDefaultBtSnoopMaxPacketsPerFile);
    LOG(INFO) << __func__ << ": Vendor btsnoop max packets: " << max_packets_per_file << ".";
...
class LoggerHandler {
  ...
  private:
        ...
        switch (task.type_) {
            ...
            case LoggerTask::LoggerTaskType::kCapture:
                if (task.args_.has_value()) {
                    Capture(task.args_->packet, task.args_->direction, task.args_->timestamp_us);
                }
                break;
            default:
    ...
    void Capture(const HalPacket& packet, VndSnoopLogger::Direction direction,
        ...
        PacketHeaderType header = {.length_original = htonl(packet.size()),
        ...
        if (++packet_counter_ > max_packets_per_file_) {
            LOG(INFO) << __func__ << ": Reach max packet per file, open new log file.";
            PrepareNewLogFile();
        }

        if (!log_ostream_.write(reinterpret_cast<const char*>(&header), sizeof(PacketHeaderType))) {
            LOG(ERROR) << __func__ << ": Failed to write packet header for btsnoop, error: \""
                       << strerror(errno) << "\".";
            return;
        }
        ...
        if (!log_ostream_.write(packet_data, captured_length - 1)) {
            LOG(ERROR) << __func__ << ": Failed to write packet payload for btsnoop, error: \""
                       << strerror(errno) << "\".";
        }
    ...
    void OpenNewLogFile() {
        ...
        if (chmod(log_file_path.c_str(), S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH) != 0) {
            LOG(ERROR) << __func__ << ": Unable to change file permissions for " << log_file_path
                       << ".";
        }

        if (!log_ostream_.is_open()) {
            LOG(ERROR) << __func__ << ": Unable to open snoop log at \"" << log_file_path
        ...
        if (!log_ostream_.write(reinterpret_cast<const char*>(&kBtSnoopFileHeader),
            ...
            LOG(ERROR) << __func__ << ": Unable to write file header to \"" << log_file_path
                       << "\", error: \"" << strerror(errno) << "\".";
        }
...
class VndSnoopLoggerImpl : public VndSnoopLogger {
  public:
    ...
    void Capture(const ::bluetooth_hal::hci::HalPacket& packet, Direction direction) override;
};
...
void VndSnoopLoggerImpl::Capture(const HalPacket& packet, Direction direction) {
    uint64_t timestamp_us = std::chrono::duration_cast<std::chrono::microseconds>(
                                    ...
                                    .count();
    LoggerHandler::GetHandler().Post(LoggerTask::CaptureTask(packet, direction, timestamp_us));
}
