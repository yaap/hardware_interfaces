...
void HciRouterInterface::OnTransportPacketReady(const HalPacket& packet) {
    ...
    DoInRouterThread([this, packet]() { hci_router_async_->OnTransportPacketReady(packet); });
}

HciRouterInterface::HciRouterInterface() : HciRouterInterface(std::make_shared<HciRouterAsync>()) {}

HciRouterInterface::HciRouterInterface(std::shared_ptr<HciRouterAsync> hci_router_async)
...
bool HciRouterInterface::Initialize(const std::shared_ptr<HciRouterCallback>& callback) {
    return DoInRouterThread([callback, this]() { hci_router_async_->Initialize(callback, this); });
}
...
bool HciRouterInterface::Send(const HalPacket& packet) {
    return DoInRouterThread([this, packet]() { hci_router_async_->Send(packet); });
}

bool HciRouterInterface::SendCommand(const HalPacket& packet, const HalPacketCallback& callback) {
    return DoInRouterThread([this, packet, callback]() {
        hci_router_async_->SendCommand(packet, std::make_shared<HalPacketCallback>(callback));
    });
}

bool HciRouterInterface::SendCommandNoAck(const HalPacket& packet) {
    return DoInRouterThread([this, packet]() { hci_router_async_->SendCommandNoAck(packet); });
}
...
void HciRouterInterface::UpdateHalState(HalState state) {
    DoInRouterThread([this, state]() { hci_router_async_->UpdateHalState(state); });
}

void HciRouterInterface::SendPacketToStack(const HalPacket& packet) {
    DoInRouterThread([this, packet]() { hci_router_async_->SendPacketToStack(packet); });
}

bool HciRouterInterface::SynchronousDoInRouterThread(std::function<void()> task) {
