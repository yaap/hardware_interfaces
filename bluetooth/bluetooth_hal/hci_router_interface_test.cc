...
class MockHciRouterAsync : public HciRouterAsync {
  public:
    ...
    MOCK_METHOD(bool, SynchronousDoInRouterThread, (std::function<void()> task), (override));
    ...
    MOCK_METHOD(bool, Initialize,
                ...
                (override));
    MOCK_METHOD(bool, InitializeModules, (TransportInstanceCallback * transport_callback),
                (override));
    MOCK_METHOD(bool, Send, (const HalPacket& packet), (override));
    MOCK_METHOD(bool, SendCommand,
                (const HalPacket& packet, const std::shared_ptr<HalPacketCallback>& callback),
                (override));
...
class HciRouterInterfaceTestInstance : public HciRouterInterface {
  public:
    HciRouterInterfaceTestInstance(std::shared_ptr<MockHciRouterAsync> mock_hci_router_async)
...
class HciRouterInterfaceTest : public Test {
  protected:
    ...
    void SetUp() override {
        ...
        // AcceleratedBtOn.
        EXPECT_CALL(*mock_hci_router_async_, DoInRouterThread(An<std::function<void()>>()))
                .WillRepeatedly(Return(true));

        hci_router_interface_ =
                std::make_shared<HciRouterInterfaceTestInstance>(mock_hci_router_async_);
    }
...
TEST_F(HciRouterInterfaceTest, Initialize) {
    ...
    EXPECT_CALL(*mock_hci_router_async_, Initialize(mock_hci_router_callback_, _)).Times(1);
...
TEST_F(HciRouterInterfaceTest, GetHalState) {
    EXPECT_CALL(*mock_hci_router_async_, GetHalState()).WillOnce(Return(HalState::kShutdown));
