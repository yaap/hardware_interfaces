...
constexpr char kRandGenBdaddrPath[] = "/mnt/vendor/persist/bluetooth/bdaddr.txt";
...
void ChipProvisioner::Initialize(const std::function<void(HalState)> on_hal_state_update) {
...
bool ChipProvisioner::DownloadFirmware() {
    ...
    if (state_ != ProvisioningState::kDone && !stop_requested_.load()) {
        LOG(FATAL) << __func__ << ": Failed to complete download firmware. Final state: "
                   << static_cast<int>(state_);
    } else {
        LOG(ERROR) << __func__
                   << ": Firmware download stopped. Final state: " << static_cast<int>(state_)
    ...
    switch (HciRouter::GetRouter().GetHalState()) {
        case HalState::kBtChipReady:
            ...
            if (!ExecuteCurrentSetupStep(SetupCommandType::kReset)) {
                LOG(FATAL) << __func__ << ": Failed to reset firmware when turning ON BT.";
            }
        ...
        case HalState::kRunning:
            ...
            if (!ExecuteCurrentSetupStep(SetupCommandType::kReset)) {
                ...
                LOG(ERROR) << __func__ << ": Failed to reset firmware when turning OFF BT.";
                kill(getpid(), SIGKILL);
            }
            break;
        default:
...
bool ChipProvisioner::ExecuteCurrentSetupStep(SetupCommandType command_type) {
    ...
    if (!next_setup_command.has_value()) {
        LOG(INFO) << __func__ << ": No command for type " << SetupCommandTypeToString(command_type);
        return true;
    }
...
void ChipProvisioner::RunProvisioningSequence() {
    ...
    while (running && !stop_requested_.load()) {
        LOG(INFO) << __func__
                  << ": Executing provisioning state: " << ProvisioningStateToString(state_) << " ("
                  << static_cast<int>(state_) << ")";
        switch (state_) {
            ...
            case ProvisioningState::kCheckFirmwareStatus:
                if (HciRouter::GetRouter().GetHalState() == HalState::kFirmwareDownloadCompleted) {
            ...
            case ProvisioningState::kWriteFirmware: {
                ...
                std::this_thread::sleep_for(std::chrono::milliseconds(mini_drv_delay_ms));
                ...
                if (WriteFwPatchramPacket()) {
                    ...
                    LOG(ERROR) << __func__ << ": Failed to write Firmware PatchRam Packets.";
                    state_ = ProvisioningState::kError;
                }
                break;
            }
...
bool ChipProvisioner::WriteFwPatchramPacket() {
    ...
    while ((data_packet = config_loader_.GetNextFirmwareData()).has_value()) {
        ...
        if (data_packet->GetDataType() == config::DataType::kDataFragment) {
            ...
            if (!SendCommandAndWait(data_packet->GetPayload())) {
                LOG(ERROR) << __func__ << ": Failed to send final firmware data packet.";
                return false;
            }
            files_completed++;
        }
    ...
    if (files_completed != config_loader_.GetFirmwareFileCount()) {
        LOG(ERROR) << __func__ << ": Incomplete firmware download. Expected "
                   << config_loader_.GetFirmwareFileCount() << " files, but completed "
                   << files_completed;
        return false;
    }
