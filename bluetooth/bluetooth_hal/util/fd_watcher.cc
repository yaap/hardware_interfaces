...
FdWatcherImpl::~FdWatcherImpl() {
    StopThread();
}

int FdWatcherImpl::StartWatching(int fd,
...
void FdWatcherImpl::StopWatching() {
    StopThread();
}
...
void FdWatcherImpl::NotifyThread() const {
    ...
    if (ret < 0) {
        LOG(ERROR) << __func__ << ": Failed to write to notification pipe: " << strerror(errno)
                   << ".";
    }
}
...
void FdWatcherImpl::ThreadRoutine() {
    ...
    if (sched_setscheduler(0, SCHED_FIFO, &rt_params) != 0) {
        LOG(WARNING) << __func__ << ": Failed to set SCHED_FIFO: " << strerror(errno) << ".";
    }
    ...
    while (running_) {
        ...
        const int num_ready_fds = select(max_fd + 1, &read_fds, nullptr, nullptr, nullptr);
...
int FdWatcher::StartWatching(int fd, const std::function<void(int)>& on_read_fd_ready_callback) {
