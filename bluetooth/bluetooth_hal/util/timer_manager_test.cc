...
class TimerManagerTest : public Test {
  protected:
     ...
     */
    std::pair<std::unique_ptr<std::promise<void>>, std::future<void>> GetPromiseFuturePair() {
...
TEST_F(TimerManagerTest, ScheduleTask) {
    ...
    auto [promise, future] = GetPromiseFuturePair();
    ASSERT_TRUE(
            timer.Schedule([&promise]() { promise->set_value(); }, std::chrono::milliseconds(50)));
    // The task should not be executed yet.
    ASSERT_NE(std::future_status::ready, future.wait_for(std::chrono::milliseconds(10)));
...
TEST_F(TimerManagerTest, ScheduleTaskWithOneSecondDelay) {
    ...
    ASSERT_TRUE(timer.Schedule([&promise]() { promise->set_value(); }, std::chrono::seconds(1)));
    // The task should not be executed before the delay.
    ASSERT_NE(std::future_status::ready, future.wait_for(std::chrono::milliseconds(900)));
    // The task should be executed after 1000ms, set a timeout of 200ms more
    // to account for scheduling delays.
    ASSERT_EQ(std::future_status::ready, future.wait_for(std::chrono::milliseconds(200)));
}
...
TEST_F(TimerManagerTest, ScheduleTaskWithDelayGreaterThanOneSecond) {
    ...
    // The task should not be executed before the delay.
    ASSERT_NE(std::future_status::ready, future.wait_for(std::chrono::milliseconds(1400)));
...
TEST_F(TimerManagerTest, CancelTask) {
    ...
    auto [promise, future] = GetPromiseFuturePair();
    ASSERT_TRUE(
            timer.Schedule([&promise]() { promise->set_value(); }, std::chrono::milliseconds(10)));
    ASSERT_TRUE(timer.Cancel());
    ASSERT_NE(std::future_status::ready, future.wait_for(std::chrono::milliseconds(100)));
}
...
TEST_F(TimerManagerTest, ScheduleMultipleTasks) {
    ...
    ASSERT_NE(std::future_status::ready, future1.wait_for(std::chrono::milliseconds(10)));
    ASSERT_NE(std::future_status::ready, future2.wait_for(std::chrono::milliseconds(10)));
    ASSERT_EQ(std::future_status::ready, future1.wait_for(std::chrono::milliseconds(100)));
    ASSERT_EQ(std::future_status::ready, future2.wait_for(std::chrono::milliseconds(150)));
}
...
TEST_F(TimerManagerTest, SecondTimerFiresFirst) {
    ...
    ASSERT_NE(std::future_status::ready, future2.wait_for(std::chrono::milliseconds(10)));
    ASSERT_NE(std::future_status::ready, future1.wait_for(std::chrono::milliseconds(10)));
    ASSERT_EQ(std::future_status::ready, future2.wait_for(std::chrono::milliseconds(100)));
    ASSERT_EQ(std::future_status::ready, future1.wait_for(std::chrono::milliseconds(150)));
}
...
TEST_F(TimerManagerTest, ScheduleMultipleTasksWithCancel) {
    ...
    ASSERT_NE(std::future_status::ready, future2.wait_for(std::chrono::milliseconds(10)));
    ASSERT_NE(std::future_status::ready, future1.wait_for(std::chrono::milliseconds(100)));
    ASSERT_EQ(std::future_status::ready, future2.wait_for(std::chrono::milliseconds(100)));
}
...
TEST_F(TimerManagerTest, CancelTaskWhichSchedulesAnotherTask) {
    ...
    ASSERT_TRUE(timer.Cancel());
    ASSERT_NE(std::future_status::ready, future1.wait_for(std::chrono::milliseconds(100)));
    ASSERT_NE(std::future_status::ready, future2.wait_for(std::chrono::milliseconds(100)));
}
