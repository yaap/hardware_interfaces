...
class WatchdogBiteHandler {
  public:
    ...
    // individually.
    void TxTimeout() { LOG(FATAL) << __func__ << ": wakelock watchdog BITE due to TX timeout!"; }

    void RxTimeout() {
        LOG(FATAL) << __func__ << ": wakelock watchdog BITE due to unable to complete RX!";
    }

    void HciTimeout() { LOG(FATAL) << __func__ << ": wakelock watchdog BITE due to HCI timeout!"; }

    void RouterTaskTimeout() {
        LOG(FATAL) << __func__ << ": wakelock watchdog BITE due to Router Task timeout!";
    }

    void InitializeTimeout() {
        LOG(FATAL) << __func__ << ": wakelock watchdog BITE due to initialize timeout!";
    }
...
void WakelockWatchdogImpl::Start(WakeSource source) {
    ...
    if (!watchdog_timer_.IsScheduled()) {
        watchdog_timer_.Schedule(std::bind_front(&WakelockWatchdogImpl::WatchdogTimerExpired, this),
...
void WakelockWatchdogImpl::WatchdogTimerExpired() {
    ...
    for (auto& it : watchdog_map_) {
        ...
        if (source != WakeSource::kTransport && remain_time <= (kWatchdogMs.at(source)) / 2) {
...
void WakelockWatchdogImpl::Bark(WakeSource source, int remain_time) {
    ANCHOR_LOG_WARNING(AnchorType::kWatchdog)
            << ": Watchdog BARK! WakeSource = " << WakelockUtil::WakeSourceToString(source)
