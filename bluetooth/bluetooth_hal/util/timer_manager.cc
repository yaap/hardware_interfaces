...
constexpr std::chrono::milliseconds kTearDownTimer = std::chrono::milliseconds(10);
...
TimerManagerImpl::TimerManagerImpl() {
    ...
    timer_thread_ = std::make_unique<Worker<TimerMessage>>([this](TimerMessage message) {
    ...
    task_thread_ = std::make_unique<Worker<TaskMessage>>([this](TaskMessage message) {
...
bool TimerManagerImpl::Schedule(Timer* timer, const std::function<void()>& task,
    ...
    std::lock_guard<std::mutex> lock(mutex_);
    TimerEvent timer_event{timer, std::chrono::steady_clock::now() + delay, std::move(task)};
...
bool TimerManagerImpl::SetTimer(std::chrono::milliseconds delay) {
    ...
    std::chrono::nanoseconds nanoseconds = duration_cast<std::chrono::nanoseconds>(delay - seconds);
    itimerspec timer_spec{.it_value = {.tv_sec = static_cast<time_t>(seconds.count()),
