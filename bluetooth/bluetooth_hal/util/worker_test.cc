...
class WorkerTest : public Test {
  protected:
     ...
     */
    template <typename T>
    std::pair<std::unique_ptr<std::promise<T>>, std::future<T>> GetPromiseFuturePair() {
...
TEST_F(WorkerTest, QueueFull) {
    ...
    auto post_thread = std::thread([&]() { worker.Post([&promise]() { promise->set_value(); }); });
...
TEST_F(WorkerTest, GetQueuedMessageSize) {
    Worker<int> worker([](int /*task*/) { std::this_thread::sleep_for(kSleepForTask); },
                       kMaxQueueSizeFive);
...
TEST_F(WorkerTest, DestructorDiscardsPendingTasks) {
    auto [promise, future] = GetPromiseFuturePair<void>();
    {
        ...
        worker.Post([&promise]() { promise->set_value(); });  // This will be in queue
...
TEST_F(WorkerTest, StopUnblocksPost) {
    ...
    ASSERT_EQ(std::future_status::ready, post_result_future.wait_for(kFutureWaitTimeout));
