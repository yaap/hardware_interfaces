...
class SystemCallWrapperImpl : public SystemCallWrapper {
  public:
    ...
    ssize_t Read(int fd, void* buffer, size_t count) override { return read(fd, buffer, count); }

    int Accept(int fd, struct sockaddr* address, socklen_t* address_len) override {
    ...
    int Bind(int fd, const struct sockaddr* address, socklen_t address_len) override {
        return bind(fd, address, address_len);
    }

    int Listen(int fd, int backlog) override { return listen(fd, backlog); }

    int Stat(const char* path, struct stat* sb) override { return stat(path, sb); }

    bool IsSocketFile(int st_mode) override { return S_ISSOCK(st_mode); }

    int CreatePipe(int pipefd[2], int flags) override { return pipe2(pipefd, flags); }
