...
constexpr size_t kLinkQualityEventV5MinSize = static_cast<size_t>(LinkQualityOffsetV5::kEnd);
...
BqrLinkQualityEventV5::BqrLinkQualityEventV5(const HalPacket& packet)
    ...
    is_valid_ = BqrLinkQualityEventV1ToV3::IsValid() && size() >= kLinkQualityEventV5MinSize;
    ParseData();
}
...
void BqrLinkQualityEventV5::ParseData() {
    if (is_valid_) {
        ...
        std::reverse_copy(begin() + static_cast<uint8_t>(LinkQualityOffsetV5::kRemoteAddr),
        ...
        v5_tx_total_packets_ = AtUint32LittleEndian(LinkQualityOffsetV5::kTxTotalPackets);
        v5_tx_unacked_packets_ = AtUint32LittleEndian(LinkQualityOffsetV5::kTxUnackedPackets);
        v5_tx_flushed_packets_ = AtUint32LittleEndian(LinkQualityOffsetV5::kTxFlushedPackets);
        v5_tx_last_subevent_packets_ =
                AtUint32LittleEndian(LinkQualityOffsetV5::kTxLastSubeventPackets);
        v5_crc_error_packets_ = AtUint32LittleEndian(LinkQualityOffsetV5::kCrcErrorPackets);
        v5_rx_duplicate_packets_ = AtUint32LittleEndian(LinkQualityOffsetV5::kRxDuplicatePackets);
    }
}

bool BqrLinkQualityEventV5::IsValid() const {
    return is_valid_;
}
...
std::string BqrLinkQualityEventV5::ToBqrString() const {
    std::stringstream ss;
    ss << BqrLinkQualityEventV1ToV3::ToBqrString() << ", Address: " << remote_addr_.ToString()
       << ", FailedCount: " << std::dec << static_cast<int>(call_failed_item_count_)
       << ", TxTotal: " << std::dec << v5_tx_total_packets_ << ", TxUnAcked: " << std::dec
       << v5_tx_unacked_packets_ << ", TxFlushed: " << std::dec << v5_tx_flushed_packets_
       << ", TxLastSubEvent: " << std::dec << v5_tx_last_subevent_packets_
       << ", CRCError: " << std::dec << v5_crc_error_packets_ << ", RxDuplicate: " << std::dec
