...
constexpr size_t kAdvanceRfStatsEventV7MinSize = static_cast<size_t>(AdvanceRfStatsOffsetV7::kEnd);

}  // namespace

BqrAdvanceRfStatsEventV7::BqrAdvanceRfStatsEventV7(const HalPacket& packet)
    : BqrAdvanceRfStatsEvent(packet) {
    is_valid_ = BqrAdvanceRfStatsEvent::IsValid() && size() >= kAdvanceRfStatsEventV7MinSize;
    ParseData();
}

void BqrAdvanceRfStatsEventV7::ParseData() {
    if (is_valid_) {
        ant_switch_count_ = AtUint32LittleEndian(AdvanceRfStatsOffsetV7::kAntSwitchCount);
        ...
        tx_buf_queue_count_ = AtUint32LittleEndian(AdvanceRfStatsOffsetV7::kTxBufQueueCount);
    }
}

bool BqrAdvanceRfStatsEventV7::IsValid() const {
    return is_valid_;
}
...
uint8_t BqrAdvanceRfStatsEventV7::GetChCountOk() const {
    return ch_count_ok_;
}
...
std::string BqrAdvanceRfStatsEventV7::ToBqrString() const {
    std::stringstream ss;
    ss << BqrAdvanceRfStatsEvent::ToBqrString() << ", Ant_Switch_Count: " << std::dec
       << ant_switch_count_ << ", Re_Tx_iPA_BF: " << std::dec << re_tx_ipa_bf_
       << ", Re_Tx_ePA_BF: " << std::dec << re_tx_epa_bf_ << ", Re_Tx_iPA_Div: " << std::dec
       << re_tx_ipa_div_ << ", Re_Tx_ePA_Div: " << std::dec << re_tx_epa_div_
