...
constexpr size_t kAdvanceRfStatsEventMinSize = static_cast<size_t>(AdvanceRfStatsOffset::kEnd);

}  // namespace

BqrAdvanceRfStatsEvent::BqrAdvanceRfStatsEvent(const HalPacket& packet)
    : BqrEvent(packet),
      is_valid_(BqrEvent::IsValid() && GetBqrEventType() == BqrEventType::kAdvancedRfStat &&
...
void BqrAdvanceRfStatsEvent::ParseData() {
    if (is_valid_) {
        ...
        rssi_delta_2_down_ = AtUint32LittleEndian(AdvanceRfStatsOffset::kRssiDelta2Down);
        ...
        rssi_delta_8_11_ = AtUint32LittleEndian(AdvanceRfStatsOffset::kRssiDelta8_11);
        rssi_delta_11_up_ = AtUint32LittleEndian(AdvanceRfStatsOffset::kRssiDelta11Up);
    }
}
bool BqrAdvanceRfStatsEvent::IsValid() const {
    return is_valid_;
}

uint8_t BqrAdvanceRfStatsEvent::GetExtInfo() const {
    return ext_info_;
}

uint32_t BqrAdvanceRfStatsEvent::GetTmPeriod() const {
    return tm_period_;
}

uint32_t BqrAdvanceRfStatsEvent::GetTxPwIpaBf() const {
    return tx_pw_ipa_bf_;
}

uint32_t BqrAdvanceRfStatsEvent::GetTxPwEpaBf() const {
    return tx_pw_epa_bf_;
}
...
uint32_t BqrAdvanceRfStatsEvent::GetRssiCh50() const {
    return rssi_ch_50_;
}
...
uint32_t BqrAdvanceRfStatsEvent::GetRssiCh90() const {
    return rssi_ch_90_;
}
...
std::string BqrAdvanceRfStatsEvent::ToBqrString() const {
    ...
    ss << "Time Period:" << std::dec << tm_period_ << " ms"
       ...
       << ", TW_Pw_iPA_BF: " << std::dec << tx_pw_ipa_bf_ << ", TW_Pw_ePA_BF: " << std::dec
       << tx_pw_epa_bf_ << ", TW_Pw_iPA_Div: " << std::dec << tx_pw_ipa_div_
       << ", TW_Pw_ePA_Div: " << std::dec << tx_pw_epa_div_ << ", RSSI_Chain_>-50: " << std::dec
       << rssi_ch_50_ << ", RSSI_Chain_-50_-55: " << std::dec << rssi_ch_50_55_
       ...
       << ", RSSI_Chain_<-90: " << std::dec << rssi_ch_90_ << ", RSSI_Delta_<2: " << std::dec
       << rssi_delta_2_down_ << ", RSSI_Delta_2_5: " << std::dec << rssi_delta_2_5_
       << ", RSSI_Delta_5_8: " << std::dec << rssi_delta_5_8_ << ", RSSI_Delta_8_11: " << std::dec
       << rssi_delta_8_11_ << ", RSSI_Delta_>11: " << std::dec << rssi_delta_11_up_;
    return ss.str();
}
