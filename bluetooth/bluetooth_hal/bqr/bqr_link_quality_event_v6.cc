...
constexpr size_t kLinkQualityEventV6MinSize = static_cast<size_t>(LinkQualityOffsetV6::kEnd);

}  // namespace

BqrLinkQualityEventV6::BqrLinkQualityEventV6(const HalPacket& packet)
    : BqrLinkQualityEventV5(packet), rx_unreceived_packets_(0), coex_info_mask_(0) {
    is_valid_ = BqrLinkQualityEventV5::IsValid() && size() >= kLinkQualityEventV6MinSize;
    ParseData();
}
...
void BqrLinkQualityEventV6::ParseData() {
    if (is_valid_) {
        ...
        rx_unreceived_packets_ = AtUint32LittleEndian(LinkQualityOffsetV6::kRxUnreceivedPackets);
        coex_info_mask_ = AtUint16LittleEndian(LinkQualityOffsetV6::kCoexInfoMask);
    }
}

bool BqrLinkQualityEventV6::IsValid() const {
    return is_valid_;
}
...
std::string BqrLinkQualityEventV6::ToBqrString() const {
    ...
    ss << BqrLinkQualityEventV5::ToBqrString() << ", RxUnreceived: " << std::dec
       << rx_unreceived_packets_ << ", CoexInfoMask: 0x" << std::hex << std::setw(4)
       << std::setfill('0') << coex_info_mask_;
    return ss.str();
}
