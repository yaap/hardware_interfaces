...
constexpr size_t kRootInflammationEventMinSize = static_cast<size_t>(RootInflammationOffset::kEnd);

}  // namespace

BqrRootInflammationEvent::BqrRootInflammationEvent(const HalPacket& packet)
    : BqrEvent(packet),
      is_valid_(BqrEvent::IsValid() && GetBqrEventType() == BqrEventType::kRootInflammation &&
...
bool BqrRootInflammationEvent::IsValid() const {
    return is_valid_;
}

uint8_t BqrRootInflammationEvent::GetErrorCode() const {
    return error_code_;
}
...
std::vector<uint8_t> BqrRootInflammationEvent::GetVendorParameter() const {
    ...
    if (size() > kRootInflammationEventMinSize) {
        vendor_parameter.assign(
                begin() + static_cast<size_t>(RootInflammationOffset::kVendorParameter), end());
    }
...
std::string BqrRootInflammationEvent::ToBqrString() const {
    ...
    ss << BqrEvent::ToBqrString() << ", Error Code:0x" << std::hex << std::setw(2)
       << std::setfill('0') << static_cast<int>(error_code_) << ", Vendor Error Code:0x" << std::hex
       << std::setw(2) << std::setfill('0') << static_cast<int>(vendor_error_code_)
