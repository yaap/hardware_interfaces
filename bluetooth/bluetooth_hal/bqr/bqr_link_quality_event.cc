...
constexpr size_t kLinkQualityEventMinSize = static_cast<size_t>(LinkQualityOffset::kEnd);

}  // namespace

BqrLinkQualityEventBase::BqrLinkQualityEventBase(const HalPacket& packet)
    : BqrEvent(packet),
      is_valid_(BqrEvent::IsValid() && GetBqrEventType() == BqrEventType::kLinkQuality &&
...
void BqrLinkQualityEventBase::ParseData() {
    if (is_valid_) {
        ...
        connection_handle_ = AtUint16LittleEndian(LinkQualityOffset::kConnectionHandle);
        ...
        afh_select_unideal_channel_count_ = At(LinkQualityOffset::kAfhSelectUnidealChannelCount);
        ...
        retransmission_count_ = AtUint32LittleEndian(LinkQualityOffset::kRetransmissionCount);
        ...
        last_tx_ack_timestamp_ = AtUint32LittleEndian(LinkQualityOffset::kLastTxAckTimestamp);
        flow_off_count_ = AtUint32LittleEndian(LinkQualityOffset::kFlowOffCount);
        last_flow_on_timestamp_ = AtUint32LittleEndian(LinkQualityOffset::kLastFlowOnTimestamp);
        buffer_overflow_bytes_ = AtUint32LittleEndian(LinkQualityOffset::kBufferOverflowBytes);
        buffer_underflow_bytes_ = AtUint32LittleEndian(LinkQualityOffset::kBufferUnderflowBytes);
    }
}
bool BqrLinkQualityEventBase::IsValid() const {
    return is_valid_;
}
...
int8_t BqrLinkQualityEventBase::GetRssi() const {
    return rssi_;
}

uint8_t BqrLinkQualityEventBase::GetSnr() const {
    return snr_;
}
...
uint16_t BqrLinkQualityEventBase::GetLsto() const {
    return lsto_;
}
...
uint32_t BqrLinkQualityEventBase::GetNoRxCount() const {
    return no_rx_count_;
}

uint32_t BqrLinkQualityEventBase::GetNakCount() const {
    return nak_count_;
}
...
BqrVersion BqrLinkQualityEventBase::GetVersion() const {
    return version_;
}
...
std::string BqrLinkQualityEventBase::ToBqrString() const {
    std::stringstream ss;
    ss << BqrEvent::ToBqrString() << ", Handle: 0x" << std::hex << std::setw(4) << std::setfill('0')
       << connection_handle_ << ", " << BqrPacketTypeToString(packet_types_)
       << (connection_role_ ? ", Peripheral" : ", Central") << ", PwLv: " << std::dec
       << static_cast<int>(tx_power_level_) << ", RSSI: " << std::dec << static_cast<int>(rssi_)
       << ", SNR: " << std::dec << static_cast<int>(snr_) << ", UnusedCh: " << std::dec
       << static_cast<int>(unused_afh_channel_count_) << ", UnidealCh: " << std::dec
       << static_cast<int>(afh_select_unideal_channel_count_) << ", LSTO: " << std::dec << lsto_
       << ", Connection Piconet Clock: " << std::dec << connection_piconet_clock_
       << ", ReTx: " << std::dec << retransmission_count_ << ", NoRx: " << std::dec << no_rx_count_
       << ", NAK: " << std::dec << nak_count_ << ", LastTX: " << std::dec << last_tx_ack_timestamp_
       << ", FlowOff: " << std::dec << flow_off_count_ << ", LastFlowOn: " << std::dec
       << last_flow_on_timestamp_ << ", Overflow: " << std::dec << buffer_overflow_bytes_
