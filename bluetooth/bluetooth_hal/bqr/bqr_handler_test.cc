...
HalPacket CreateVendorCapabilityEvent(BqrVersion version) {
    ...
    return HalPacket({0x04, 0x0e, 0x16, 0x01, 0x53, 0xfd, 0x00,         0x10, 0x01,
                      0x00, 0x04, 0x00, 0x01, 0x40, 0x01, version_byte, 0x01, 0x14,
                      0x00, 0x01, 0x01, 0x07, 0x00, 0x00, 0x00});
}
...
class TestBqrHandler : public BqrHandler {
  public:
    MOCK_METHOD(void, HandleRootInflammationEvent, (const BqrEvent& event), (override));
    MOCK_METHOD(void, HandleLinkQualityEvent, (const BqrEvent& bqr_event), (override));
    MOCK_METHOD(void, HandleAdvancedRfStatEvent, (const BqrEvent& bqr_event), (override));
    MOCK_METHOD(void, HandleEnergyMonitoringEvent, (const BqrEvent& bqr_event), (override));
    MOCK_METHOD(void, HandleUnspecifiedVendorEvent, (const BqrEvent& bqr_event), (override));

    BqrVersion GetLocalSupportedBqrVersion() { return BqrHandler::GetLocalSupportedBqrVersion(); }
};
...
TEST_P(BqrHandlerTest, HandleVendorCapabilityEvent) {
    ...
    test_bqr_handler_callack_->OnPacketCallback(CreateVendorCapabilityEvent(version));
    EXPECT_EQ(test_bqr_handler_->GetLocalSupportedBqrVersion(), version);
}

INSTANTIATE_TEST_SUITE_P(BqrHandlerTest, BqrHandlerTest,
                         Values(BqrVersion::kV1ToV3, BqrVersion::kV4, BqrVersion::kV5,
                                BqrVersion::kV6, BqrVersion::kV7));
...
TEST_F(BqrHandlerTest, HandleRootInflammationEvent) {
    ...
    test_bqr_handler_callack_->OnPacketCallback(CreateVendorCapabilityEvent(BqrVersion::kV7));
    test_bqr_handler_callack_->OnPacketCallback(event);
}
