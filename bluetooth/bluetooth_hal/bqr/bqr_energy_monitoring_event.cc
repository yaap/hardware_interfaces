...
constexpr size_t kEnergyMonitoringEventMinSize = static_cast<size_t>(EnergyMonitoringOffset::kEnd);

}  // namespace

BqrEnergyMonitoringEvent::BqrEnergyMonitoringEvent(const HalPacket& packet)
    : BqrEvent(packet),
      is_valid_(BqrEvent::IsValid() && GetBqrEventType() == BqrEventType::kEnergyMonitoring &&
...
void BqrEnergyMonitoringEvent::ParseData() {
    if (is_valid_) {
        average_current_consumption_ =
                AtUint16LittleEndian(EnergyMonitoringOffset::kAverageCurrentConsumption);
        idle_total_time_ = AtUint32LittleEndian(EnergyMonitoringOffset::kIdleTotalTime);
        idle_state_enter_count_ =
                AtUint32LittleEndian(EnergyMonitoringOffset::kIdleStateEnterCount);
        active_total_time_ = AtUint32LittleEndian(EnergyMonitoringOffset::kActiveTotalTime);
        active_state_enter_count_ =
                AtUint32LittleEndian(EnergyMonitoringOffset::kActiveStateEnterCount);
        br_edr_tx_total_time_ = AtUint32LittleEndian(EnergyMonitoringOffset::kBrEdrTxTotalTime);
        br_edr_tx_state_enter_count_ =
                AtUint32LittleEndian(EnergyMonitoringOffset::kBrEdrTxStateEnterCount);
        br_edr_tx_average_power_level_ =
                static_cast<int8_t>(At(EnergyMonitoringOffset::kBrEdrTxAveragePowerLevel));
        br_edr_rx_total_time_ = AtUint32LittleEndian(EnergyMonitoringOffset::kBrEdrRxTotalTime);
        br_edr_rx_state_enter_count_ =
                AtUint32LittleEndian(EnergyMonitoringOffset::kBrEdrRxStateEnterCount);
        le_tx_total_time_ = AtUint32LittleEndian(EnergyMonitoringOffset::kLeTxTotalTime);
        ...
        le_rx_total_time_ = AtUint32LittleEndian(EnergyMonitoringOffset::kLeRxTotalTime);
        le_rx_state_enter_count_ =
                AtUint32LittleEndian(EnergyMonitoringOffset::kLeRxStateEnterCount);
    }
}

bool BqrEnergyMonitoringEvent::IsValid() const {
    return is_valid_;
}
...
std::string BqrEnergyMonitoringEvent::ToBqrString() const {
    ...
    ss << "Avg_Cur_Pwr: " << std::dec << average_current_consumption_
       ...
       << " dBm, Idle_Tm: " << idle_total_time_ << " ms, Act_Tm: " << active_total_time_
       << " ms, BEr_Tx_Tm: " << br_edr_tx_total_time_ << " ms, BEr_Rx_Tm: " << br_edr_rx_total_time_
       << " ms, Le_Tx_Tm: " << le_tx_total_time_ << " ms, Le_Rx_Tm: " << le_rx_total_time_ << " ms";
    return ss.str();
}
