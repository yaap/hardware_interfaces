...
class HciRouterClientTestInstance : public HciRouterClient {
  public:
    ...
    void OnMonitorPacketCallback(MonitorMode mode, const HalPacket& packet) override {
        on_monitor_callbacks_.push_back(std::pair(mode, packet));
    }

    // Wrappers to access protected methods
    bool IsBluetoothChipReady() { return HciRouterClient::IsBluetoothChipReady(); }
    ...
    bool SendCommand(const HalPacket& packet) { return HciRouterClient::SendCommand(packet); }

    bool SendData(const HalPacket& packet) { return HciRouterClient::SendData(packet); }
...
class HciRouterClientTest : public Test {
  protected:
    ...
    void SetUp() override {
        ...
        ON_CALL(mock_hci_router_client_agent_, RegisterClient(_)).WillByDefault(Return(true));
        ON_CALL(mock_hci_router_client_agent_, UnregisterClient(_)).WillByDefault(Return(true));
    ...
    void TearDown() override {
        EXPECT_CALL(mock_hci_router_client_agent_, UnregisterClient(router_client_)).Times(1);
    ...
    HalPacket GenerateHciResetCompleteEvent() {
        return HalPacket(std::vector<uint8_t>{0x04, 0x0E, 0x04, 0x01, 0x03, 0x0C, 0x00});
    }

    HalPacket GenerateBleAdvReportEvent() {
        return HalPacket(std::vector<uint8_t>{0x04, 0x3E, 0x1D, 0x0D, 0x01, 0x12, 0x00, 0x01,
                                              0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00,
                                              0xFF, 0x7F, 0xC0, 0x00, 0x00, 0x00, 0x00, 0x00,
                                              0x00, 0x00, 0x00, 0x00, 0x03, 0x02, 0x01, 0x02});
    }

    HalPacket GenerateRandomPacket() {
        return HalPacket(std::vector<uint8_t>{0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09,
                                              0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F});
    }

    void TestHandleRegisterMonitor(HciMonitor monitor, MonitorMode mode, HalPacket packet,
        ...
        ASSERT_EQ(router_client_->OnPacketCallback(packet_random), MonitorMode::kNone);
        ASSERT_EQ(router_client_->on_monitor_callbacks_.size(), expect_call_count);
...
TEST_F(HciRouterClientTest, HandleCreate) {
    ...
    EXPECT_CALL(mock_hci_router_client_agent_, UnregisterClient(instance1.get())).Times(1);
    ...
    EXPECT_CALL(mock_hci_router_client_agent_, UnregisterClient(instance2.get())).Times(1);
    ...
    EXPECT_CALL(mock_hci_router_client_agent_, UnregisterClient(instance3.get())).Times(1);
    ...
    EXPECT_CALL(mock_hci_router_client_agent_, UnregisterClient(instance4.get())).Times(1);
...
TEST_F(HciRouterClientTest, HandleSendCommand) {
    ...
    EXPECT_CALL(mock_hci_router_, SendCommand(packet, _))
            .WillOnce(Invoke(
                    [&](const HalPacket& captured_packet, const HalPacketCallback& /* callback */) {
...
TEST_F(HciRouterClientTest, HandleRegisterMonitorEventMonitorWithEventCode) {
    ...
    HciEventMonitor monitor(static_cast<uint8_t>(EventCode::kBleMeta), kHciBleAdvSubCode,
...
TEST_F(HciRouterClientTest, HandleRegisterMonitorHasOverlapDifferentMode) {
    ...
    ASSERT_TRUE(router_client_->RegisterMonitor(monitor2, MonitorMode::kIntercept));
...
TEST_F(HciRouterClientTest, HandleRegisterMonitorDoubleRegister) {
    ...
    HciCommandCompleteEventMonitor monitor2(kHciResetCommandOpcode);
    ASSERT_TRUE(router_client_->RegisterMonitor(monitor1, MonitorMode::kMonitor));
    ASSERT_FALSE(router_client_->RegisterMonitor(monitor2, MonitorMode::kMonitor));
}
