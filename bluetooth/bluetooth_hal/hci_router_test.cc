...
class FakeHciRouterCallback : public HciRouterCallback {
  public:
    ...
    void OnCommandCallback(const HalPacket& packet) override { OnPacketCallback(packet); };
    MOCK_METHOD(MonitorMode, OnPacketCallback, (const HalPacket& packet), (override));
    MOCK_METHOD(void, OnHalStateChanged, (const HalState new_state, const HalState old_state),
                (override));
};
  ...
  protected:
    ...
    void SetUp() override {
        ...
        ON_CALL(mock_transport_factory_, CleanupTransport())
                .WillByDefault(Invoke([&mock_transport_instance = mock_transport_instance_]() {
                    mock_transport_instance.Cleanup();
                }));

        ON_CALL(mock_transport_instance_, IsTransportActive()).WillByDefault(Return(true));
        ON_CALL(mock_transport_instance_, Send(_))
                .WillByDefault(Invoke(this, &HciRouterTest::OnSendToTransport));
        ON_CALL(mock_transport_instance_, Initialize(_))
                .WillByDefault(
                        Invoke([this](TransportInstanceCallback* transport_instance_callback) {
                            this->transport_instance_callback_ = transport_instance_callback;
                            return true;
                        }));
        ON_CALL(mock_transport_instance_, SetHciRouterBusy(_))
                .WillByDefault(Invoke(this, &HciRouterTest::OnSetHciRouterBusyInTransport));
        ON_CALL(mock_hal_config_loader_, IsAcceleratedBtOnSupported()).WillByDefault(Return(false));
        ...
        ON_CALL(*fake_hci_callback_, OnPacketCallback(_))
                .WillByDefault(DoAll(SaveArg<0>(&hal_packet_), Return(MonitorMode::kNone)));
    ...
    void CompleteFirmwareDownloadAndStackInit() {
        ...
        EXPECT_CALL(*fake_hci_callback_, OnHalStateChanged(_, _))
                .Times(2)
                .WillRepeatedly(
                        Invoke([&](HalState new_state, [[maybe_unused]] HalState old_state) {
    ...
    bool SendCommandToRouter(const HalPacket& packet, const HalPacketCallback& callback) {
        std::promise<void> promise;
        std::future<void> future = promise.get_future();
        command_sent_promises_[packet] = std::move(promise);
        command_sent_futures_[packet] = std::move(future);
    ...
    void ExpectHalStateChange(HalState new_state, HalState old_state, int count = 1) {
        EXPECT_CALL(*fake_hci_callback_, OnHalStateChanged(new_state, old_state)).Times(count);
        EXPECT_CALL(mock_hci_router_client_agent_, NotifyHalStateChange(new_state, old_state))
                .Times(count);
        EXPECT_CALL(mock_transport_factory_, NotifyHalStateChange(new_state)).Times(count);
    }
...
TEST_F(HciRouterTest, InitializeWithAcceleratedBtOn) {
    ...
    ON_CALL(mock_hal_config_loader_, IsAcceleratedBtOnSupported()).WillByDefault(Return(true));
    ...
    transport_instance_callback_->OnTransportPacketReady(evt_reset);
...
TEST_F(HciRouterTest, HandleSendAclData) {
    ...
    EXPECT_CALL(mock_hci_router_client_agent_, DispatchPacketToClients(acl_data))
            .WillOnce(Invoke([&](const HalPacket& captured_packet) {
                EXPECT_EQ(captured_packet.GetDestination(), PacketDestination::kController);
...
TEST_F(HciRouterTest, HandleSendHciCommandTwiceWithoutEventWithoutFlag) {
    ...
    EXPECT_CALL(mock_hci_router_client_agent_, DispatchPacketToClients(cmd_set_host_le_support))
            .Times(0);
...
TEST_F(HciRouterTest, HandleSendHciCommandTwiceWithLateEvent) {
    ...
    transport_instance_callback_->OnTransportPacketReady(evt_set_host_le_support);
...
TEST_F(HciRouterTest, HandleSendCommandTwiceWithoutEvent) {
    ...
    WaitPacketSentToTransport(cmd_reset);
    EXPECT_TRUE(GetIsRouterBusy());
    EXPECT_TRUE(SendCommandToRouter(cmd_set_host_le_support, EmptyHalPacketCallback));
    EXPECT_TRUE(GetIsRouterBusy());
}
...
TEST_F(HciRouterTest, HandleSendCommandTwiceWithEvent) {
    ...
    HalPacket event;
    EXPECT_TRUE(
            SendCommandToRouter(cmd_reset, [&event](const HalPacket& packet) { event = packet; }));
    WaitPacketSentToTransport(cmd_reset);
    EXPECT_TRUE(GetIsRouterBusy());
...
TEST_F(HciRouterTest, HandleSendHciCommandInCallback) {
    ...
    EXPECT_TRUE(SendCommandToRouter(
            cmd_reset, [&event, &cmd_set_host_le_support, this](const HalPacket& packet) {
                event = packet;
                EXPECT_TRUE(SendCommandToRouter(cmd_set_host_le_support, EmptyHalPacketCallback));
            }));
    WaitPacketSentToTransport(cmd_reset);
...
TEST_F(HciRouterTest, HandleSendHciCommandInCallbackAfterAnotherSendCommand) {
    ...
    HalPacket evt_set_min_enc_key_size({0x04, 0x0e, 0x04, 0x01, 0x84, 0x0c, 0x00});
    HalPacket cmd_set_host_le_support({0x01, 0x6d, 0x0c, 0x02, 0x01, 0x00});
    cmd_set_host_le_support.SetSource(PacketSource::kStack);
    HalPacket evt_set_host_le_support({0x04, 0x0e, 0x04, 0x01, 0x6d, 0x0c, 0x00});
    ...
    // transport layer, and no callback to the stack.
    EXPECT_CALL(mock_transport_instance_, Send(cmd_reset)).Times(1);
    EXPECT_CALL(mock_transport_instance_, Send(cmd_set_min_enc_key_size)).Times(1);
    EXPECT_CALL(mock_transport_instance_, Send(cmd_set_host_le_support)).Times(1);
    EXPECT_CALL(*fake_hci_callback_, OnPacketCallback(_)).Times(0);
    ...
    WaitPacketSentToTransport(cmd_reset);
    EXPECT_TRUE(GetIsRouterBusy());
    EXPECT_TRUE(SendCommandToRouter(cmd_set_min_enc_key_size,
    ...
    transport_instance_callback_->OnTransportPacketReady(evt_set_min_enc_key_size);
...
TEST_F(HciRouterTest, HandleDispatchPacketToClientsMonitorNone) {
    ...
    transport_instance_callback_->OnTransportPacketReady(event);
...
TEST_F(HciRouterTest, HandleDispatchPacketToClientsInterceptThreadData) {
    ...
    EXPECT_CALL(mock_hci_router_client_agent_, DispatchPacketToClients(thread_data)).Times(1);
    EXPECT_CALL(*fake_hci_callback_, OnPacketCallback(_)).Times(0);

    transport_instance_callback_->OnTransportPacketReady(thread_data);
}
...
TEST_F(HciRouterTest, HandleUpdateHalState) {
    ...
    ExpectHalStateChange(HalState::kFirmwareDownloading, HalState::kPreFirmwareDownload);
    ExpectHalStateChange(HalState::kFirmwareDownloadCompleted, HalState::kFirmwareDownloading);
    ExpectHalStateChange(HalState::kFirmwareReady, HalState::kFirmwareDownloadCompleted);
...
TEST_F(HciRouterTest, HandleCleanupAndRxAtTheSameTime) {
    ...
    ON_CALL(mock_transport_factory_, CleanupTransport())
            .WillByDefault(Invoke([&m, &cv, &rx_dispatched]() {
                ...
                bool result =
                        cv.wait_for(lock, std::chrono::seconds(3), [&] { return rx_dispatched; });
                EXPECT_TRUE(rx_dispatched) << "Timeout: Main thread never signaled rx_dispatched";
            }));
    ...
    ON_CALL(mock_transport_factory_, CleanupTransport()).WillByDefault(Invoke([]() {}));
}
...
TEST_F(HciRouterTest, HandleReplaceHciEventByClient) {
    ...
    HalPacket event_before_intercept({0x04, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07});
    HalPacket event_after_intercept({0x04, 0x07, 0x06, 0x05, 0x04, 0x03, 0x02, 0x01});

    // Check if the received event is replaced by a client.
    EXPECT_CALL(mock_hci_router_client_agent_, DispatchPacketToClients(event_before_intercept))
            .WillOnce(Invoke([&](const HalPacket& captured_event) {
                EXPECT_EQ(captured_event.GetDestination(), PacketDestination::kHost);
    ...
    EXPECT_CALL(mock_hci_router_client_agent_, DispatchPacketToClients(event_after_intercept))
            .WillOnce(Invoke([&](const HalPacket&) { return MonitorMode::kBypass; }));

    // Expecting the stack to get the new event.
    EXPECT_CALL(*fake_hci_callback_, OnPacketCallback(event_after_intercept)).Times(1);

    transport_instance_callback_->OnTransportPacketReady(event_before_intercept);
...
TEST_F(HciRouterTest, HandleReplaceHciCommandByClient) {
    ...
    EXPECT_CALL(mock_hci_router_client_agent_, DispatchPacketToClients(cmd_before_intercept))
            .WillOnce(Invoke([&](const HalPacket& captured_packet) {
                EXPECT_EQ(captured_packet.GetDestination(), PacketDestination::kController);
                EXPECT_EQ(captured_packet.GetSource(), PacketSource::kStack);
    ...
    EXPECT_CALL(mock_hci_router_client_agent_, DispatchPacketToClients(cmd_after_intercept))
            .WillOnce(Invoke([&](const HalPacket&) { return MonitorMode::kBypass; }));
