...
static std::unique_ptr<ChannelSoundingAlgorithm> channel_sounding_algorithm = nullptr;
...
ndk::ScopedAStatus BluetoothChannelSoundingSession::isAbortedProcedureRequired(bool* _aidl_return) {
    *_aidl_return = false;
    return ::ndk::ScopedAStatus::ok();
}
ndk::ScopedAStatus BluetoothChannelSoundingSession::writeRawData(
    ...
    ranging_result.resultMeters = channel_sounding_algorithm->EstimateDistance(in_rawData);
    ATRACE_END();
    ranging_result.confidenceLevel = channel_sounding_algorithm->GetConfidenceLevel() * 100;
...
static bool is_valid_antenna_permutation_data(uint8_t permutation_index,
    ...
    uint8_t max_valid_permutation_index = kMaxValidPermutationIndexTable[num_antenna_paths - 1];
 ...
 * ModeOneData.
 */
static void populate_mode_one_data(const ModeOneData& data,
    ...
    if (data.packetPct1.has_value()) {
        single_side_data.packetPct1->push_back(get_complex_number(*data.packetPct1));
    }
    if (data.packetPct2.has_value()) {
        single_side_data.packetPct2->push_back(get_complex_number(*data.packetPct2));
    }

    if (data.rttToaTodData.getTag() == RttToaTodData::Tag::toaTodInitiator) {
        toa_tod_values.push_back(data.rttToaTodData.get<RttToaTodData::Tag::toaTodInitiator>());
    } else {
        toa_tod_values.push_back(data.rttToaTodData.get<RttToaTodData::Tag::todToaReflector>());
    }
}

static void populate_mode_two_data(const ModeTwoData& data, int8_t num_antenna_paths,
                                   ChannelSoundingSingleSideData& single_side_data) {
    ...
    if (!is_valid_antenna_permutation_data(permutation_index, num_antenna_paths)) {
        LOG(WARNING) << __func__
                     << " Invalid antenna permutation data (index: " << (int)permutation_index
    ...
    for (uint16_t k = 0; k < num_tone_data; k++) {
        ...
        uint8_t antenna_path_idx = (k == num_antenna_paths)
                                           ? num_antenna_paths
                                           : kCsAntennaPermutationArray[permutation_index][k] -
                                                     1;  // -1 for 0-based index

        if (antenna_path_idx > num_antenna_paths) {
            LOG(ERROR) << __func__ << " Calculated antenna path " << (int)antenna_path_idx
                       << " is out of bounds for " << (int)num_antenna_paths << " antenna paths.";
            continue;
        }

        // Get the correct StepTonePct object for this antenna path
        auto& target_step_tone_pct = single_side_data.stepTonePcts->at(antenna_path_idx).value();

        // Append the data for this step to the correct antenna's vectors.
        target_step_tone_pct.tonePcts.push_back(get_complex_number(data.tonePctIQSamples[k]));
        if (k < data.toneQualityIndicators.size()) {
            target_step_tone_pct.toneQualityIndicator.push_back(data.toneQualityIndicators[k]);
        }
    }
}

static void populate_single_side_data(const std::vector<SubeventResultData>& subevent_results,
                                      bool is_initiator,
                                      ChannelSoundingSingleSideData& single_side_data,
    ...
    for (const auto& subevent : subevent_results) {
        ...
        if (num_antenna_paths > 0) {
            single_side_data.stepTonePcts->resize(num_antenna_paths + 1);  // one more extent pct
            for (int i = 0; i <= num_antenna_paths; ++i) {
                single_side_data.stepTonePcts->at(i).emplace();  // Create the StepTonePct objects
            }
        }
        for (const auto& step : subevent.stepData) {
            switch (step.stepMode) {
                case ModeType::ZERO: {
                    const auto& data = step.stepModeData.get<ModeData::Tag::modeZeroData>();
                    single_side_data.packetQuality->push_back(data.packetQuality);
                    single_side_data.packetRssiDbm->push_back(data.packetRssiDbm);
                ...
                case ModeType::ONE: {
                    const auto& data = step.stepModeData.get<ModeData::Tag::modeOneData>();
                ...
                case ModeType::TWO: {
                    const auto& data = step.stepModeData.get<ModeData::Tag::modeTwoData>();
                ...
                case ModeType::THREE: {
                    const auto& data = step.stepModeData.get<ModeData::Tag::modeThreeData>();
                    // ModeThree is a combination of ModeOne and ModeTwo.
                    populate_mode_one_data(data.modeOneData, single_side_data, toa_tod_values);

                    populate_mode_two_data(data.modeTwoData, num_antenna_paths, single_side_data);
...
static ChannelSoudingRawData convert_procedure_data_to_Raw(
    ...
    raw_data.aborted =
            (procedure_data.initiatorProcedureAbortReason != ProcedureAbortReason::SUCCESS) ||
            (procedure_data.reflectorProcedureAbortReason != ProcedureAbortReason::SUCCESS);
    ...
    if (!procedure_data.initiatorSubeventResultData.empty()) {
        ...
        raw_data.numAntennaPaths = procedure_data.initiatorSubeventResultData[0].numAntennaPaths;
    }
    ...
    populate_single_side_data(procedure_data.initiatorSubeventResultData,
                              /*is_initiator=*/true, raw_data.initiatorData, raw_data.stepChannels,
                              initiator_toa_tod_values);

    // Process reflector data
    populate_single_side_data(procedure_data.reflectorSubeventResultData,
                              /*is_initiator=*/false, raw_data.reflectorData, raw_data.stepChannels,
                              reflector_toa_tod_values);
