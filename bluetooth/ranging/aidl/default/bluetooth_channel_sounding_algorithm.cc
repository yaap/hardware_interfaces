...
void ChannelSoundingAlgorithm::ParseRawData(const ChannelSoudingRawData& channel_souding_raw_data) {
    ...
    reference_power_level_initiator_ = channel_souding_raw_data.initiatorData.referencePowerDbm;
    reference_power_level_reflector_ = channel_souding_raw_data.reflectorData.referencePowerDbm;
...
double ChannelSoundingAlgorithm::EstimateDistance(
    ...
    if (channel_souding_raw_data.initiatorData.stepTonePcts.value()[0].value().tonePcts.empty()) {
...
void ChannelSoundingAlgorithm::DataCleaning::Run(ChannelSoundingAlgorithm& cs_algo) {
    // fix doppler
...
void ChannelSoundingAlgorithm::DataCleaning::MultiplyPCT(ChannelSoundingAlgorithm& cs_algo) {
...
void ChannelSoundingAlgorithm::DataCleaning::SortPCT(ChannelSoundingAlgorithm& cs_algo) {
    // sort
    std::vector<uint8_t> perm;
    cs_algo.step_channel_cleaned_ = cs_algo.step_channel_;
    std::vector<std::vector<std::complex<double>>> pct_mult = cs_algo.pct_cleaned_;
    perm = ChannelSoundingAlgorithm::DataCleaning::SortPermutation(cs_algo.step_channel_);
    ...
    for (size_t i = 1; i < cs_algo.step_channel_cleaned_.size(); i++) {
        if (cs_algo.step_channel_cleaned_[i] == cs_algo.step_channel_cleaned_[i - 1]) {
    ...
    for (size_t idx = 0; idx < remove_idx.size(); idx++) {
        cs_algo.step_channel_cleaned_.erase(cs_algo.step_channel_cleaned_.begin() +
                                            remove_idx[idx]);
        for (size_t ap = 0; ap < cs_algo.pct_initiator_.size(); ap++) {
            cs_algo.pct_cleaned_[ap].erase(cs_algo.pct_cleaned_[ap].begin() + remove_idx[idx]);
        }
    }
}

void ChannelSoundingAlgorithm::DataCleaning::FixDoppler(ChannelSoundingAlgorithm& cs_algo) {
    std::vector<uint8_t> perm;
    ...
    for (uint8_t ap = 0; ap < cs_algo.n_ap_; ap++) {
        for (uint16_t i = 0; i < cs_algo.step_channel_.size() - 1; i++) {
            theta1 = atan2(cs_algo.pct_cleaned_[ap][i].imag(), cs_algo.pct_cleaned_[ap][i].real());
    ...
    double dopp_mean =
            std::accumulate(doppler_est.begin(), doppler_est.end(), 0.0) / doppler_est.size();
    // fix steps
...
void ChannelSoundingAlgorithm::DataCleaning::UpdateDeltaF(ChannelSoundingAlgorithm& cs_algo) {
    uint8_t df = cs_algo.step_channel_cleaned_[cs_algo.step_channel_cleaned_.size() - 1] -
                 cs_algo.step_channel_cleaned_[0];
    for (size_t i = 1; i < cs_algo.step_channel_cleaned_.size(); i++) {
        if (cs_algo.step_channel_cleaned_[i] - cs_algo.step_channel_cleaned_[i - 1] < df) {
            df = cs_algo.step_channel_cleaned_[i] - cs_algo.step_channel_cleaned_[i - 1];
        }
    }
    cs_algo.delta_f_ = df;
}

void ChannelSoundingAlgorithm::DataCleaning::CalculateAutocorr(ChannelSoundingAlgorithm& cs_algo) {
    size_t K = cs_algo.autocorr_K_ / cs_algo.delta_f_;
    double k_count;
    std::vector<std::vector<std::complex<double>>> R_k(cs_algo.n_ap_,
        ...
        for (size_t k = 0; k < K; k++) {
            ...
            for (size_t i = 0; i < cs_algo.step_channel_cleaned_.size(); i++) {
                for (size_t j = 0; j < cs_algo.step_channel_cleaned_.size(); j++) {
                    if (cs_algo.step_channel_cleaned_[j] - cs_algo.step_channel_cleaned_[i] ==
                        k * cs_algo.delta_f_) {
...
void ChannelSoundingAlgorithm::DataCleaning::CalculateCovarianceMatrix(
    ...
    for (size_t ap = 0; ap < cs_algo.n_ap_; ap++) {
        ...
        for (uint8_t ch_head = cs_algo.step_channel_cleaned_[0];
             ch_head <
             cs_algo.step_channel_cleaned_[cs_algo.step_channel_cleaned_.size() - 1] - K + 1;
             ch_head++) {
            ...
            for (size_t i = 0; i < K; i++) {
                if (i * cs_algo.delta_f_ + idx_offset == cs_algo.step_channel_cleaned_[cur_pos]) {
        ...
        for (size_t i = 0; i < K; i++) {
            for (size_t j = 0; j < K; j++) {
                R_k[ap](i, j) = 0.5 * (R_tmp(i, j) + std::conj(R_tmp(K - i - 1, K - j - 1)));
            }
        }
    }
...
void ChannelSoundingAlgorithm::RangingAlgorithm::Run(ChannelSoundingAlgorithm& cs_algo) {
    this->fft_.Init(this->fft_size_);

    if (cs_algo.use_pre_combining_) {
        ChannelSoundingAlgorithm::RangingAlgorithm::PreCombiningAddAutocorr(cs_algo);
    }
    if (!cs_algo.use_post_combining_) {
        switch (cs_algo.algo_type_) {
            case AlgoType::kZpIfft:
                ChannelSoundingAlgorithm::RangingAlgorithm::EstimateDistanceZpIfft(cs_algo);
                break;
            default:
...
void ChannelSoundingAlgorithm::RangingAlgorithm::PreCombiningAddAutocorr(
        ChannelSoundingAlgorithm& cs_algo) {
    std::vector<std::complex<double>> pct_autocorr_combined(cs_algo.pct_autocorr_[0].size(), 0);
...
double ChannelSoundingAlgorithm::RangingAlgorithm::EstimateDistanceZpIfft(
    ...
    std::rotate(CFR_ZP.rbegin(), CFR_ZP.rbegin() + (this->fft_size_ - CFR_size) / 2, CFR_ZP.rend());
    ...
    std::vector<double> ifft_output_dB(this->fft_size_), ifft_output_power(this->fft_size_);
    ...
    size_t cir_shift_size = static_cast<size_t>(std::round(
            this->shift_distance_ * 2 * this->fft_size_ * cs_algo.delta_f_ * 1e6 / kSpeedOfLight));
    double actual_shift_distance = (double)cir_shift_size / 2 / (double)this->fft_size_ /
                                   (cs_algo.delta_f_ * 1e6) * kSpeedOfLight;
    size_t noise_interval_size =
            (size_t)noise_est_interval_ * 2 * fft_size_ * cs_algo.delta_f_ * 1e6 / kSpeedOfLight;
    double noise_level =
            10 * log10(static_cast<double>(std::accumulate(
    ...
    auto peaks = ChannelSoundingAlgorithm::RangingAlgorithm::FindPeaksInDb(
            ifft_output_dB, valid_peak, noise_level + threshold_zp_ifft_, cir_shift_size);
    double distance_report = kSpeedOfLight * dt * peaks[0].first / 2 - actual_shift_distance;
    for (size_t i = 0; i < peaks.size(); i++) {
        double peak_distance = static_cast<double>(peaks[i].first) * kSpeedOfLight * dt / 2 -
...
std::vector<std::pair<int, double>> ChannelSoundingAlgorithm::RangingAlgorithm::FindPeaksInDb(
        std::vector<double> input, bool& valid, const double threshold, size_t cir_shift_size) {
    valid = true;
    ...
    // 0
    if (input[0] > input[input.size() - 1] && input[0] > input[1] && input[0] >= threshold) {
...
void ChannelSoundingAlgorithm::RangingAlgorithm::Fft::ComputeComplexIfft(
        const std::vector<std::complex<double>>& input, std::vector<std::complex<double>>& output) {
