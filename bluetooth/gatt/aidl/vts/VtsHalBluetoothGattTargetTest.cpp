...
class MockBluetoothGattCallback : public BnBluetoothGattCallback {
  public:
    MOCK_METHOD(ScopedAStatus, registerServiceComplete,
                (int32_t in_sessionId, ::aidl::android::hardware::bluetooth::gatt::Status in_status,
    ...
    MOCK_METHOD(ScopedAStatus, clearServicesComplete,
                (int32_t in_acl_connection_handle, const std::string& in_reason), (override));
    MOCK_METHOD(ScopedAStatus, errorReport,
                (const ::aidl::android::hardware::bluetooth::gatt::ErrorReport& in_report),
                (override));
};
...
class BluetoothGattTest : public ::testing::TestWithParam<std::string> {
  public:
    ...
    void SetUpCapability() {
        ScopedAStatus status = bluetooth_gatt_->getGattCapabilities(&gatt_capabilities_);
...
TEST_P(BluetoothGattTest, GetGattCapabilities) {
    ...
    if (gatt_capabilities_.supportedGattClientProperties) {
        ...
        ASSERT_TRUE((gatt_capabilities_.supportedGattClientProperties & kGattPropertyNotify) != 0);
    }
    if (gatt_capabilities_.supportedGattServerProperties) {
        ...
        ASSERT_TRUE((gatt_capabilities_.supportedGattServerProperties & kGattPropertyNotify) != 0);
    }
}
...
TEST_P(BluetoothGattTest, UnregisterService) {
    ...
    std::future<void> unregister_service_cb_future{unregister_service_cb_promise.get_future()};
    ...
    if (status.isOk()) {
        ...
        // the timeout.
        EXPECT_EQ(unregister_service_cb_future.wait_for(timeout), std::future_status::ready);
    } else {
        ...
        EXPECT_EQ(unregister_service_cb_future.wait_for(timeout), std::future_status::timeout);
    }
}
...
TEST_P(BluetoothGattTest, ClearService) {
    ...
    std::future<void> clear_service_cb_future{clear_service_cb_promise.get_future()};
    ...
    if (status.isOk()) {
        ...
        // the timeout.
        EXPECT_EQ(clear_service_cb_future.wait_for(timeout), std::future_status::ready);
    } else {
        ...
        EXPECT_EQ(clear_service_cb_future.wait_for(timeout), std::future_status::timeout);
    }
}
...
void BluetoothGattTest::RegisterService(GattSession::Role role) {
    ...
    std::future<void> register_service_cb_future{register_service_cb_promise.get_future()};
    ...
    if (status.isOk()) {
        ...
        // timeout.
        EXPECT_EQ(register_service_cb_future.wait_for(timeout), std::future_status::ready);
    } else {
        ...
        EXPECT_EQ(register_service_cb_future.wait_for(timeout), std::future_status::timeout);
    }
}

GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(BluetoothGattTest);
INSTANTIATE_TEST_SUITE_P(
        PerInstance, BluetoothGattTest,
        testing::ValuesIn(android::getAidlHalInstanceNames(IBluetoothGatt::descriptor)),
