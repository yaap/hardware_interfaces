...
using aidl::android::hardware::bluetooth::audio::LeAudioCodecCapabilitiesSetting;
...
using LeAudioDeviceCapabilities = IBluetoothAudioProvider::LeAudioDeviceCapabilities;
using LeAudioConfigurationRequirement = IBluetoothAudioProvider::LeAudioConfigurationRequirement;
...
using LeAudioSubgroupBisConfiguration = IBluetoothAudioProvider::LeAudioSubgroupBisConfiguration;
...
static constexpr ChannelMode a2dp_channel_modes[] = {ChannelMode::UNKNOWN, ChannelMode::MONO,
                                                     ChannelMode::STEREO};
...
template <class T>
bool contained_in_vector(const std::vector<T>& vector, const typename identity<T>::type& target) {
...
class BluetoothAudioPort : public BnBluetoothAudioPort {
  public:
    ...
    ndk::ScopedAStatus updateSourceMetadata(const SourceMetadata&) { return ScopedAStatus::ok(); }

    ndk::ScopedAStatus updateSinkMetadata(const SinkMetadata&) { return ScopedAStatus::ok(); }

    ndk::ScopedAStatus setLatencyMode(const LatencyMode) { return ScopedAStatus::ok(); }

    ndk::ScopedAStatus updateSinkLatency(int64_t) { return ScopedAStatus::ok(); }

    ndk::ScopedAStatus setCodecType(const CodecType) { return ScopedAStatus::ok(); }

  protected:
    virtual ~BluetoothAudioPort() = default;
};

class BluetoothAudioProviderFactoryAidl : public testing::TestWithParam<std::string> {
  public:
    ...
    void GetProviderInfoHelper(const SessionType& session_type) {
        ...
        auto aidl_reval = provider_factory_->getProviderInfo(session_type, &temp_provider_info_);
    }
        ...
        switch (session_type) {
            ...
            case SessionType::A2DP_HARDWARE_OFFLOAD_DECODING_DATAPATH: {
                ...
                for (auto& audio_capability : temp_provider_capabilities_) {
                    ASSERT_EQ(audio_capability.getTag(), AudioCapabilities::a2dpCapabilities);
            ...
            case SessionType::LE_AUDIO_BROADCAST_HARDWARE_OFFLOAD_ENCODING_DATAPATH: {
                ...
                // not hardcoded
                for (auto audio_capability : temp_provider_capabilities_) {
                    ASSERT_EQ(audio_capability.getTag(), AudioCapabilities::leAudioCapabilities);
                }
            } break;
    ...
    void OpenProviderHelper(const SessionType& session_type) {
        auto aidl_retval = provider_factory_->openProvider(session_type, &audio_provider_);
        if (aidl_retval.isOk()) {
            ...
            ASSERT_TRUE(
                    ...
                    session_type == SessionType::A2DP_HARDWARE_OFFLOAD_ENCODING_DATAPATH ||
    ...
    void GetA2dpOffloadCapabilityHelper(const CodecType& codec_type) {
        ...
        for (auto& codec_capability : temp_provider_capabilities_) {
            auto& a2dp_capabilities = codec_capability.get<AudioCapabilities::a2dpCapabilities>();
    ...
    std::vector<CodecConfiguration::CodecSpecific> GetSbcCodecSpecificSupportedList(
            bool supported) {
        ...
        if (!supported) {
            ...
            sbc_codec_specifics.push_back(CodecConfiguration::CodecSpecific(sbc_config));
        ...
        auto& sbc_capability = temp_codec_capabilities_->capabilities
    ...
    std::vector<CodecConfiguration::CodecSpecific> GetAacCodecSpecificSupportedList(
            bool supported) {
        ...
        if (!supported) {
            ...
            aac_codec_specifics.push_back(CodecConfiguration::CodecSpecific(aac_config));
        ...
        // parse the capability
        auto& aac_capability = temp_codec_capabilities_->capabilities
                ...
                for (auto channel_mode : aac_capability.channelMode) {
                    for (int8_t bits_per_sample : aac_capability.bitsPerSample) {
                        for (auto variable_bit_rate_enabled : variable_bit_rate_enableds) {
                            for (auto adaptive_bit_rate_supported : adaptive_bit_rate_supporteds) {
    ...
    std::vector<CodecConfiguration::CodecSpecific> GetLdacCodecSpecificSupportedList(
            bool supported) {
        ...
        if (!supported) {
            ...
            ldac_codec_specifics.push_back(CodecConfiguration::CodecSpecific(ldac_config));
        ...
        // parse the capability
        auto& ldac_capability = temp_codec_capabilities_->capabilities
    ...
    std::vector<CodecConfiguration::CodecSpecific> GetAptxCodecSpecificSupportedList(
        ...
        if (!supported) {
            ...
            aptx_codec_specifics.push_back(CodecConfiguration::CodecSpecific(aptx_config));
            return aptx_codec_specifics;
        }
        GetA2dpOffloadCapabilityHelper((is_hd ? CodecType::APTX_HD : CodecType::APTX));
        if (temp_codec_capabilities_ == nullptr) {
            return aptx_codec_specifics;
        }
        ...
        // parse the capability
        auto& aptx_capability = temp_codec_capabilities_->capabilities
            ...
            for (int32_t sample_rate : aptx_capability.sampleRateHz) {
                for (auto channel_mode : aptx_capability.channelMode) {
                    AptxConfiguration aptx_data{.sampleRateHz = sample_rate,
                                                .channelMode = channel_mode,
                                                .bitsPerSample = bits_per_sample};
                    aptx_codec_specifics.push_back(CodecConfiguration::CodecSpecific(aptx_data));
                }
            }
        }
        return aptx_codec_specifics;
    ...
    std::vector<CodecConfiguration::CodecSpecific> GetOpusCodecSpecificSupportedList(
            bool supported) {
        std::vector<CodecConfiguration::CodecSpecific> opus_codec_specifics;
        if (!supported) {
            OpusConfiguration opus_config{.samplingFrequencyHz = 0, .frameDurationUs = 0};
            opus_codec_specifics.push_back(CodecConfiguration::CodecSpecific(opus_config));
        ...
        // parse the capability
        auto& opus_capability = temp_codec_capabilities_->capabilities
        ...
        for (int32_t samplingFrequencyHz : opus_capability->samplingFrequencyHz) {
            for (int32_t frameDurationUs : opus_capability->frameDurationUs) {
                for (auto channel_mode : opus_capability->channelMode) {
                    ...
                    opus_codec_specifics.push_back(CodecConfiguration::CodecSpecific(opus_data));
                }
            }
        }
        return opus_codec_specifics;
    ...
    bool IsPcmConfigSupported(const PcmConfiguration& pcm_config) {
        if (temp_provider_capabilities_.size() != 1 ||
            temp_provider_capabilities_[0].getTag() != AudioCapabilities::pcmCapabilities) {
            return false;
        }
        auto pcm_capability =
                temp_provider_capabilities_[0].get<AudioCapabilities::pcmCapabilities>();
        return (contained_in_vector(pcm_capability.channelMode, pcm_config.channelMode) &&
                contained_in_vector(pcm_capability.sampleRateHz, pcm_config.sampleRateHz) &&
                contained_in_vector(pcm_capability.bitsPerSample, pcm_config.bitsPerSample));
    }
    ...
    std::optional<IBluetoothAudioProviderFactory::ProviderInfo> temp_provider_info_;
 ...
 */
TEST_P(BluetoothAudioProviderFactoryAidl, OpenProviderAndCheckCapabilitiesBySession) {
    ...
    if (GetProviderFactoryInterfaceVersion() >= BluetoothAudioHalVersion::VERSION_AIDL_V4) {
    ...
    if (GetProviderFactoryInterfaceVersion() >= BluetoothAudioHalVersion::VERSION_AIDL_V6) {
...
TEST_P(BluetoothAudioProviderFactoryAidl, getProviderInfo_invalidSessionTypes) {
    ...
    for (auto session_type : kInvalidSessionTypes) {
        std::optional<IBluetoothAudioProviderFactory::ProviderInfo> provider_info = std::nullopt;
        auto aidl_retval = provider_factory_->getProviderInfo(session_type, &provider_info);
        if (!aidl_retval.isOk()) {
            continue;
        }
...
class BluetoothAudioProviderAidl : public BluetoothAudioProviderFactoryAidl {
  ...
  public:
    void SetUp() override {
        ...
        (void)provider_factory_->openProvider(SessionType::A2DP_HARDWARE_OFFLOAD_ENCODING_DATAPATH,
                                              &a2dp_encoding_provider_);

        (void)provider_factory_->openProvider(SessionType::A2DP_HARDWARE_OFFLOAD_DECODING_DATAPATH,
...
TEST_P(BluetoothAudioProviderAidl, parseA2dpConfiguration_invalidSessionType) {
    ...
    for (auto session_type : kInvalidSessionTypes) {
        ...
        auto aidl_retval = provider->parseA2dpConfiguration(codec_id, std::vector<uint8_t>{},
 ...
 */
TEST_P(BluetoothAudioProviderAidl, parseA2dpConfiguration_unsupportedCodecType) {
    ...
    for (auto& provider : {a2dp_encoding_provider_, a2dp_decoding_provider_}) {
        ...
        aidl_retval = provider->parseA2dpConfiguration(unsupported_core_id, std::vector<uint8_t>{},
                                                       &codec_parameters, &a2dp_status);
        EXPECT_TRUE(!aidl_retval.isOk() || a2dp_status == A2dpStatus::NOT_SUPPORTED_CODEC_TYPE);
 ...
 */
TEST_P(BluetoothAudioProviderAidl, parseA2dpConfiguration_invalidConfiguration) {
    for (auto& [provider, provider_info] :
        ...
        aidl_retval = provider->parseA2dpConfiguration(provider_info->codecInfos[0].id,
                                                       std::vector<uint8_t>{}, &codec_parameters,
                                                       &a2dp_status);
        ASSERT_TRUE(aidl_retval.isOk());
...
TEST_P(BluetoothAudioProviderAidl, parseA2dpConfiguration_valid) {
    for (auto& [provider, provider_info] :
        ...
        aidl_retval = provider->getA2dpConfiguration(
                std::vector<A2dpRemoteCapabilities>{remote_capabilities}, A2dpConfigurationHint(),
                &configuration);
        ASSERT_TRUE(aidl_retval.isOk());
        ASSERT_TRUE(configuration.has_value());

        aidl_retval = provider->parseA2dpConfiguration(
                configuration->id, configuration->configuration, &codec_parameters, &a2dp_status);
        ASSERT_TRUE(aidl_retval.isOk());
        EXPECT_TRUE(a2dp_status == A2dpStatus::OK);
...
TEST_P(BluetoothAudioProviderAidl, getA2dpConfiguration_invalidSessionType) {
    ...
    for (auto session_type : kInvalidSessionTypes) {
        ...
        std::optional<A2dpConfiguration> configuration;
        aidl_retval = provider->getA2dpConfiguration(std::vector<A2dpRemoteCapabilities>{},
 ...
 */
TEST_P(BluetoothAudioProviderAidl, getA2dpConfiguration_unknownRemoteCapabilities) {
    for (auto& [provider, provider_info] :
         {std::pair(a2dp_encoding_provider_, a2dp_encoding_provider_info_),
          std::pair(a2dp_decoding_provider_, a2dp_decoding_provider_info_)}) {
        if (provider == nullptr || !provider_info.has_value() ||
 ...
 * must return an empty configuration.
 */
TEST_P(BluetoothAudioProviderAidl, getA2dpConfiguration_invalidRemoteCapabilities) {
    for (auto& [provider, provider_info] :
         {std::pair(a2dp_encoding_provider_, a2dp_encoding_provider_info_),
          std::pair(a2dp_decoding_provider_, a2dp_decoding_provider_info_)}) {
        if (provider == nullptr || !provider_info.has_value() ||
        ...
        aidl_retval = provider->getA2dpConfiguration(
                std::vector<A2dpRemoteCapabilities>{
                        A2dpRemoteCapabilities(/*seid*/ 0, codec_info.id, std::vector<uint8_t>()),
                        A2dpRemoteCapabilities(/*seid*/ 1, codec_info.id, invalid_capabilities),
                },
                A2dpConfigurationHint(), &configuration);
        ASSERT_TRUE(aidl_retval.isOk());
        EXPECT_FALSE(configuration.has_value());
 ...
 * input SEID.
 */
TEST_P(BluetoothAudioProviderAidl, getA2dpConfiguration_validRemoteCapabilities) {
    for (auto& [provider, provider_info] :
         {std::pair(a2dp_encoding_provider_, a2dp_encoding_provider_info_),
          std::pair(a2dp_decoding_provider_, a2dp_decoding_provider_info_)}) {
        if (provider == nullptr || !provider_info.has_value() ||
        ...
        for (auto const& codec_info : provider_info->codecInfos) {
            ...
            // parameters.
            EXPECT_NE(std::find(a2dp_info.channelMode.begin(), a2dp_info.channelMode.end(),
            ...
            CodecParameters codec_parameters;
            A2dpStatus a2dp_status = A2dpStatus::OK;
            aidl_retval = provider->parseA2dpConfiguration(configuration->id,
...
TEST_P(BluetoothAudioProviderAidl, getA2dpConfiguration_hintCodecId) {
    for (auto& [provider, provider_info] :
        ...
        for (auto const& codec_info : provider_info->codecInfos) {
            ...
            aidl_retval = provider->getA2dpConfiguration(remote_capabilities, hint, &configuration);
        ...
        for (auto& codec_id : {CodecId(CodecId::Core::CVSD),
 ...
 */
TEST_P(BluetoothAudioProviderAidl, getA2dpConfiguration_hintSamplingFrequencyHz) {
    for (auto& [provider, provider_info] :
         {std::pair(a2dp_encoding_provider_, a2dp_encoding_provider_info_),
          std::pair(a2dp_decoding_provider_, a2dp_decoding_provider_info_)}) {
        if (provider == nullptr || !provider_info.has_value() ||
        ...
        for (auto const& codec_info : provider_info->codecInfos) {
            ...
            for (auto& sampling_frequency_hz : {
                ...
                // by the codec, and applied otherwise.
                ASSERT_EQ(configuration->parameters.samplingFrequencyHz == sampling_frequency_hz,
                          std::find(a2dp_info.samplingFrequencyHz.begin(),
                                    a2dp_info.samplingFrequencyHz.end(),
                                    sampling_frequency_hz) != a2dp_info.samplingFrequencyHz.end());
            }
        }
    }
}
    ...
    for (auto& [provider, provider_info] :
        ...
        for (auto const& codec_info : provider_info->codecInfos) {
            ...
            for (auto& bitdepth : {0, 1, 16, 24, 32}) {
                ...
                // by the codec, and applied otherwise.
                ASSERT_EQ(configuration->parameters.bitdepth == bitdepth,
...
TEST_P(BluetoothAudioProviderAidl, startSession_unknownCodecId) {
    for (auto& [provider, provider_info] :
        ...
        for (auto& codec_id : {CodecId(CodecId::Core::CVSD),
            ...
            auto aidl_retval = provider->startSession(audio_port_, AudioConfiguration(a2dp_config),
...
TEST_P(BluetoothAudioProviderAidl, startSession_valid) {
    for (auto& [provider, provider_info] :
        ...
        aidl_retval = provider->startSession(audio_port_, AudioConfiguration(a2dp_config),
                                             std::vector<LatencyMode>{}, &data_mq_desc);
 ...
 */
class BluetoothAudioProviderA2dpEncodingSoftwareAidl : public BluetoothAudioProviderFactoryAidl {
  public:
    virtual void SetUp() override {
        BluetoothAudioProviderFactoryAidl::SetUp();
 ...
 */
TEST_P(BluetoothAudioProviderA2dpEncodingSoftwareAidl, OpenA2dpEncodingSoftwareProvider) {}
 ...
 */
class BluetoothAudioProviderHfpSoftwareEncodingAidl : public BluetoothAudioProviderFactoryAidl {
  public:
    virtual void SetUp() override {
        BluetoothAudioProviderFactoryAidl::SetUp();
        if (GetProviderFactoryInterfaceVersion() < BluetoothAudioHalVersion::VERSION_AIDL_V4) {
    ...
    bool OpenSession(int32_t sample_rate, int8_t bits_per_sample, ChannelMode channel_mode,
 ...
 * Test whether we can open a provider of type
 */
TEST_P(BluetoothAudioProviderHfpSoftwareEncodingAidl, OpenHfpSoftwareEncodingProvider) {}
 ...
 * Test whether we can open a provider of type
 */
TEST_P(BluetoothAudioProviderHfpSoftwareDecodingAidl, OpenHfpSoftwareDecodingProvider) {}
 ...
 */
class BluetoothAudioProviderA2dpEncodingHardwareAidl : public BluetoothAudioProviderFactoryAidl {
  public:
    virtual void SetUp() override {
        BluetoothAudioProviderFactoryAidl::SetUp();
        GetProviderCapabilitiesHelper(SessionType::A2DP_HARDWARE_OFFLOAD_ENCODING_DATAPATH);
        OpenProviderHelper(SessionType::A2DP_HARDWARE_OFFLOAD_ENCODING_DATAPATH);
        ASSERT_TRUE(temp_provider_capabilities_.empty() || audio_provider_ != nullptr);
    }
 ...
 * Test whether we can open a provider of type
 */
TEST_P(BluetoothAudioProviderA2dpEncodingHardwareAidl, OpenA2dpEncodingHardwareProvider) {}
 ...
 */
TEST_P(BluetoothAudioProviderA2dpEncodingHardwareAidl, StartAndEndA2dpSbcEncodingHardwareSession) {
 ...
 */
TEST_P(BluetoothAudioProviderA2dpEncodingHardwareAidl, StartAndEndA2dpAacEncodingHardwareSession) {
    if (!IsOffloadSupported()) {
        GTEST_SKIP();
    }
 ...
 */
TEST_P(BluetoothAudioProviderA2dpEncodingHardwareAidl, StartAndEndA2dpLdacEncodingHardwareSession) {
    if (!IsOffloadSupported()) {
        GTEST_SKIP();
    }
 ...
 */
TEST_P(BluetoothAudioProviderA2dpEncodingHardwareAidl, StartAndEndA2dpOpusEncodingHardwareSession) {
    if (!IsOffloadSupported()) {
        GTEST_SKIP();
    }
 ...
 */
TEST_P(BluetoothAudioProviderA2dpEncodingHardwareAidl, StartAndEndA2dpAptxEncodingHardwareSession) {
    if (!IsOffloadSupported()) {
        GTEST_SKIP();
    }
    ...
    for (auto codec_type : {CodecType::APTX, CodecType::APTX_HD}) {
        CodecConfiguration codec_config = {
                ...
                .encodedAudioBitrate = (codec_type == CodecType::APTX ? 352000 : 576000),
                .peerMtu = 1005,
                .isScmstEnabled = false,
        };
 ...
 */
class BluetoothAudioProviderHfpHardwareAidl : public BluetoothAudioProviderFactoryAidl {
  public:
    virtual void SetUp() override {
        BluetoothAudioProviderFactoryAidl::SetUp();
        if (GetProviderFactoryInterfaceVersion() < BluetoothAudioHalVersion::VERSION_AIDL_V4) {
    ...
    bool OpenSession(CodecId codec_id, int connection_handle, bool nrec, bool controller_codec) {
        ...
        DataMQDesc mq_desc;
        auto aidl_retval = audio_provider_->startSession(
                audio_port_, AudioConfiguration(hfp_configuration), latency_modes, &mq_desc);
 ...
 */
TEST_P(BluetoothAudioProviderHfpHardwareAidl, StartAndEndHfpHardwareSessionWithPossiblePcmConfig) {
 ...
 */
class BluetoothAudioProviderHearingAidSoftwareAidl : public BluetoothAudioProviderFactoryAidl {
  public:
    virtual void SetUp() override {
        BluetoothAudioProviderFactoryAidl::SetUp();
        GetProviderCapabilitiesHelper(SessionType::HEARING_AID_SOFTWARE_ENCODING_DATAPATH);
 ...
 * Test whether we can open a provider of type
 */
TEST_P(BluetoothAudioProviderHearingAidSoftwareAidl, OpenHearingAidSoftwareProvider) {}
 ...
 */
class BluetoothAudioProviderLeAudioOutputSoftwareAidl : public BluetoothAudioProviderFactoryAidl {
  public:
    virtual void SetUp() override {
        BluetoothAudioProviderFactoryAidl::SetUp();
        GetProviderCapabilitiesHelper(SessionType::LE_AUDIO_SOFTWARE_ENCODING_DATAPATH);
    ...
    static constexpr ChannelMode le_audio_output_channel_modes_[] = {
            ChannelMode::UNKNOWN, ChannelMode::MONO, ChannelMode::STEREO};
    static constexpr int32_t le_audio_output_data_interval_us_[] = {0 /* Invalid */,
                                                                    10000 /* Valid 10ms */};
};
 ...
 * stopped
 */
TEST_P(BluetoothAudioProviderLeAudioOutputSoftwareAidl, OpenLeAudioOutputSoftwareProvider) {}
 ...
 */
class BluetoothAudioProviderLeAudioInputSoftwareAidl : public BluetoothAudioProviderFactoryAidl {
  public:
    virtual void SetUp() override {
        BluetoothAudioProviderFactoryAidl::SetUp();
        GetProviderCapabilitiesHelper(SessionType::LE_AUDIO_SOFTWARE_DECODING_DATAPATH);
    ...
    static constexpr int32_t le_audio_input_sample_rates_[] = {0,     8000,  16000, 24000,
                                                               32000, 44100, 48000};
    ...
    static constexpr ChannelMode le_audio_input_channel_modes_[] = {
            ChannelMode::UNKNOWN, ChannelMode::MONO, ChannelMode::STEREO};
    static constexpr int32_t le_audio_input_data_interval_us_[] = {0 /* Invalid */,
                                                                   10000 /* Valid 10ms */};
};
 ...
 * stopped
 */
TEST_P(BluetoothAudioProviderLeAudioInputSoftwareAidl, OpenLeAudioInputSoftwareProvider) {}
 ...
 */
class BluetoothAudioProviderLeAudioOutputHardwareAidl : public BluetoothAudioProviderFactoryAidl {
  public:
    virtual void SetUp() override {
        BluetoothAudioProviderFactoryAidl::SetUp();
        GetProviderCapabilitiesHelper(SessionType::LE_AUDIO_HARDWARE_OFFLOAD_ENCODING_DATAPATH);
    ...
    bool IsOffloadOutputSupported() {
        for (auto& capability : temp_provider_capabilities_) {
            ...
            auto& le_audio_capability = capability.get<AudioCapabilities::leAudioCapabilities>();
            if (le_audio_capability.unicastEncodeCapability.codecType != CodecType::UNKNOWN)
                return true;
        }
        return false;
    }
    ...
    bool IsOffloadOutputProviderInfoSupported() {
        ...
        for (auto& codec_info : temp_provider_info_.value().codecInfos) {
            if (codec_info.transport.getTag() != CodecInfo::Transport::leAudio) return false;
        }
        return true;
    }
    ...
    LeAudioDeviceCapabilities GetDefaultRemoteSinkCapability() {
        ...
        auto sampling_rate = CodecSpecificCapabilitiesLtv::SupportedSamplingFrequencies();
        ...
        auto frame_duration = CodecSpecificCapabilitiesLtv::SupportedFrameDurations();
        frame_duration.bitmask = CodecSpecificCapabilitiesLtv::SupportedFrameDurations::US7500 |
        ...
        frames.value = 2;
        capability.codecSpecificCapabilities = {sampling_rate, frame_duration, octets, frames};
        return capability;
    }
    ...
    LeAudioDeviceCapabilities GetOpusRemoteSinkCapability() {
        ...
        auto pref_context_metadata = MetadataLtv::PreferredAudioContexts();
        pref_context_metadata.values = GetAudioContext(
                AudioContext::MEDIA | AudioContext::CONVERSATIONAL | AudioContext::GAME);
        capability.metadata = {pref_context_metadata};
    ...
    LeAudioDeviceCapabilities GetDefaultRemoteSourceCapability() {
        ...
        auto pref_context_metadata = MetadataLtv::PreferredAudioContexts();
        pref_context_metadata.values = GetAudioContext(
                AudioContext::LIVE_AUDIO | AudioContext::CONVERSATIONAL | AudioContext::GAME);
        capability.metadata = {pref_context_metadata};
    ...
    bool IsAseRequirementSatisfiedWithUnknownChannelCount(
            const std::vector<std::optional<AseDirectionRequirement>>& ase_requirements,
            const std::vector<std::optional<AseDirectionConfiguration>>& ase_configurations) {
        ...
        for (const auto& ase_req : ase_requirements) {
            auto required_allocation_ltv =
                    GetConfigurationLtv(ase_req->aseConfiguration.codecConfiguration,
        ...
        for (const auto& ase_req : ase_requirements) {
            ...
            for (const auto& ase_conf : ase_configurations) {
                ...
                auto config_sample_freq_ltv =
                        GetConfigurationLtv(ase_conf->aseConfiguration.codecConfiguration,
                                            CodecSpecificConfigurationLtv::Tag::samplingFrequency);
                ...
                int configured_allocation =
                        config_allocation_ltv
                                ->get<CodecSpecificConfigurationLtv::Tag::audioChannelAllocation>()
                                .bitmask;
    ...
    bool IsAseRequirementSatisfied(
        ...
        if (ase_requirements.size() != required_allocations) {
             ...
             * different verifier */
            return IsAseRequirementSatisfiedWithUnknownChannelCount(ase_requirements,
                                                                    ase_configurations);
        }
    ...
    static void VerifyCodecParameters(
        ...
        for (auto const& param : config.aseConfiguration.codecConfiguration) {
            if (param.getTag() ==
                ::aidl::android::hardware::bluetooth::audio::CodecSpecificConfigurationLtv::Tag::
                        codecFrameBlocksPerSDU) {
                frame_blocks = param.get<::aidl::android::hardware::bluetooth::audio::
                ...
                allocation_bitmask =
                        static_cast<int32_t>(param.get<::aidl::android::hardware::bluetooth::audio::
                                                               CodecSpecificConfigurationLtv::Tag::
        ...
        ASSERT_TRUE(config.qosConfiguration->sduIntervalUs >= frame_blocks * frame_duration);
    ...
    void VerifyIfRequirementsSatisfied(
        ...
        for (const auto& conf : configurations) {
            num_of_configurations += std::bitset<32>(conf.audioContext.bitmask).count();
        }
        ...
        for (const auto& req : requirements) {
            for (const auto& conf : configurations) {
                ...
                if (req.sourceAseRequirement) {
                    ...
                    source_req_satisfied = IsAseRequirementSatisfied(*req.sourceAseRequirement,
    ...
    static bool VerifyMaxSduForAbrCodec(
        ...
        auto it = std::find(maxSduForAbrCodec.begin(), maxSduForAbrCodec.end(), qos.maxSdu);
        ...
        auto last = std::unique(sortedMaxSduForAbrCodec.begin(), sortedMaxSduForAbrCodec.end());
    ...
    LeAudioConfigurationRequirement GetUnicastDefaultRequirement(
            int32_t context_bits, bool is_sink_requirement, bool is_source_requriement,
        ...
        if (is_sink_requirement) requirement.sinkAseRequirement = {direction_ase_requriement};

        if (is_source_requriement) requirement.sourceAseRequirement = {direction_ase_requriement};
    ...
    LeAudioAseQosConfigurationRequirement GetQosRequirements(bool is_sink_requirement,
    ...
    std::vector<Lc3Configuration> GetUnicastLc3SupportedList(bool decoding, bool supported) {
        std::vector<Lc3Configuration> le_audio_codec_configs;
        if (!supported) {
        ...
        for (auto& capability : temp_provider_capabilities_) {
            ...
            auto& le_audio_capability = capability.get<AudioCapabilities::leAudioCapabilities>();
            auto& unicast_capability = decoding ? le_audio_capability.unicastDecodeCapability
    ...
    std::vector<AptxAdaptiveLeConfiguration> GetUnicastAptxAdaptiveLeSupportedList(
            bool decoding, bool supported, bool is_le_extended) {
        std::vector<AptxAdaptiveLeConfiguration> le_audio_codec_configs;
        if (!supported) {
            AptxAdaptiveLeConfiguration aptx_adaptive_le_config{.pcmBitDepth = 0,
        ...
        for (auto& capability : temp_provider_capabilities_) {
            ...
            auto& aptx_adaptive_le_capability = unicast_capability.leAudioCodecCapabilities.get<
                    UnicastCapability::LeAudioCodecCapabilities::aptxAdaptiveLeCapabilities>();
            ...
            for (int32_t samplingFrequencyHz : aptx_adaptive_le_capability.samplingFrequencyHz) {
                for (int32_t frameDurationUs : aptx_adaptive_le_capability.frameDurationUs) {
                    for (int32_t octetsPerFrame : aptx_adaptive_le_capability.octetsPerFrame) {
                        for (int8_t blocksPerSdu : aptx_adaptive_le_capability.blocksPerSdu) {
 ...
 * stopped
 */
TEST_P(BluetoothAudioProviderLeAudioOutputHardwareAidl, OpenLeAudioOutputHardwareProvider) {}
...
TEST_P(BluetoothAudioProviderLeAudioOutputHardwareAidl,
    ...
    for (auto& lc3_config : lc3_codec_configs) {
        le_audio_config.leAudioCodecConfig.set<LeAudioCodecConfiguration::lc3Config>(lc3_config);
        DataMQDesc mq_desc;
        auto aidl_retval = audio_provider_->startSession(
                audio_port_, AudioConfiguration(le_audio_config), latency_modes, &mq_desc);
...
TEST_P(BluetoothAudioProviderLeAudioOutputHardwareAidl, GetEmptyAseConfigurationEmptyCapability) {
    if (GetProviderFactoryInterfaceVersion() < BluetoothAudioHalVersion::VERSION_AIDL_V4) {
        GTEST_SKIP();
    }
    ...
    aidl_retval = audio_provider_->getLeAudioAseConfiguration(empty_capability, std::nullopt,
                                                              empty_requirement, &configurations);
...
TEST_P(BluetoothAudioProviderLeAudioOutputHardwareAidl,
    ...
    std::vector<LeAudioConfigurationRequirement> source_requirements = {
            GetUnicastDefaultRequirement(AudioContext::LIVE_AUDIO, false /*sink */,
                                         true /* source */, not_supported_sampling_rate_by_remote)};
    auto aidl_retval = audio_provider_->getLeAudioAseConfiguration(
    ...
    std::vector<LeAudioConfigurationRequirement> sink_requirements = {
            GetUnicastDefaultRequirement(AudioContext::MEDIA, true /*sink */, false /* source */,
                                         not_supported_sampling_rate_by_remote)};
    aidl_retval = audio_provider_->getLeAudioAseConfiguration(sink_capabilities, std::nullopt,
                                                              sink_requirements, &configurations);
...
TEST_P(BluetoothAudioProviderLeAudioOutputHardwareAidl, GetAseConfiguration) {
    ...
    std::vector<LeAudioConfigurationRequirement> multi_sink_requirements = {
            GetUnicastDefaultRequirement(AudioContext::MEDIA, true /* sink */, false /* source */),
            GetUnicastDefaultRequirement(AudioContext::CONVERSATIONAL, true /* sink */,
                                         false /* source */)};
    aidl_retval = audio_provider_->getLeAudioAseConfiguration(
            sink_capabilities, std::nullopt, multi_sink_requirements, &configurations);
    ...
    std::vector<LeAudioConfigurationRequirement> multi_context_sink_requirements = {
            GetUnicastDefaultRequirement(AudioContext::MEDIA | AudioContext::SOUND_EFFECTS,
                                         true /* sink */, false /* source */)};
    aidl_retval = audio_provider_->getLeAudioAseConfiguration(
            sink_capabilities, std::nullopt, multi_context_sink_requirements, &configurations);
...
TEST_P(BluetoothAudioProviderLeAudioOutputHardwareAidl, GetOpusAseConfiguration) {
    if (GetProviderFactoryInterfaceVersion() < BluetoothAudioHalVersion::VERSION_AIDL_V4) {
        GTEST_SKIP();
    }
    ...
    std::vector<LeAudioAseConfigurationSetting> configurations;
    std::vector<LeAudioConfigurationRequirement> sink_requirements = {
            GetOpusUnicastRequirement(AudioContext::MEDIA, true /* sink */, false /* source */)};
    auto aidl_retval = audio_provider_->getLeAudioAseConfiguration(
...
TEST_P(BluetoothAudioProviderLeAudioOutputHardwareAidl, GetDsaAseConfiguration) {
    if (GetProviderFactoryInterfaceVersion() < BluetoothAudioHalVersion::VERSION_AIDL_V4) {
        GTEST_SKIP();
    }
    ...
    auto aidl_retval = audio_provider_->getLeAudioAseConfiguration(
            sink_capabilities, source_capabilities, sink_requirements, &configurations);
    ...
    for (auto& configuration : configurations) {
        ...
        ASSERT_TRUE(configuration.flags.value().bitmask & ConfigurationFlags::SPATIAL_AUDIO);
    }
}

TEST_P(BluetoothAudioProviderLeAudioOutputHardwareAidl, GetAseConfiguration_Multidirectional) {
    if (GetProviderFactoryInterfaceVersion() < BluetoothAudioHalVersion::VERSION_AIDL_V4) {
        GTEST_SKIP();
    }
    ...
    aidl_retval = audio_provider_->getLeAudioAseConfiguration(
            sink_capabilities, source_capabilities, combined_requirements, &configurations);
...
TEST_P(BluetoothAudioProviderLeAudioOutputHardwareAidl,
    ...
    auto aidl_retval = audio_provider_->getLeAudioAseConfiguration(
            sink_capabilities, source_capabilities, asymmetric_requirements, &configurations);
...
TEST_P(BluetoothAudioProviderLeAudioOutputHardwareAidl, GetQoSConfiguration_Multidirectional) {
    if (GetProviderFactoryInterfaceVersion() < BluetoothAudioHalVersion::VERSION_AIDL_V4) {
        GTEST_SKIP();
    }
    ...
    LeAudioAseQosConfigurationRequirement requirement = GetQosRequirements(true, true);

    std::vector<IBluetoothAudioProvider::LeAudioAseQosConfiguration> QoSConfigurations;
    ...
    for (auto bitmask : all_context_bitmasks) {
        ...
        auto aidl_retval = audio_provider_->getLeAudioAseQosConfiguration(requirement, &result);
        if (!aidl_retval.isOk()) {
...
TEST_P(BluetoothAudioProviderLeAudioOutputHardwareAidl, GetQoSConfiguration_InvalidRequirements) {
    if (GetProviderFactoryInterfaceVersion() < BluetoothAudioHalVersion::VERSION_AIDL_V4) {
        GTEST_SKIP();
    }
    auto allocation = CodecSpecificConfigurationLtv::AudioChannelAllocation();
    ...
    LeAudioAseQosConfigurationRequirement invalid_requirement =
            GetQosRequirements(true /* sink */, false /* source */, false /* valid */);
    ...
    for (auto bitmask : all_context_bitmasks) {
        ...
        IBluetoothAudioProvider::LeAudioAseQosConfigurationPair result;
        auto aidl_retval =
                audio_provider_->getLeAudioAseQosConfiguration(invalid_requirement, &result);
        ASSERT_FALSE(aidl_retval.isOk());
    }
}
...
TEST_P(BluetoothAudioProviderLeAudioOutputHardwareAidl,
    ...
    std::vector<LeAudioConfigurationRequirement> media_requirements = {
            ...
            GetUnicastDefaultRequirement(AudioContext::CONVERSATIONAL, true /* sink */,
                                         true /* source */,
                                         CodecSpecificConfigurationLtv::SamplingFrequency::HZ16000),
            GetUnicastDefaultRequirement(AudioContext::GAME, true /* sink */, true /* source */,
                                         CodecSpecificConfigurationLtv::SamplingFrequency::HZ16000),
    ...
    for (auto& configuration : configurations) {
        ...
        if (configuration.sinkAseConfiguration.has_value()) {
            for (auto& ase_direct_config : configuration.sinkAseConfiguration.value()) {
                if (!ase_direct_config) continue;
                if (ase_direct_config->qosConfiguration.has_value()) {
                    if (VerifyMaxSduForAbrCodec(ase_direct_config->qosConfiguration.value())) {
                        is_abr_codec = true;
                    }
                }
            }
        ...
        if (configuration.sourceAseConfiguration.has_value()) {
            for (auto& ase_direct_config : configuration.sourceAseConfiguration.value()) {
                if (!ase_direct_config) continue;
        ...
        if (is_abr_codec) {
            ...
            EXPECT_TRUE(configuration.flags->bitmask & ConfigurationFlags::ADAPTER_BIT_RATE);
        }
    }
}
    ...
    for (auto& configuration : configurations) {
        ...
        if (configuration.flags.has_value() &&
            (configuration.flags->bitmask &
             (ConfigurationFlags::ADAPTER_BIT_RATE | ConfigurationFlags::ISO_PARAMETER_UPDATE)))
        ...
        if (!configuration.latencySetting->suggestedLatencyRules.has_value()) continue;

        for (auto& rules : configuration.latencySetting->suggestedLatencyRules.value()) {
        ...
        ASSERT_EQ(last, flag_bitmasks.end()) << "LatencySettings should not contain duplicates";
    }
}

TEST_P(BluetoothAudioProviderLeAudioOutputHardwareAidl, GetQoSConfiguration_AbrConfiguration) {
    if (GetProviderFactoryInterfaceVersion() < BluetoothAudioHalVersion::VERSION_AIDL_V6) {
        GTEST_SKIP();
    }
...
TEST_P(BluetoothAudioProviderLeAudioOutputHardwareAidl, GetDataPathConfiguration_Multidirectional) {
    IBluetoothAudioProvider::StreamConfig sink_requirement;
    IBluetoothAudioProvider::StreamConfig source_requirement;
    std::vector<IBluetoothAudioProvider::LeAudioDataPathConfiguration> DataPathConfigurations;
    ...
    streamMap.aseConfiguration = {
            .targetLatency = LeAudioAseConfiguration::TargetLatency::BALANCED_LATENCY_RELIABILITY,
            .codecId = CodecId::Core::LC3,
            .codecConfiguration = {CodecSpecificConfigurationLtv::SamplingFrequency::HZ16000,
                                   CodecSpecificConfigurationLtv::FrameDuration::US10000,
                                   allocation},
    };
...
TEST_P(BluetoothAudioProviderLeAudioOutputHardwareAidl, GetDataPathConfiguration) {
    if (GetProviderFactoryInterfaceVersion() < BluetoothAudioHalVersion::VERSION_AIDL_V4) {
        GTEST_SKIP();
    }
    IBluetoothAudioProvider::StreamConfig sink_requirement;
...
TEST_P(BluetoothAudioProviderLeAudioOutputHardwareAidl,
    ...
    auto lc3_codec_configs = GetUnicastLc3SupportedList(false /* decoding */, true /* supported */);
    LeAudioConfiguration le_audio_config = {
            .codecType = CodecType::LC3,
            .peerDelayUs = 0,
    };
...
TEST_P(BluetoothAudioProviderLeAudioOutputHardwareAidl,
    ...
    for (auto codec_type : {CodecType::APTX_ADAPTIVE_LE, CodecType::APTX_ADAPTIVE_LEX}) {
        ...
        for (auto& aptx_adaptive_le_config : aptx_adaptive_le_codec_configs) {
            le_audio_config.leAudioCodecConfig.set<LeAudioCodecConfiguration::aptxAdaptiveLeConfig>(
...
class BluetoothAudioProviderLeAudioInputHardwareAidl
  ...
  public:
    virtual void SetUp() override {
        BluetoothAudioProviderFactoryAidl::SetUp();
        GetProviderCapabilitiesHelper(SessionType::LE_AUDIO_HARDWARE_OFFLOAD_DECODING_DATAPATH);
        GetProviderInfoHelper(SessionType::LE_AUDIO_HARDWARE_OFFLOAD_DECODING_DATAPATH);
        OpenProviderHelper(SessionType::LE_AUDIO_HARDWARE_OFFLOAD_DECODING_DATAPATH);
        ASSERT_TRUE(temp_provider_capabilities_.empty() || audio_provider_ != nullptr);
    }
    ...
    bool IsOffloadInputSupported() {
        for (auto& capability : temp_provider_capabilities_) {
            ...
            auto& le_audio_capability = capability.get<AudioCapabilities::leAudioCapabilities>();
            if (le_audio_capability.unicastDecodeCapability.codecType != CodecType::UNKNOWN)
                return true;
        }
        return false;
    }
 ...
 * stopped
 */
TEST_P(BluetoothAudioProviderLeAudioInputHardwareAidl, OpenLeAudioInputHardwareProvider) {}
...
TEST_P(BluetoothAudioProviderLeAudioInputHardwareAidl,
    ...
    auto lc3_codec_configs = GetUnicastLc3SupportedList(true /* decoding */, true /* supported */);
    LeAudioConfiguration le_audio_config = {
            .codecType = CodecType::LC3,
            .peerDelayUs = 0,
    };
...
TEST_P(BluetoothAudioProviderLeAudioInputHardwareAidl,
    ...
    auto lc3_codec_configs = GetUnicastLc3SupportedList(true /* decoding */, false /* supported */);
    LeAudioConfiguration le_audio_config = {
            .codecType = CodecType::LC3,
            .peerDelayUs = 0,
    };
...
TEST_P(BluetoothAudioProviderLeAudioInputHardwareAidl, GetEmptyAseConfigurationEmptyCapability) {
    if (GetProviderFactoryInterfaceVersion() < BluetoothAudioHalVersion::VERSION_AIDL_V4) {
        GTEST_SKIP();
    }
...
TEST_P(BluetoothAudioProviderLeAudioInputHardwareAidl, GetAseConfiguration_Multidirectional) {
    if (GetProviderFactoryInterfaceVersion() < BluetoothAudioHalVersion::VERSION_AIDL_V4) {
        GTEST_SKIP();
    }
...
TEST_P(BluetoothAudioProviderLeAudioInputHardwareAidl, GetQoSConfiguration_InvalidRequirements) {
    if (GetProviderFactoryInterfaceVersion() < BluetoothAudioHalVersion::VERSION_AIDL_V4) {
        GTEST_SKIP();
    }
    auto allocation = CodecSpecificConfigurationLtv::AudioChannelAllocation();
    ...
    LeAudioAseQosConfigurationRequirement invalid_requirement =
            GetQosRequirements(false /* sink */, true /* source */, false /* valid */);
...
class BluetoothAudioProviderLeAudioBroadcastOutputSoftwareAidl
  ...
  public:
    virtual void SetUp() override {
        BluetoothAudioProviderFactoryAidl::SetUp();
        GetProviderCapabilitiesHelper(SessionType::LE_AUDIO_BROADCAST_SOFTWARE_ENCODING_DATAPATH);
...
class BluetoothAudioProviderLeAudioBroadcastInputSoftwareAidl
  ...
  public:
    virtual void SetUp() override {
        ...
        BluetoothAudioProviderFactoryAidl::SetUp();
        GetProviderCapabilitiesHelper(SessionType::LE_AUDIO_BROADCAST_SOFTWARE_DECODING_DATAPATH);
        OpenProviderHelper(SessionType::LE_AUDIO_BROADCAST_SOFTWARE_DECODING_DATAPATH);
        ASSERT_NE(audio_provider_, nullptr);
    }
  ...
  public:
    virtual void SetUp() override {
        ...
        GetProviderCapabilitiesHelper(
                SessionType::LE_AUDIO_BROADCAST_HARDWARE_OFFLOAD_ENCODING_DATAPATH);
        GetProviderInfoHelper(SessionType::LE_AUDIO_BROADCAST_HARDWARE_OFFLOAD_ENCODING_DATAPATH);
        OpenProviderHelper(SessionType::LE_AUDIO_BROADCAST_HARDWARE_OFFLOAD_ENCODING_DATAPATH);
        ASSERT_TRUE(temp_provider_capabilities_.empty() || audio_provider_ != nullptr);
    }
    ...
    bool IsBroadcastOffloadSupported() {
        for (auto& capability : temp_provider_capabilities_) {
            ...
            auto& le_audio_capability = capability.get<AudioCapabilities::leAudioCapabilities>();
            if (le_audio_capability.broadcastCapability.codecType != CodecType::UNKNOWN)
                return true;
        }
        return false;
    }
    ...
    std::optional<CodecSpecificConfigurationLtv::SamplingFrequency> GetBisSampleFreq(
            const LeAudioBisConfiguration& bis_conf) {
        auto sample_freq_ltv = GetConfigurationLtv(
                bis_conf.codecConfiguration, CodecSpecificConfigurationLtv::Tag::samplingFrequency);
        if (!sample_freq_ltv) {
            return std::nullopt;
        }
        return (*sample_freq_ltv).get<CodecSpecificConfigurationLtv::samplingFrequency>();
    }

    std::vector<CodecSpecificConfigurationLtv::SamplingFrequency> GetSubgroupSampleFreqs(
    ...
    void VerifyBroadcastConfiguration(const LeAudioBroadcastConfigurationRequirement& requirements,
        ...
        for (const auto& subgroup_req : requirements.subgroupConfigurationRequirements) {
    ...
    LeAudioBroadcastConfigurationRequirement GetBroadcastRequirement(bool standard_quality,
                                                                     bool high_quality) {
        ...
        LeAudioBroadcastSubgroupConfigurationRequirement standard_quality_requirement = {
                .audioContext = media_audio_context,
        ...
        if (standard_quality) {
            requirement.subgroupConfigurationRequirements.push_back(standard_quality_requirement);
        }

        if (high_quality) {
            requirement.subgroupConfigurationRequirements.push_back(high_quality_requirement);
        }
        return requirement;
    }
    ...
    std::vector<Lc3Configuration> GetBroadcastLc3SupportedList(bool supported) {
        ...
        for (auto& capability : temp_provider_capabilities_) {
            ...
            auto& lc3_capability =
                    broadcast_capability.leAudioCodecCapabilities
...
TEST_P(BluetoothAudioProviderLeAudioBroadcastOutputHardwareAidl,
    ...
    for (auto& lc3_config : lc3_codec_configs) {
        ...
        le_audio_broadcast_config.streamMap[0]
                .leAudioCodecConfig.set<LeAudioCodecConfiguration::lc3Config>(lc3_config);
        ...
        DataMQDesc mq_desc;
        auto aidl_retval = audio_provider_->startSession(
                audio_port_, AudioConfiguration(le_audio_broadcast_config), latency_modes,
                &mq_desc);
...
TEST_P(BluetoothAudioProviderLeAudioBroadcastOutputHardwareAidl,
    ...
    std::vector<std::optional<LeAudioDeviceCapabilities>> empty_capability;
    IBluetoothAudioProvider::LeAudioBroadcastConfigurationRequirement empty_requirement;
...
TEST_P(BluetoothAudioProviderLeAudioBroadcastOutputHardwareAidl,
    ...
    IBluetoothAudioProvider::LeAudioBroadcastConfigurationRequirement one_subgroup_requirement =
...
TEST_P(BluetoothAudioProviderLeAudioBroadcastOutputHardwareAidl,
    ...
    IBluetoothAudioProvider::LeAudioBroadcastConfigurationRequirement requirement =
            GetBroadcastRequirement(true /* standard*/, false /* high */);

    IBluetoothAudioProvider::LeAudioBroadcastConfigurationSetting configuration;

    // Check empty capability for source direction
    auto aidl_retval = audio_provider_->getLeAudioBroadcastConfiguration(capability, requirement,
                                                                         &configuration);
...
class BluetoothAudioProviderLeAudioBroadcastInputHardwareAidl
  ...
  public:
    virtual void SetUp() override {
        ...
        BluetoothAudioProviderFactoryAidl::SetUp();
        GetProviderInfoHelper(SessionType::LE_AUDIO_BROADCAST_HARDWARE_OFFLOAD_DECODING_DATAPATH);
        OpenProviderHelper(SessionType::LE_AUDIO_BROADCAST_HARDWARE_OFFLOAD_DECODING_DATAPATH);
 ...
 * started and stopped
 */
TEST_P(BluetoothAudioProviderLeAudioBroadcastInputHardwareAidl, OpenLeAudioOutputHardwareProvider) {
}
 ...
 */
class BluetoothAudioProviderA2dpDecodingSoftwareAidl : public BluetoothAudioProviderFactoryAidl {
  public:
    virtual void SetUp() override {
        BluetoothAudioProviderFactoryAidl::SetUp();
 ...
 * Test whether we can open a provider of type
 */
TEST_P(BluetoothAudioProviderA2dpDecodingSoftwareAidl, OpenA2dpDecodingSoftwareProvider) {}
 ...
 */
class BluetoothAudioProviderA2dpDecodingHardwareAidl : public BluetoothAudioProviderFactoryAidl {
  public:
    virtual void SetUp() override {
        BluetoothAudioProviderFactoryAidl::SetUp();
        GetProviderCapabilitiesHelper(SessionType::A2DP_HARDWARE_OFFLOAD_DECODING_DATAPATH);
 ...
 * Test whether we can open a provider of type
 */
TEST_P(BluetoothAudioProviderA2dpDecodingHardwareAidl, OpenA2dpDecodingHardwareProvider) {}
 ...
 * with SBC hardware encoding config
 */
TEST_P(BluetoothAudioProviderA2dpDecodingHardwareAidl, StartAndEndA2dpSbcDecodingHardwareSession) {
    if (!IsOffloadSupported()) {
        GTEST_SKIP();
    }
 ...
 * with AAC hardware encoding config
 */
TEST_P(BluetoothAudioProviderA2dpDecodingHardwareAidl, StartAndEndA2dpAacDecodingHardwareSession) {
    if (!IsOffloadSupported()) {
        GTEST_SKIP();
    }
 ...
 * with LDAC hardware encoding config
 */
TEST_P(BluetoothAudioProviderA2dpDecodingHardwareAidl, StartAndEndA2dpLdacDecodingHardwareSession) {
    if (!IsOffloadSupported()) {
        GTEST_SKIP();
    }
 ...
 * with Opus hardware encoding config
 */
TEST_P(BluetoothAudioProviderA2dpDecodingHardwareAidl, StartAndEndA2dpOpusDecodingHardwareSession) {
    if (!IsOffloadSupported()) {
        GTEST_SKIP();
    }
 ...
 * with AptX hardware encoding config
 */
TEST_P(BluetoothAudioProviderA2dpDecodingHardwareAidl, StartAndEndA2dpAptxDecodingHardwareSession) {
    if (!IsOffloadSupported()) {
        GTEST_SKIP();
    }
...
class BluetoothAudioProviderLeAudioPeripheralOutputOffloadAidl
  ...
  public:
    virtual void SetUp() override {
        ...
        GetProviderInfoHelper(SessionType::LE_AUDIO_PERIPHERAL_OFFLOAD_ENCODING_DATAPATH);
        OpenProviderHelper(SessionType::LE_AUDIO_PERIPHERAL_OFFLOAD_ENCODING_DATAPATH);
        ASSERT_TRUE(audio_provider_ != nullptr);
    }
    ...
    bool IsUnicastPeripheralOffloadOutputProviderInfoSupported() {
        if (temp_provider_info_.has_value() && !temp_provider_info_.value().codecInfos.empty()) {
            for (auto& codec_info : temp_provider_info_.value().codecInfos) {
...
TEST_P(BluetoothAudioProviderLeAudioPeripheralOutputOffloadAidl,
    ...
    auto test_sampling_rate = CodecSpecificConfigurationLtv::SamplingFrequency::HZ16000;
    auto test_frame_duration = CodecSpecificConfigurationLtv::FrameDuration::US10000;
    ...
    ASSERT_TRUE(aidl_retval.isOk());
    ASSERT_TRUE(codecConfiguredResponse->sinkAseCodecConfiguredParams->size() == 2);
    ASSERT_TRUE(codecConfiguredResponse->sourceAseCodecConfiguredParams->size() == 1);
}
...
TEST_P(BluetoothAudioProviderLeAudioPeripheralOutputOffloadAidl,
    ...
    for (auto& codec_info : temp_provider_info_.value().codecInfos) {
        ASSERT_TRUE(codec_info.transport.getTag() == CodecInfo::Transport::leAudioPeripheral);
        ...
        for (auto& leaudio_peripheral_capa : peripheral_info.codecCapabilities) {
            ...
            ASSERT_TRUE(leaudio_peripheral_capa.vendorCapabilities->vendorCodecSpecificCapabilities
                                .size() > 0)
...
TEST_P(BluetoothAudioProviderLeAudioPeripheralOutputOffloadAidl,
    ...
    for (const auto& codec_info : provider_info.codecInfos) {
        ASSERT_EQ(codec_info.transport.getTag(), CodecInfo::Transport::leAudioPeripheral);
        ...
        auto last_unique = std::unique(capabilities_copy.begin(), capabilities_copy.end());

        const size_t unique_count = std::distance(capabilities_copy.begin(), last_unique);

        EXPECT_EQ(initial_size, unique_count)
                << "The codecCapabilities array contains duplicate "
                   "LeAudioPeripheralCapabilities"
                << ". Expected size: " << initial_size << ", Actual unique size: " << unique_count;
    }
}
...
class BluetoothAudioProviderLeAudioPeripheralInputOffloadAidl
  ...
  public:
    virtual void SetUp() override {
        ...
        GetProviderInfoHelper(SessionType::LE_AUDIO_PERIPHERAL_OFFLOAD_DECODING_DATAPATH);
        OpenProviderHelper(SessionType::LE_AUDIO_PERIPHERAL_OFFLOAD_DECODING_DATAPATH);
        ASSERT_TRUE(audio_provider_ != nullptr);
    }
...
GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(BluetoothAudioProviderFactoryAidl);
...
GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(BluetoothAudioProviderA2dpEncodingSoftwareAidl);
INSTANTIATE_TEST_SUITE_P(PerInstance, BluetoothAudioProviderA2dpEncodingSoftwareAidl,
                         testing::ValuesIn(android::getAidlHalInstanceNames(
                                 IBluetoothAudioProviderFactory::descriptor)),
                         android::PrintInstanceNameToString);

GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(BluetoothAudioProviderA2dpEncodingHardwareAidl);
INSTANTIATE_TEST_SUITE_P(PerInstance, BluetoothAudioProviderA2dpEncodingHardwareAidl,
                         testing::ValuesIn(android::getAidlHalInstanceNames(
                                 IBluetoothAudioProviderFactory::descriptor)),
                         android::PrintInstanceNameToString);

GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(BluetoothAudioProviderHearingAidSoftwareAidl);
INSTANTIATE_TEST_SUITE_P(PerInstance, BluetoothAudioProviderHearingAidSoftwareAidl,
                         testing::ValuesIn(android::getAidlHalInstanceNames(
                                 IBluetoothAudioProviderFactory::descriptor)),
                         android::PrintInstanceNameToString);

GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(BluetoothAudioProviderLeAudioOutputSoftwareAidl);
INSTANTIATE_TEST_SUITE_P(PerInstance, BluetoothAudioProviderLeAudioOutputSoftwareAidl,
                         testing::ValuesIn(android::getAidlHalInstanceNames(
                                 IBluetoothAudioProviderFactory::descriptor)),
                         android::PrintInstanceNameToString);

GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(BluetoothAudioProviderLeAudioInputSoftwareAidl);
INSTANTIATE_TEST_SUITE_P(PerInstance, BluetoothAudioProviderLeAudioInputSoftwareAidl,
                         testing::ValuesIn(android::getAidlHalInstanceNames(
                                 IBluetoothAudioProviderFactory::descriptor)),
                         android::PrintInstanceNameToString);

GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(BluetoothAudioProviderLeAudioOutputHardwareAidl);
INSTANTIATE_TEST_SUITE_P(PerInstance, BluetoothAudioProviderLeAudioOutputHardwareAidl,
                         testing::ValuesIn(android::getAidlHalInstanceNames(
                                 IBluetoothAudioProviderFactory::descriptor)),
                         android::PrintInstanceNameToString);

GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(BluetoothAudioProviderLeAudioInputHardwareAidl);
INSTANTIATE_TEST_SUITE_P(PerInstance, BluetoothAudioProviderLeAudioInputHardwareAidl,
                         testing::ValuesIn(android::getAidlHalInstanceNames(
                                 IBluetoothAudioProviderFactory::descriptor)),
                         android::PrintInstanceNameToString);
...
INSTANTIATE_TEST_SUITE_P(PerInstance, BluetoothAudioProviderLeAudioBroadcastOutputSoftwareAidl,
                         testing::ValuesIn(android::getAidlHalInstanceNames(
                                 IBluetoothAudioProviderFactory::descriptor)),
                         android::PrintInstanceNameToString);
...
INSTANTIATE_TEST_SUITE_P(PerInstance, BluetoothAudioProviderLeAudioBroadcastInputSoftwareAidl,
                         testing::ValuesIn(android::getAidlHalInstanceNames(
                                 IBluetoothAudioProviderFactory::descriptor)),
                         android::PrintInstanceNameToString);
...
INSTANTIATE_TEST_SUITE_P(PerInstance, BluetoothAudioProviderLeAudioBroadcastOutputHardwareAidl,
                         testing::ValuesIn(android::getAidlHalInstanceNames(
                                 IBluetoothAudioProviderFactory::descriptor)),
                         android::PrintInstanceNameToString);
...
INSTANTIATE_TEST_SUITE_P(PerInstance, BluetoothAudioProviderLeAudioBroadcastInputHardwareAidl,
                         testing::ValuesIn(android::getAidlHalInstanceNames(
                                 IBluetoothAudioProviderFactory::descriptor)),
                         android::PrintInstanceNameToString);

GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(BluetoothAudioProviderA2dpDecodingSoftwareAidl);
INSTANTIATE_TEST_SUITE_P(PerInstance, BluetoothAudioProviderA2dpDecodingSoftwareAidl,
                         testing::ValuesIn(android::getAidlHalInstanceNames(
                                 IBluetoothAudioProviderFactory::descriptor)),
                         android::PrintInstanceNameToString);

GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(BluetoothAudioProviderA2dpDecodingHardwareAidl);
INSTANTIATE_TEST_SUITE_P(PerInstance, BluetoothAudioProviderA2dpDecodingHardwareAidl,
                         testing::ValuesIn(android::getAidlHalInstanceNames(
                                 IBluetoothAudioProviderFactory::descriptor)),
                         android::PrintInstanceNameToString);

GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(BluetoothAudioProviderHfpHardwareAidl);
INSTANTIATE_TEST_SUITE_P(PerInstance, BluetoothAudioProviderHfpHardwareAidl,
                         testing::ValuesIn(android::getAidlHalInstanceNames(
                                 IBluetoothAudioProviderFactory::descriptor)),
                         android::PrintInstanceNameToString);

GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(BluetoothAudioProviderHfpSoftwareDecodingAidl);
INSTANTIATE_TEST_SUITE_P(PerInstance, BluetoothAudioProviderHfpSoftwareDecodingAidl,
                         testing::ValuesIn(android::getAidlHalInstanceNames(
                                 IBluetoothAudioProviderFactory::descriptor)),
                         android::PrintInstanceNameToString);

GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(BluetoothAudioProviderHfpSoftwareEncodingAidl);
INSTANTIATE_TEST_SUITE_P(PerInstance, BluetoothAudioProviderHfpSoftwareEncodingAidl,
                         testing::ValuesIn(android::getAidlHalInstanceNames(
                                 IBluetoothAudioProviderFactory::descriptor)),
                         android::PrintInstanceNameToString);
...
INSTANTIATE_TEST_SUITE_P(PerInstance, BluetoothAudioProviderLeAudioPeripheralOutputOffloadAidl,
                         testing::ValuesIn(android::getAidlHalInstanceNames(
                                 IBluetoothAudioProviderFactory::descriptor)),
                         android::PrintInstanceNameToString);
...
INSTANTIATE_TEST_SUITE_P(PerInstance, BluetoothAudioProviderLeAudioPeripheralInputOffloadAidl,
                         testing::ValuesIn(android::getAidlHalInstanceNames(
                                 IBluetoothAudioProviderFactory::descriptor)),
                         android::PrintInstanceNameToString);
