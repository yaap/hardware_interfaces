...
static const PcmParameters kDefaultSoftwarePcmCapabilities = {
        ...
        .channelMode = static_cast<ChannelMode>(ChannelMode::MONO | ChannelMode::STEREO),
        .bitsPerSample = static_cast<BitsPerSample>(
                BitsPerSample::BITS_16 | BitsPerSample::BITS_24 | BitsPerSample::BITS_32)};
...
static const SbcParameters kDefaultOffloadSbcCapability = {
        ...
        .channelMode =
                static_cast<SbcChannelMode>(SbcChannelMode::MONO | SbcChannelMode::JOINT_STEREO),
        .blockLength =
                static_cast<SbcBlockLength>(SbcBlockLength::BLOCKS_4 | SbcBlockLength::BLOCKS_8 |
...
static const LdacParameters kDefaultOffloadLdacCapability = {
        .sampleRate = static_cast<SampleRate>(SampleRate::RATE_44100 | SampleRate::RATE_48000 |
                                              SampleRate::RATE_88200 | SampleRate::RATE_96000),
        .channelMode =
                static_cast<LdacChannelMode>(LdacChannelMode::DUAL | LdacChannelMode::STEREO),
...
static bool IsOffloadSbcConfigurationValid(const CodecConfiguration::CodecSpecific& codec_specific);
static bool IsOffloadAacConfigurationValid(const CodecConfiguration::CodecSpecific& codec_specific);
...
static bool IsOffloadSbcConfigurationValid(
    ...
    if (codec_specific.getDiscriminator() !=
        ...
        LOG(WARNING) << __func__ << ": Invalid CodecSpecific=" << toString(codec_specific);
        return false;
    }
    ...
    if (!IsSingleBit(static_cast<uint32_t>(sbc_data.sampleRate), 0xff) ||
        ...
        return false;
               ...
               (sbc_data.channelMode & kDefaultOffloadSbcCapability.channelMode) &&
               (sbc_data.blockLength & kDefaultOffloadSbcCapability.blockLength) &&
               (sbc_data.numSubbands & kDefaultOffloadSbcCapability.numSubbands) &&
               (sbc_data.allocMethod & kDefaultOffloadSbcCapability.allocMethod) &&
               (sbc_data.bitsPerSample & kDefaultOffloadSbcCapability.bitsPerSample) &&
               (kDefaultOffloadSbcCapability.minBitpool <= sbc_data.minBitpool &&
                sbc_data.maxBitpool <= kDefaultOffloadSbcCapability.maxBitpool)) {
        return true;
    }
    LOG(WARNING) << __func__ << ": Unsupported CodecSpecific=" << toString(codec_specific);
    return false;
}
...
static bool IsOffloadAacConfigurationValid(
    ...
    if (!IsSingleBit(static_cast<uint32_t>(aac_data.objectType), 0xf0) ||
        ...
        return false;
               ...
               (aac_data.channelMode & kDefaultOffloadAacCapability.channelMode) &&
               ...
               (aac_data.bitsPerSample & kDefaultOffloadAacCapability.bitsPerSample)) {
        return true;
    }
    LOG(WARNING) << __func__ << ": Unsupported CodecSpecific=" << toString(codec_specific);
    return false;
...
static bool IsOffloadLdacConfigurationValid(
    ...
    if (!IsSingleBit(static_cast<uint32_t>(ldac_data.sampleRate), 0xff) ||
        ...
        LOG(WARNING) << __func__ << ": Invalid CodecSpecific=" << toString(codec_specific);
        return false;
    } else if ((ldac_data.sampleRate & kDefaultOffloadLdacCapability.sampleRate) &&
               (ldac_data.channelMode & kDefaultOffloadLdacCapability.channelMode) &&
               (ldac_data.bitsPerSample & kDefaultOffloadLdacCapability.bitsPerSample)) {
        return true;
    }
    LOG(WARNING) << __func__ << ": Unsupported CodecSpecific=" << toString(codec_specific);
    return false;
...
static bool IsOffloadAptxConfigurationValid(
    ...
    if (!IsSingleBit(static_cast<uint32_t>(aptx_data.sampleRate), 0xff) ||
        ...
        LOG(WARNING) << __func__ << ": Invalid CodecSpecific=" << toString(codec_specific);
        return false;
    } else if ((aptx_data.sampleRate & kDefaultOffloadAptxCapability.sampleRate) &&
               (aptx_data.channelMode & kDefaultOffloadAptxCapability.channelMode) &&
               (aptx_data.bitsPerSample & kDefaultOffloadAptxCapability.bitsPerSample)) {
        return true;
    }
    LOG(WARNING) << __func__ << ": Unsupported CodecSpecific=" << toString(codec_specific);
    return false;
...
static bool IsOffloadAptxHdConfigurationValid(
    ...
    if (!IsSingleBit(static_cast<uint32_t>(aptx_data.sampleRate), 0xff) ||
        ...
        LOG(WARNING) << __func__ << ": Invalid CodecSpecific=" << toString(codec_specific);
        return false;
    } else if ((aptx_data.sampleRate & kDefaultOffloadAptxHdCapability.sampleRate) &&
               (aptx_data.channelMode & kDefaultOffloadAptxHdCapability.channelMode) &&
               (aptx_data.bitsPerSample & kDefaultOffloadAptxHdCapability.bitsPerSample)) {
        return true;
    }
    LOG(WARNING) << __func__ << ": Unsupported CodecSpecific=" << toString(codec_specific);
    return false;
...
bool IsSoftwarePcmConfigurationValid(const PcmParameters& pcm_config) {
    if ((pcm_config.sampleRate != SampleRate::RATE_44100 &&
        ...
        LOG(WARNING) << __func__ << ": Invalid PCM Configuration=" << toString(pcm_config);
        return false;
    } else if (pcm_config.sampleRate & kDefaultSoftwarePcmCapabilities.sampleRate &&
               pcm_config.bitsPerSample & kDefaultSoftwarePcmCapabilities.bitsPerSample &&
               pcm_config.channelMode & kDefaultSoftwarePcmCapabilities.channelMode) {
        return true;
    }
    LOG(WARNING) << __func__ << ": Unsupported PCM Configuration=" << toString(pcm_config);
    return false;
}
...
bool IsOffloadCodecConfigurationValid(const SessionType& session_type,
    ...
    if (session_type != SessionType::A2DP_HARDWARE_OFFLOAD_DATAPATH) {
        ...
        LOG(ERROR) << __func__ << ": Unsupported Codec Configuration=" << toString(codec_config);
        return false;
    }
