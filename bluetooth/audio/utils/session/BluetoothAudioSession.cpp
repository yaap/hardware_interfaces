...
static inline timespec timespec_convert_from_hal(const TimeSpec& TS) {
    return {.tv_sec = static_cast<long>(TS.tvSec), .tv_nsec = static_cast<long>(TS.tvNSec)};
}
...
void BluetoothAudioSession::OnSessionStarted(const sp<IBluetoothAudioPort> stack_iface,
    ...
    if (stack_iface == nullptr) {
        ...
        LOG(ERROR) << __func__ << " - SessionType=" << toString(session_type_)
                   << ", AudioConfiguration=" << toString(audio_config) << " Invalid";
    } else if (!UpdateDataPath(dataMQ)) {
        LOG(ERROR) << __func__ << " - SessionType=" << toString(session_type_) << " DataMQ Invalid";
...
void BluetoothAudioSession::ReportSessionStatus() {
    ...
    for (auto& observer : observers_) {
        ...
        LOG(INFO) << __func__ << " - SessionType=" << toString(session_type_)
                  << " notify to bluetooth_audio=0x" << android::base::StringPrintf("%04x", cookie);
...
bool BluetoothAudioSession::IsSessionReady() {
    ...
    std::lock_guard<std::recursive_mutex> guard(mutex_);
    bool dataMQ_valid = (session_type_ == SessionType::A2DP_HARDWARE_OFFLOAD_DATAPATH ||
...
bool BluetoothAudioSession::UpdateAudioConfig(const AudioConfiguration& audio_config) {
    ...
    bool is_offload_session = (session_type_ == SessionType::A2DP_HARDWARE_OFFLOAD_DATAPATH);
...
uint16_t BluetoothAudioSession::RegisterStatusCback(const PortStatusCallbacks& cbacks) {
    if (HidlToAidlMiddleware_2_0::IsAidlAvailable())
        return HidlToAidlMiddleware_2_0::RegisterControlResultCback(session_type_, cbacks);
    std::lock_guard<std::recursive_mutex> guard(mutex_);
    ...
    std::shared_ptr<struct PortStatusCallbacks> cb = std::make_shared<struct PortStatusCallbacks>();
    *cb = cbacks;
...
void BluetoothAudioSession::UnregisterStatusCback(uint16_t cookie) {
    if (HidlToAidlMiddleware_2_0::IsAidlAvailable())
        return HidlToAidlMiddleware_2_0::UnregisterControlResultCback(session_type_, cookie);
    std::lock_guard<std::recursive_mutex> guard(mutex_);
...
const AudioConfiguration& BluetoothAudioSession::GetAudioConfig() {
    if (HidlToAidlMiddleware_2_0::IsAidlAvailable())
        return (audio_config_ = HidlToAidlMiddleware_2_0::GetAudioConfig(session_type_));
    std::lock_guard<std::recursive_mutex> guard(mutex_);
    if (IsSessionReady()) {
...
bool BluetoothAudioSession::GetPresentationPosition(uint64_t* remote_delay_report_ns,
                                                    uint64_t* total_bytes_readed,
                                                    timespec* data_position) {
    if (HidlToAidlMiddleware_2_0::IsAidlAvailable())
        return HidlToAidlMiddleware_2_0::GetPresentationPosition(
                session_type_, remote_delay_report_ns, total_bytes_readed, data_position);
    std::lock_guard<std::recursive_mutex> guard(mutex_);
    if (!IsSessionReady()) {
        LOG(DEBUG) << __func__ << " - SessionType=" << toString(session_type_) << " has NO session";
        return false;
    ...
    auto hal_retval = stack_iface_->getPresentationPosition(
            [&retval, &remote_delay_report_ns, &total_bytes_readed, &data_position](
                    BluetoothAudioStatus status, const uint64_t& remoteDeviceAudioDelayNanos,
                    uint64_t transmittedOctets, const TimeSpec& transmittedOctetsTimeStamp) {
                if (status == BluetoothAudioStatus::SUCCESS) {
                    if (remote_delay_report_ns)
                        *remote_delay_report_ns = remoteDeviceAudioDelayNanos;
                    if (total_bytes_readed) *total_bytes_readed = transmittedOctets;
                    if (data_position)
                        *data_position = timespec_convert_from_hal(transmittedOctetsTimeStamp);
                    retval = true;
                }
            });
    if (!hal_retval.isOk()) {
...
void BluetoothAudioSession::UpdateTracksMetadata(const struct source_metadata* source_metadata) {
    if (HidlToAidlMiddleware_2_0::IsAidlAvailable())
        return HidlToAidlMiddleware_2_0::UpdateTracksMetadata(session_type_, source_metadata);
    std::lock_guard<std::recursive_mutex> guard(mutex_);
    if (!IsSessionReady()) {
        LOG(DEBUG) << __func__ << " - SessionType=" << toString(session_type_) << " has NO session";
        return;
...
size_t BluetoothAudioSession::OutWritePcmData(const void* buffer, size_t bytes) {
    if (HidlToAidlMiddleware_2_0::IsAidlAvailable())
        return HidlToAidlMiddleware_2_0::OutWritePcmData(session_type_, buffer, bytes);
...
size_t BluetoothAudioSession::InReadPcmData(void* buffer, size_t bytes) {
    if (HidlToAidlMiddleware_2_0::IsAidlAvailable())
        return HidlToAidlMiddleware_2_0::InReadPcmData(session_type_, buffer, bytes);
    if (buffer == nullptr || !bytes) return 0;
    size_t totalRead = 0;
    ...
    do {
        ...
        if (availableToRead) {
            ...
            if (!mDataMQ->read(static_cast<uint8_t*>(buffer) + totalRead, availableToRead)) {
...
std::unique_ptr<BluetoothAudioSessionInstance> BluetoothAudioSessionInstance::instance_ptr =
        std::unique_ptr<BluetoothAudioSessionInstance>(new BluetoothAudioSessionInstance());
