...
static const char* kHfpCodecCapabilitiesFile = "/vendor/etc/aidl/hfp/hfp_codec_capabilities.xml";

std::optional<HfpOffloadSetting> BluetoothHfpCodecsProvider::ParseFromHfpOffloadSettingFile() {
    auto hfp_offload_setting = hfp::setting::readHfpOffloadSetting(kHfpCodecCapabilitiesFile);
...
std::vector<CodecInfo> BluetoothHfpCodecsProvider::GetHfpAudioCodecInfo(
    ...
    for (const auto& cfg : hfp_offload_setting.value().getConfiguration()) {
        ...
        auto output_path_cfg = path_config_map.find(cfg.getOutputPathConfiguration());
        ...
        codec_info.transport = CodecInfo::Transport::make<CodecInfo::Transport::Tag::hfp>();

        auto& transport = codec_info.transport.get<CodecInfo::Transport::Tag::hfp>();
