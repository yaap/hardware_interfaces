  ...
  public:
    ...
    static void OnSessionStarted(const SessionType& session_type,
        ...
        if (session_ptr != nullptr) {
            session_ptr->OnSessionStarted(host_iface, data_mq, audio_config, latency_modes);
        }
    }
     ...
     ***/
    static void ReportControlStatus(const SessionType& session_type, const bool& start_resp,
     ...
     ***/
    static void ReportLowLatencyModeAllowedChanged(const SessionType& session_type, bool allowed) {
        std::shared_ptr<BluetoothAudioSession> session_ptr =
                BluetoothAudioSessionInstance::GetSessionInstance(session_type);
        if (session_ptr != nullptr) {
