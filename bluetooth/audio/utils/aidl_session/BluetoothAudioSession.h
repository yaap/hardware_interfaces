...
using DataMQDesc = ::aidl::android::hardware::common::fmq::MQDescriptor<MQDataType, MQDataMode>;
...
inline uint16_t ObserversCookieGetUpperBound(SessionType session_type) {
    return (static_cast<uint16_t>(session_type) << 8 & 0xff00) + kObserversCookieSize;
}
...
class BluetoothAudioSession {
  ...
  private:
    ...
    std::unordered_map<uint16_t, std::shared_ptr<struct PortStatusCallbacks>> observers_;
...
class BluetoothAudioSessionInstance {
  ...
  private:
    ...
    static std::unordered_map<SessionType, std::shared_ptr<BluetoothAudioSession>> sessions_map_;
};
