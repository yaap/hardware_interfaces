...
A2dpOffloadCodecAac::A2dpOffloadCodecAac()
    : A2dpOffloadCodec(info_), info_({.id = CodecId(CodecId::A2dp::AAC), .name = "AAC"}) {
...
A2dpStatus A2dpOffloadCodecAac::SelectA2dpConfiguration(const std::vector<uint8_t>& configuration,
    ...
    if (aac_parameters) aac_parameters->object_type = GetObjectTypeEnum(object_type);

    return A2dpStatus::OK;
}

bool A2dpOffloadCodecAac::BuildConfiguration(const std::vector<uint8_t>& remote_capabilities,
