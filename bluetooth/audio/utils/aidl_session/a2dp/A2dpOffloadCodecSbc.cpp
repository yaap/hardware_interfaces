...
enum { kBlockLength4 = kBlockLength.first, kBlockLength8, kBlockLength12, kBlockLength16 };

enum { kSubbands4 = kSubbands.first, kSubbands8 };

enum { kAllocationMethodSnr = kAllocationMethod.first, kAllocationMethodLoudness };
...
static int GetFrameSize(const A2dpBits& configuration, int bitpool) {
    ...
    unsigned bits = ((4 * subbands) << !configuration.get(kChannelModeMono)) +
...
A2dpOffloadCodecSbc::A2dpOffloadCodecSbc()
    : A2dpOffloadCodec(info_), info_({.id = CodecId(CodecId::A2dp::SBC), .name = "SBC"}) {
    ...
    for (auto v : {16000, 32000, 44100, 48000})
        if (capabilities.get(GetSamplingFrequencyBit(int32_t(v)))) sampling_frequency.push_back(v);
...
bool A2dpOffloadCodecSbc::BuildConfiguration(const std::vector<uint8_t>& remote_capabilities,
    ...
    if (min_bitpool < 2 || min_bitpool > 250 || max_bitpool < 2 || max_bitpool > 250 ||
