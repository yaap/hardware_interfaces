...
static constexpr int kFmqReceiveTimeoutMs = 1000;  // 1000 ms timeout for receiving
...
void BluetoothAudioSession::OnSessionStarted(const std::shared_ptr<IBluetoothAudioPort> stack_iface,
    ...
    if (stack_iface == nullptr) {
        ...
        LOG(ERROR) << __func__ << " - SessionType=" << toString(session_type_)
                   << ", AudioConfiguration=" << audio_config.toString() << " Invalid";
...
std::vector<CodecSpecificConfigurationLtv> getCodecConfigFromVendorCodecConfiguration(
    ...
    while (i < vendor_codec_config.size()) {
        ...
        if (opcode == kCodecConfigOpcode) {
            if (subopcode == kSamplingFrequencySubOpcode) {
                auto p = codec_cfg_map_to_sampling_rate_ltv.find(vendor_codec_config[i++]);
                if (p != codec_cfg_map_to_sampling_rate_ltv.end()) {
                    codec_config.push_back(p->second);
                }
            } else if (subopcode == kFrameDurationSubOpcode) {
                auto p = codec_cfg_map_to_frame_duration_ltv.find(vendor_codec_config[i++]);
                if (p != codec_cfg_map_to_frame_duration_ltv.end()) {
                    codec_config.push_back(p->second);
                }
            } else if (subopcode == kFrameBlocksPerSDUSubOpcode) {
                auto frame_block = CodecSpecificConfigurationLtv::CodecFrameBlocksPerSDU();
...
OpusConfiguration getOpusConfigFromCodecConfig(
    ...
    for (auto ltv : codecConfiguration) {
        if (ltv.getTag() == CodecSpecificConfigurationLtv::samplingFrequency) {
            ...
            auto block = ltv.get<CodecSpecificConfigurationLtv::codecFrameBlocksPerSDU>();
...
std::optional<AudioConfiguration> convertToOpusAudioConfiguration(
        const AudioConfiguration& audio_config_, std::optional<bool>& is_stream_active) {
    if (audio_config_.getTag() == AudioConfiguration::leAudioConfig) {
        is_stream_active = false;
        auto le_audio_config = audio_config_.get<AudioConfiguration::leAudioConfig>();
        // Conversion from extension to Lc3Configuration
        LOG(DEBUG) << __func__
                   << ": leAudioConfig detected, len = " << le_audio_config.streamMap.size();
        for (auto info : le_audio_config.streamMap) {
            ...
            if (info.aseConfiguration.has_value()) {
                ...
                if (codec_id.has_value() && codec_id.value().getTag() == CodecId::vendor) {
                    auto cid = codec_id.value().get<CodecId::vendor>();
                    if (cid == opus_codec && ase_config.vendorCodecConfiguration.has_value()) {
...
void BluetoothAudioSession::ReportAudioConfigChanged(const AudioConfiguration& audio_config) {
    std::lock_guard<std::recursive_mutex> guard(mutex_);
    if (session_type_ == SessionType::LE_AUDIO_HARDWARE_OFFLOAD_ENCODING_DATAPATH ||
    ...
    for (auto& observer : observers_) {
        ...
        LOG(INFO) << __func__ << " for SessionType=" << toString(session_type_)
                  << ", bluetooth_audio=0x" << ::android::base::StringPrintf("%04x", cookie);
...
uint16_t BluetoothAudioSession::RegisterStatusCback(const PortStatusCallbacks& callbacks) {
    std::lock_guard<std::recursive_mutex> guard(mutex_);
    ...
    if (cookie >= cookie_upper_bound) {
        LOG(ERROR) << __func__ << " - SessionType=" << toString(session_type_) << " has "
                   << observers_.size() << " observers already (No Resource)";
        return kObserversCookieUndefined;
    }
    std::shared_ptr<PortStatusCallbacks> cb = std::make_shared<PortStatusCallbacks>();
    *cb = callbacks;
...
void BluetoothAudioSession::UnregisterStatusCback(uint16_t cookie) {
    ...
    if (observers_.erase(cookie) != 1) {
        LOG(WARNING) << __func__ << " - SessionType=" << toString(session_type_)
                     << " no such provider=0x" << ::android::base::StringPrintf("%04x", cookie);
    }
}
...
bool BluetoothAudioSession::StartStream(bool is_low_latency) {
    std::lock_guard<std::recursive_mutex> guard(mutex_);
    if (!IsSessionReadyInternal()) {
        LOG(DEBUG) << __func__ << " - SessionType=" << toString(session_type_) << " has NO session";
        return false;
    }
    auto hal_retval = stack_iface_->startStream(is_low_latency);
    if (!hal_retval.isOk()) {
        LOG(WARNING) << __func__ << " - IBluetoothAudioPort SessionType=" << toString(session_type_)
                     << " failed";
        return false;
    }
    return true;
...
bool BluetoothAudioSession::UpdateAudioConfig(const AudioConfiguration& audio_config) {
    ...
    bool is_offload_a2dp_session =
            (session_type_ == SessionType::A2DP_HARDWARE_OFFLOAD_ENCODING_DATAPATH ||
             session_type_ == SessionType::A2DP_HARDWARE_OFFLOAD_DECODING_DATAPATH);
    bool is_offload_hfp_session = session_type_ == SessionType::HFP_HARDWARE_OFFLOAD_DATAPATH;
    ...
    bool is_a2dp_offload_audio_config =
            (is_offload_a2dp_session && (audio_config_tag == AudioConfiguration::a2dp ||
    ...
    if (!is_software_audio_config && !is_a2dp_offload_audio_config &&
        !is_hfp_offload_audio_config && !is_le_audio_offload_unicast_audio_config &&
...
size_t BluetoothAudioSession::OutWritePcmData(const void* buffer, size_t bytes) {
    ...
    if (session_type_ == SessionType::LE_AUDIO_HARDWARE_OFFLOAD_ENCODING_DATAPATH) {
        ...
        do {
            ...
            if (num_bytes_to_write) {
                ...
                if (!data_mq_->write(static_cast<const MQDataType*>(buffer) + total_written,
                                     num_bytes_to_write)) {
...
size_t BluetoothAudioSession::InReadPcmData(void* buffer, size_t bytes) {
    ...
    int poll_ms = com::android::btaudio::hal::flags::extend_thread_poll_time() ? kReadPollExtendedMs
                                                                               : kReadPollMs;
    do {
        std::unique_lock<std::recursive_mutex> lock(mutex_);
        if (!IsSessionReadyInternal()) {
        ...
        if (num_bytes_to_read) {
            ...
            if (!data_mq_->read(static_cast<MQDataType*>(buffer) + total_read, num_bytes_to_read)) {
                LOG(ERROR) << "FMQ datapath reading " << total_read << "/" << bytes << " failed";
                return total_read;
            }
...
void BluetoothAudioSession::ReportControlStatus(bool start_resp, BluetoothAudioStatus status) {
    std::lock_guard<std::recursive_mutex> guard(mutex_);
    if (observers_.empty()) {
        LOG(WARNING) << __func__ << " - SessionType=" << toString(session_type_)
                     << " has NO port state observer";
    ...
    for (auto& observer : observers_) {
        ...
        LOG(INFO) << __func__ << " - status=" << toString(status)
                  << " for SessionType=" << toString(session_type_) << ", bluetooth_audio=0x"
                  << ::android::base::StringPrintf("%04x", cookie)
...
void BluetoothAudioSession::ReportLowLatencyModeAllowedChanged(bool allowed) {
    ...
    if (low_latency_allowed_) {
        if (std::find(latency_modes_.begin(), latency_modes_.end(), LatencyMode::LOW_LATENCY) ==
            latency_modes_.end()) {
    ...
    for (auto& observer : observers_) {
        uint16_t cookie = observer.first;
        std::shared_ptr<PortStatusCallbacks> callback = observer.second;
        LOG(INFO) << __func__ << " - allowed=" << (allowed ? " allowed" : " disallowed");
...
bool BluetoothAudioSession::GetPresentationPosition(PresentationPosition& presentation_position) {
    std::lock_guard<std::recursive_mutex> guard(mutex_);
    if (!IsSessionReadyInternal()) {
        LOG(DEBUG) << __func__ << " - SessionType=" << toString(session_type_) << " has NO session";
        return false;
...
void BluetoothAudioSession::UpdateSourceMetadata(const struct source_metadata& source_metadata) {
    ssize_t track_count = source_metadata.track_count;
    LOG(INFO) << __func__ << " - SessionType=" << toString(session_type_) << "," << track_count
              << " track(s)";
    SourceMetadata hal_source_metadata;
    hal_source_metadata.tracks.resize(track_count);
    for (int i = 0; i < track_count; i++) {
        hal_source_metadata.tracks[i].usage =
                static_cast<media::audio::common::AudioUsage>(source_metadata.tracks[i].usage);
        ...
        LOG(VERBOSE) << __func__ << " - SessionType=" << toString(session_type_)
                     ...
                     << ", content=" << toString(hal_source_metadata.tracks[i].contentType)
                     << ", gain=" << hal_source_metadata.tracks[i].gain;
    }
    UpdateSourceMetadata(hal_source_metadata);
}

void BluetoothAudioSession::UpdateSinkMetadata(const struct sink_metadata& sink_metadata) {
    ...
    for (int i = 0; i < track_count; i++) {
        hal_sink_metadata.tracks[i].source =
                static_cast<media::audio::common::AudioSource>(sink_metadata.tracks[i].source);
        ...
        LOG(INFO) << __func__ << " - SessionType=" << toString(session_type_)
                  ...
                  << ", dest_device_address=" << sink_metadata.tracks[i].dest_device_address;
    }
    UpdateSinkMetadata(hal_sink_metadata);
}

bool BluetoothAudioSession::UpdateSourceMetadata(const SourceMetadata& hal_source_metadata) {
    std::lock_guard<std::recursive_mutex> guard(mutex_);
    if (!IsSessionReadyInternal()) {
        LOG(DEBUG) << __func__ << " - SessionType=" << toString(session_type_) << " has NO session";
        return false;
...
bool BluetoothAudioSession::UpdateSinkMetadata(const SinkMetadata& hal_sink_metadata) {
    std::lock_guard<std::recursive_mutex> guard(mutex_);
    if (!IsSessionReadyInternal()) {
        LOG(DEBUG) << __func__ << " - SessionType=" << toString(session_type_) << " has NO session";
        return false;
...
std::vector<LatencyMode> BluetoothAudioSession::GetSupportedLatencyModes() {
    ...
    LOG(DEBUG) << __func__ << " - Supported LatencyMode=" << toString(supported_latency_modes);
...
bool BluetoothAudioSession::IsAidlAvailable() {
    ...
    is_aidl_available = (AServiceManager_checkService(
...
std::shared_ptr<BluetoothAudioSession> BluetoothAudioSessionInstance::GetSessionInstance(
 ...
 *
 ***/
std::shared_ptr<LeAudioSwOffloadCallbacks> LeAudioSwOffloadInstance::sw_offload_cbacks_;
std::shared_ptr<swoff::LeAudioStream> LeAudioSwOffloadInstance::sw_offload_streams_;
