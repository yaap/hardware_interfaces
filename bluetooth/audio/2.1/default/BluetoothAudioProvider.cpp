...
void BluetoothAudioDeathRecipient::serviceDied(
        uint64_t cookie __unused, const wp<::android::hidl::base::V1_0::IBase>& who __unused) {
    LOG(ERROR) << "BluetoothAudioDeathRecipient::" << __func__ << " - BluetoothAudio Service died";
...
Return<void> BluetoothAudioProvider::startSession(const sp<IBluetoothAudioPort>& hostIf,
                                                  const V2_0::AudioConfiguration& audioConfig,
                                                  startSession_cb _hidl_cb) {
    AudioConfiguration audioConfig_2_1;

    if (audioConfig.getDiscriminator() == V2_0::AudioConfiguration::hidl_discriminator::pcmConfig) {
...
Return<void> BluetoothAudioProvider::startSession_2_1(const sp<IBluetoothAudioPort>& hostIf,
...
Return<void> BluetoothAudioProvider::streamStarted(BluetoothAudioStatus status) {
     ...
     */
    if (stack_iface_) {
        BluetoothAudioSessionReport_2_1::ReportControlStatus(session_type_, true, status);
    } else {
...
Return<void> BluetoothAudioProvider::streamSuspended(BluetoothAudioStatus status) {
     ...
     */
    if (stack_iface_) {
        BluetoothAudioSessionReport_2_1::ReportControlStatus(session_type_, false, status);
    } else {
        LOG(WARNING) << __func__ << " - SessionType=" << toString(session_type_)
                     << ", status=" << toString(status) << " has NO session";
    }
