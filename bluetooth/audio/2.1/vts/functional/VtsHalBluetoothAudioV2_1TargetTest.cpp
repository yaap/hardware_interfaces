...
using ::android::hardware::bluetooth::audio::V2_1::IBluetoothAudioProvidersFactory;
using ::android::hardware::bluetooth::audio::V2_1::SampleRate;

using DataMQ = MessageQueue<uint8_t, kSynchronizedReadWrite>;
using BluetoothAudioStatus = ::android::hardware::bluetooth::audio::V2_0::Status;
using CodecSpecificConfig =
        ::android::hardware::bluetooth::audio::V2_0::CodecConfiguration::CodecSpecific;
...
constexpr BitsPerSample a2dp_bits_per_samples[4] = {BitsPerSample::BITS_UNKNOWN,
                                                    BitsPerSample::BITS_16, BitsPerSample::BITS_24,
                                                    BitsPerSample::BITS_32};
constexpr ChannelMode a2dp_channel_modes[3] = {ChannelMode::UNKNOWN, ChannelMode::MONO,
                                               ChannelMode::STEREO};
constexpr CodecType a2dp_codec_types[6] = {CodecType::UNKNOWN, CodecType::SBC,     CodecType::AAC,
                                           CodecType::APTX,    CodecType::APTX_HD, CodecType::LDAC};

template <typename T>
std::vector<T> ExtractValuesFromBitmask(T bitmasks, uint32_t bitfield, bool supported) {
    std::vector<T> retval;
    if (!supported) {
    ...
    while (test_bit <= static_cast<uint32_t>(bitmasks) && test_bit <= bitfield) {
        if ((bitfield & test_bit)) {
            if ((!(bitmasks & test_bit) && !supported) || ((bitmasks & test_bit) && supported)) {
...
class BluetoothAudioProvidersFactoryHidlTest : public ::testing::TestWithParam<std::string> {
  public:
    virtual void SetUp() override {
        providers_factory_ = IBluetoothAudioProvidersFactory::getService(GetParam());
    ...
    class BluetoothAudioPort
        : public ::testing::VtsHalHidlTargetCallbackBase<BluetoothAudioProvidersFactoryHidlTest>,
          public IBluetoothAudioPort {
        BluetoothAudioProvidersFactoryHidlTest& parent_;

      public:
        BluetoothAudioPort(BluetoothAudioProvidersFactoryHidlTest& parent) : parent_(parent) {}
    ...
    void GetProviderCapabilitiesHelper(const SessionType& session_type) {
        ...
        auto hidl_retval = providers_factory_->getProviderCapabilities(session_type, hidl_cb);
        ...
        if (session_type == SessionType::UNKNOWN) {
            ...
            for (auto audio_capability : temp_provider_capabilities_) {
                ...
                const CodecCapabilities& codec_capabilities = audio_capability.codecCapabilities();
                // Every codec can present once at most
                ASSERT_EQ(codec_type_bitmask & static_cast<uint32_t>(codec_capabilities.codecType),
                          0);
    ...
    void GetProviderCapabilitiesHelper_2_1(
        ...
        auto hidl_retval = providers_factory_->getProviderCapabilities_2_1(session_type, hidl_cb);
        // HIDL calls should not be failed and callback has to be executed
        ASSERT_TRUE(hidl_retval.isOk());
        ...
        ASSERT_EQ(temp_provider_capabilities_2_1_[0].getDiscriminator(),
                  android::hardware::bluetooth::audio::V2_1::AudioCapabilities::hidl_discriminator::
                          pcmCapabilities);
    }
    ...
    void OpenProviderHelper_2_1(
        ...
        BluetoothAudioStatus cb_status;
        auto hidl_cb =
                [&cb_status, &local_provider = this->audio_provider_2_1_](
                        BluetoothAudioStatus status,
                        const sp<
    ...
    bool IsPcmParametersSupported_2_1(
            const android::hardware::bluetooth::audio::V2_1::PcmParameters& pcm_parameters) {
        ...
        bool is_parameter_valid = (pcm_parameters.sampleRate != SampleRate::RATE_UNKNOWN &&
                                   pcm_parameters.channelMode != ChannelMode::UNKNOWN &&
                                   pcm_parameters.bitsPerSample != BitsPerSample::BITS_UNKNOWN &&
 ...
 */
TEST_P(BluetoothAudioProvidersFactoryHidlTest, OpenProviderAndCheckCapabilitiesBySession) {
 ...
 */
class BluetoothAudioProviderA2dpSoftwareHidlTest : public BluetoothAudioProvidersFactoryHidlTest {
  public:
    virtual void SetUp() override {
...
TEST_P(BluetoothAudioProviderA2dpSoftwareHidlTest,
    ...
    auto hidl_cb = [&is_codec_config_valid, &tempDataMQ](BluetoothAudioStatus status,
 ...
 */
class BluetoothAudioProviderA2dpHardwareHidlTest : public BluetoothAudioProvidersFactoryHidlTest {
  public:
    virtual void SetUp() override {
        BluetoothAudioProvidersFactoryHidlTest::SetUp();
    ...
    std::vector<CodecSpecificConfig> GetSbcCodecSpecificSupportedList(bool supported) {
        ...
        SbcParameters sbc_capability = temp_codec_capabilities_.capabilities.sbcCapabilities();
        if (sbc_capability.minBitpool > sbc_capability.maxBitpool) {
            return sbc_codec_specifics;
        }
        std::vector<android::hardware::bluetooth::audio::V2_0::SampleRate> sample_rates =
                ExtractValuesFromBitmask<android::hardware::bluetooth::audio::V2_0::SampleRate>(
                        sbc_capability.sampleRate, 0xff, supported);
        std::vector<SbcChannelMode> channel_modes = ExtractValuesFromBitmask<SbcChannelMode>(
                sbc_capability.channelMode, 0x0f, supported);
        std::vector<SbcBlockLength> block_lengths = ExtractValuesFromBitmask<SbcBlockLength>(
                sbc_capability.blockLength, 0xf0, supported);
        std::vector<SbcNumSubbands> num_subbandss = ExtractValuesFromBitmask<SbcNumSubbands>(
                sbc_capability.numSubbands, 0x0c, supported);
        std::vector<SbcAllocMethod> alloc_methods = ExtractValuesFromBitmask<SbcAllocMethod>(
                sbc_capability.allocMethod, 0x03, supported);
        std::vector<BitsPerSample> bits_per_samples = ExtractValuesFromBitmask<BitsPerSample>(
                sbc_capability.bitsPerSample, 0x07, supported);
    ...
    std::vector<CodecSpecificConfig> GetAacCodecSpecificSupportedList(bool supported) {
        ...
        // parse the capability
        AacParameters aac_capability = temp_codec_capabilities_.capabilities.aacCapabilities();
        std::vector<AacObjectType> object_types =
                ExtractValuesFromBitmask<AacObjectType>(aac_capability.objectType, 0xf0, supported);
        std::vector<android::hardware::bluetooth::audio::V2_0::SampleRate> sample_rates =
                ExtractValuesFromBitmask<android::hardware::bluetooth::audio::V2_0::SampleRate>(
                        aac_capability.sampleRate, 0xff, supported);
        std::vector<ChannelMode> channel_modes =
                ExtractValuesFromBitmask<ChannelMode>(aac_capability.channelMode, 0x03, supported);
        std::vector<AacVariableBitRate> variable_bit_rate_enableds = {AacVariableBitRate::DISABLED};
        ...
        std::vector<BitsPerSample> bits_per_samples = ExtractValuesFromBitmask<BitsPerSample>(
                aac_capability.bitsPerSample, 0x07, supported);
        // combine those parameters into one list of
        // CodecConfiguration::CodecSpecific
        CodecSpecificConfig codec_specific = {};
    ...
    std::vector<CodecSpecificConfig> GetLdacCodecSpecificSupportedList(bool supported) {
        ...
        // parse the capability
        LdacParameters ldac_capability = temp_codec_capabilities_.capabilities.ldacCapabilities();
        std::vector<android::hardware::bluetooth::audio::V2_0::SampleRate> sample_rates =
                ExtractValuesFromBitmask<android::hardware::bluetooth::audio::V2_0::SampleRate>(
                        ldac_capability.sampleRate, 0xff, supported);
        std::vector<LdacChannelMode> channel_modes = ExtractValuesFromBitmask<LdacChannelMode>(
                ldac_capability.channelMode, 0x07, supported);
        ...
        std::vector<BitsPerSample> bits_per_samples = ExtractValuesFromBitmask<BitsPerSample>(
                ldac_capability.bitsPerSample, 0x07, supported);
        // combine those parameters into one list of
        // CodecConfiguration::CodecSpecific
        CodecSpecificConfig codec_specific = {};
    ...
    std::vector<CodecSpecificConfig> GetAptxCodecSpecificSupportedList(bool is_hd, bool supported) {
        std::vector<CodecSpecificConfig> aptx_codec_specifics;
        GetOffloadCodecCapabilityHelper((is_hd ? CodecType::APTX_HD : CodecType::APTX));
        ...
        // parse the capability
        AptxParameters aptx_capability = temp_codec_capabilities_.capabilities.aptxCapabilities();
        std::vector<android::hardware::bluetooth::audio::V2_0::SampleRate> sample_rates =
                ExtractValuesFromBitmask<android::hardware::bluetooth::audio::V2_0::SampleRate>(
                        aptx_capability.sampleRate, 0xff, supported);
        std::vector<ChannelMode> channel_modes =
                ExtractValuesFromBitmask<ChannelMode>(aptx_capability.channelMode, 0x03, supported);
        std::vector<BitsPerSample> bits_per_samples = ExtractValuesFromBitmask<BitsPerSample>(
                aptx_capability.bitsPerSample, 0x07, supported);
        // combine those parameters into one list of
        // CodecConfiguration::CodecSpecific
        CodecSpecificConfig codec_specific = {};
 ...
 */
TEST_P(BluetoothAudioProviderA2dpHardwareHidlTest, StartAndEndA2dpSbcHardwareSession) {
    ...
    AudioConfiguration audio_config = {};
    std::vector<CodecSpecificConfig> sbc_codec_specifics = GetSbcCodecSpecificSupportedList(true);
    auto hidl_cb = [](BluetoothAudioStatus status, const DataMQ::Descriptor& dataMQ) {
 ...
 */
TEST_P(BluetoothAudioProviderA2dpHardwareHidlTest, StartAndEndA2dpAacHardwareSession) {
    if (!IsOffloadSupported()) {
        return;
    }
    ...
    codec_config.isScmstEnabled = false;
    AudioConfiguration audio_config = {};
    std::vector<CodecSpecificConfig> aac_codec_specifics = GetAacCodecSpecificSupportedList(true);
    auto hidl_cb = [](BluetoothAudioStatus status, const DataMQ::Descriptor& dataMQ) {
        EXPECT_EQ(status, BluetoothAudioStatus::SUCCESS);
        EXPECT_FALSE(dataMQ.isHandleValid());
    };
 ...
 */
TEST_P(BluetoothAudioProviderA2dpHardwareHidlTest, StartAndEndA2dpLdacHardwareSession) {
    if (!IsOffloadSupported()) {
        return;
    }
    ...
    codec_config.isScmstEnabled = false;
    AudioConfiguration audio_config = {};
    std::vector<CodecSpecificConfig> ldac_codec_specifics = GetLdacCodecSpecificSupportedList(true);
    auto hidl_cb = [](BluetoothAudioStatus status, const DataMQ::Descriptor& dataMQ) {
        EXPECT_EQ(status, BluetoothAudioStatus::SUCCESS);
        EXPECT_FALSE(dataMQ.isHandleValid());
    };
 ...
 */
TEST_P(BluetoothAudioProviderA2dpHardwareHidlTest, StartAndEndA2dpAptxHardwareSession) {
    if (!IsOffloadSupported()) {
        return;
    }
    ...
    for (auto codec_type : {CodecType::APTX, CodecType::APTX_HD}) {
        ...
        codec_config.isScmstEnabled = false;
        AudioConfiguration audio_config = {};
        std::vector<CodecSpecificConfig> aptx_codec_specifics = GetAptxCodecSpecificSupportedList(
 ...
 */
TEST_P(BluetoothAudioProviderHearingAidSoftwareHidlTest, OpenHearingAidSoftwareProvider) {}
...
class BluetoothAudioProviderLeAudioOutputSoftwareHidlTest
  ...
  public:
    virtual void SetUp() override {
        BluetoothAudioProvidersFactoryHidlTest::SetUp();
        GetProviderCapabilitiesHelper_2_1(android::hardware::bluetooth::audio::V2_1::SessionType::
                                                  LE_AUDIO_SOFTWARE_ENCODING_DATAPATH);
        OpenProviderHelper_2_1(android::hardware::bluetooth::audio::V2_1::SessionType::
                                       LE_AUDIO_SOFTWARE_ENCODING_DATAPATH);
        ASSERT_NE(audio_provider_2_1_, nullptr);
    }
    ...
    static constexpr ChannelMode le_audio_output_channel_modes_[3] = {
            ChannelMode::UNKNOWN, ChannelMode::MONO, ChannelMode::STEREO};
    static constexpr uint32_t le_audio_output_data_interval_us_[2] = {0 /* Invalid */,
                                                                      10000 /* Valid 10ms */};
};
 ...
 * stopped
 */
TEST_P(BluetoothAudioProviderLeAudioOutputSoftwareHidlTest, OpenLeAudioOutputSoftwareProvider) {}
        ...
        for (auto bits_per_sample : le_audio_output_bits_per_samples_) {
            ...
            for (auto channel_mode : le_audio_output_channel_modes_) {
                ...
                for (auto data_interval_us : le_audio_output_data_interval_us_) {
                    ...
                    audio_config.pcmConfig(pcm_parameters);
                    auto hidl_retval = audio_provider_2_1_->startSession_2_1(audio_port_,
                                                                             audio_config, hidl_cb);
                    // HIDL calls should not be failed and callback has to be executed
                    ASSERT_TRUE(hidl_retval.isOk());
                    if (is_codec_config_valid) {
                        EXPECT_TRUE(tempDataMQ != nullptr && tempDataMQ->isValid());
 ...
 * stopped
 */
TEST_P(BluetoothAudioProviderLeAudioInputSoftwareHidlTest, OpenLeAudioInputSoftwareProvider) {}
...
static const std::vector<std::string> kAudioInstances =
        android::hardware::getAllHalInstanceNames(IBluetoothAudioProvidersFactory::descriptor);

GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(BluetoothAudioProvidersFactoryHidlTest);
INSTANTIATE_TEST_SUITE_P(PerInstance, BluetoothAudioProvidersFactoryHidlTest,
                         testing::ValuesIn(kAudioInstances),
                         android::hardware::PrintInstanceNameToString);

GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(BluetoothAudioProviderA2dpSoftwareHidlTest);
INSTANTIATE_TEST_SUITE_P(PerInstance, BluetoothAudioProviderA2dpSoftwareHidlTest,
                         testing::ValuesIn(kAudioInstances),
                         android::hardware::PrintInstanceNameToString);

GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(BluetoothAudioProviderA2dpHardwareHidlTest);
INSTANTIATE_TEST_SUITE_P(PerInstance, BluetoothAudioProviderA2dpHardwareHidlTest,
                         testing::ValuesIn(kAudioInstances),
                         android::hardware::PrintInstanceNameToString);

GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(BluetoothAudioProviderHearingAidSoftwareHidlTest);
INSTANTIATE_TEST_SUITE_P(PerInstance, BluetoothAudioProviderHearingAidSoftwareHidlTest,
                         testing::ValuesIn(kAudioInstances),
                         android::hardware::PrintInstanceNameToString);

GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(BluetoothAudioProviderLeAudioOutputSoftwareHidlTest);
INSTANTIATE_TEST_SUITE_P(PerInstance, BluetoothAudioProviderLeAudioOutputSoftwareHidlTest,
                         testing::ValuesIn(kAudioInstances),
                         android::hardware::PrintInstanceNameToString);

GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(BluetoothAudioProviderLeAudioInputSoftwareHidlTest);
INSTANTIATE_TEST_SUITE_P(PerInstance, BluetoothAudioProviderLeAudioInputSoftwareHidlTest,
                         testing::ValuesIn(kAudioInstances),
                         android::hardware::PrintInstanceNameToString);
