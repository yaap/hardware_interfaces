...
#define COMMAND_HCI_READ_LOCAL_VERSION_INFORMATION {0x01, 0x10, 0x00}
#define COMMAND_HCI_READ_BUFFER_SIZE {0x05, 0x10, 0x00}
#define COMMAND_HCI_WRITE_LOOPBACK_MODE_LOCAL {0x02, 0x18, 0x01, 0x01}
#define COMMAND_HCI_RESET {0x03, 0x0c, 0x00}
#define COMMAND_HCI_WRITE_LOCAL_NAME {0x13, 0x0c, 0xf8}
...
class BluetoothHidlTest : public ::testing::TestWithParam<std::string> {
  public:
    virtual void SetUp() override {
        ...
        ASSERT_TRUE(bluetooth_cb->WaitForCallback(kCallbackNameInitializationComplete).no_timeout);
...
void BluetoothHidlTest::handle_no_ops() {
    while (event_queue.size() > 0) {
        ...
        ASSERT_GE(event.size(), static_cast<size_t>(EVENT_COMMAND_COMPLETE_STATUS_BYTE));
...
void BluetoothHidlTest::sendAndCheckSCO(int num_packets, size_t size, uint16_t handle) {
    ...
    for (int n = 0; n < num_packets; n++) {
        ...
        ASSERT_TRUE(bluetooth_cb->WaitForCallback(kCallbackNameScoEventReceived).no_timeout);
...
void BluetoothHidlTest::sendAndCheckACL(int num_packets, size_t size, uint16_t handle) {
    ...
    for (int n = 0; n < num_packets; n++) {
        ...
        ASSERT_TRUE(bluetooth_cb->WaitForCallback(kCallbackNameAclEventReceived).no_timeout);
...
void BluetoothHidlTest::enterLoopbackMode(std::vector<uint16_t>& sco_handles,
    ...
    while (true) {
        ...
        hidl_vec<uint8_t> event = event_queue.front();
        event_queue.pop();
        ASSERT_GT(event.size(), static_cast<size_t>(EVENT_COMMAND_COMPLETE_STATUS_BYTE));
        if (event[EVENT_CODE_BYTE] == EVENT_CONNECTION_COMPLETE) {
            ASSERT_GT(event.size(), static_cast<size_t>(EVENT_CONNECTION_COMPLETE_TYPE));
            ASSERT_EQ(event[EVENT_LENGTH_BYTE], EVENT_CONNECTION_COMPLETE_PARAM_LENGTH);
...
TEST_P(BluetoothHidlTest, HciUnknownCommand) {
    ...
    if (event[EVENT_CODE_BYTE] == EVENT_COMMAND_COMPLETE) {
        ASSERT_EQ(cmd[0], event[EVENT_COMMAND_COMPLETE_OPCODE_LSBYTE]);
        ASSERT_EQ(cmd[1], event[EVENT_COMMAND_COMPLETE_OPCODE_LSBYTE + 1]);
        ASSERT_EQ(HCI_STATUS_UNKNOWN_HCI_COMMAND, event[EVENT_COMMAND_COMPLETE_STATUS_BYTE]);
    } else {
        ...
        ASSERT_EQ(HCI_STATUS_UNKNOWN_HCI_COMMAND, event[EVENT_COMMAND_STATUS_STATUS_BYTE]);
    }
}
...
INSTANTIATE_TEST_SUITE_P(
        ...
        testing::ValuesIn(android::hardware::getAllHalInstanceNames(IBluetoothHci::descriptor)),
